...
#ifndef __SYNO_KEXEC_TEST_H
#define __SYNO_KEXEC_TEST_H
...
static void test_e820_table(void)
{
	...
	for (i = 0; i < boot_params.e820_entries; ++i) {
		if (E820_RAM == boot_params.e820_map[i].type &&
				0x100 == boot_params.e820_map[i].addr) {
			passed = false;
			break;
		}
	}
...
static void remove_decompression_setup_data(void)
{
	...
	while (pa_data) {
		...
		if (__DECOMPRESSION_TYPE__ == data_type) {
			break;
		}
 ...
 */
static int __init syno_kexec_test_init(void)
{
	test_decompression();
	test_bootloader();
	test_e820_table();
...
/*
 * The following are available init levels for this module.
 *   - early
 *   - core
 *   - postcore
 *
 * See also remove_decompression_setup_data() in this file and
 * initcall_level_names in init/main.c
 */
postcore_initcall(syno_kexec_test_init);
#endif /* MY_DEF_HERE */

#endif /* __SYNO_KEXEC_TEST_H */
