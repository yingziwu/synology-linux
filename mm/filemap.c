			...
			size_t count, size_t * rbytes, size_t * wbytes)
{
	...
	*wbytes = 0;
...
#ifdef MY_ABC_HERE
	if (mapping->a_ops->recvfile_da_check) {
		if (mapping->a_ops->recvfile_da_check(inode->i_sb)) {
	...
	if (mapping->a_ops->aggregate_write_end) {
		...
		for (	; crgPagePtr < cPagesAllocated; crgPagePtr++) {
			page = rgPageList[crgPagePtr];
			kunmap(page);
		}
		write_end_ret = mapping->a_ops->aggregate_write_end(
				file, mapping, rgPos[0],
				kernel_recvmsg_ret, kernel_recvmsg_ret,
				rgPageList, NULL, cPagesAllocated);
			...
			size_t count, size_t * rbytes, size_t * wbytes, unsigned flush_only)
{
	...
	size_t          cBytesToReceive = 0;
	int             crgPagePtr = 0;
	unsigned		cBytesWritten = 0;
	unsigned		totalBytes = 0;
	unsigned		isPageAlign = 1;
	...
	*wbytes = 0;
	...
	cPagesAllocated_LastTime = cPagesAllocated;
	do {
		pgoff_t         offset;
		char           *kaddr;
		...
		if (cPagesAllocated > MAX_PAGES_PER_AGGREGATE_RECVFILE + 1) {
			panic("allocate %d pages in do_recvfile()\n",
					cPagesAllocated);
		}
	...
	msg.msg_name = NULL;
	msg.msg_namelen = 0;
	msg.msg_iov = (struct iovec *) &iov[cPagesAllocated_LastTime];
	msg.msg_iovlen = cPagesAllocated - cPagesAllocated_LastTime;
	msg.msg_control = NULL;
	msg.msg_controllen = 0;
	msg.msg_flags = MSG_KERNSPACE;
	rcvtimeo = sock->sk->sk_rcvtimeo;
	...
	kernel_recvmsg_ret = kernel_recvmsg(
			sock, &msg, &iov[cPagesAllocated_LastTime], cPagesAllocated - cPagesAllocated_LastTime, cBytesToReceive,
			MSG_WAITALL | MSG_NOCATCHSIGNAL);
	...
	if (!err && isPageAlign && (cPagesAllocated <= MAX_PAGES_PER_AGGREGATE_RECVFILE - MAX_PAGES_PER_RECVFILE)) {
	...
	crgPagePtr = 0;
	cBytesWritten = bytes_received;
	totalBytes = 0;

	for (	; crgPagePtr < cPagesAllocated; crgPagePtr++) {
		page = rgPageList[crgPagePtr];
		totalBytes += rgBytes[crgPagePtr];
		kunmap(page);
	}
	write_end_ret = mapping->a_ops->aggregate_write_end(
			file, mapping, rgPos[0],
...
int do_aggregate_recvfile_flush(int fd)
{
	int ret = 0;
...
EXPORT_SYMBOL(do_aggregate_recvfile_flush);
#endif /* MY_ABC_HERE */
#endif
