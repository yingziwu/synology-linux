...
#define mvOsIoRemap	    ioremap
#define mvOsVirtToPhys	    __pa
...
#define MV_REG_READ(offset)             \
        (MV_MEMIO_LE32_READ(KIRKWOOD_REGS_VIRT_BASE | (offset)))

#if defined(REG_DEBUG)
#define MV_REG_WRITE(offset, val)    \
        MV_MEMIO_LE32_WRITE((KIRKWOOD_REGS_VIRT_BASE | (offset)), (val)); \
        { \
                reg_arry[reg_arry_index][0] = (KIRKWOOD_REGS_VIRT_BASE | (offset));\
...
#define MV_REG_BYTE_READ(offset)        \
        (MV_MEMIO8_READ((KIRKWOOD_REGS_VIRT_BASE | (offset))))

#if defined(REG_DEBUG)
#define MV_REG_BYTE_WRITE(offset, val)  \
        MV_MEMIO8_WRITE((KIRKWOOD_REGS_VIRT_BASE | (offset)), (val)); \
        { \
                reg_arry[reg_arry_index][0] = (KIRKWOOD_REGS_VIRT_BASE | (offset));\
                reg_arry[reg_arry_index][1] = (val);\
                reg_arry_index++;\
...
#define MV_REG_BIT_SET(offset, bitMask)                 \
        (MV_MEMIO32_WRITE((KIRKWOOD_REGS_VIRT_BASE | (offset)), \
         (MV_MEMIO32_READ(KIRKWOOD_REGS_VIRT_BASE | (offset)) | \
          MV_32BIT_LE_FAST(bitMask)))); \
        { \
                reg_arry[reg_arry_index][0] = (KIRKWOOD_REGS_VIRT_BASE | (offset));\
                reg_arry[reg_arry_index][1] = (MV_MEMIO32_READ(KIRKWOOD_REGS_VIRT_BASE | (offset)));\
                reg_arry_index++;\
        }
#else
#define MV_REG_BIT_SET(offset, bitMask)                 \
