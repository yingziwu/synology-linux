...
#ifdef CESA_OCF_TASKLET
	disable_irq(IRQ_KIRKWOOD_CRYPTO);
#endif
...
#ifdef CESA_OCF_TASKLET
	enable_irq(IRQ_KIRKWOOD_CRYPTO);
#endif
...
cesa_ocf_init(void)
{
	...
	pSram = ioremap(KIRKWOOD_SRAM_PHYS_BASE, KIRKWOOD_SRAM_SIZE);
	if (!pSram)
		return EINVAL;

	if (mvCesaHalInit(CESA_OCF_MAX_SES * 5, CESA_Q_SIZE, pSram,
			  KIRKWOOD_SRAM_PHYS_BASE, NULL) != MV_OK) {
		printk("%s,%d: mvCesaHalInit Failed. \n", __FILE__, __LINE__);
		return EINVAL;
	}
...
#endif
	/* register interrupt */
	if (request_irq(IRQ_KIRKWOOD_CRYPTO, cesa_interrupt_handler,
...
cesa_ocf_exit(void)
{
	...
	free_irq(IRQ_KIRKWOOD_CRYPTO, NULL);
