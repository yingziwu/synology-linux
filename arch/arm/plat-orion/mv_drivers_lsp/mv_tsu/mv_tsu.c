...
int mvtsu_release (struct inode *inode, struct file *filp)
{
	...
	free_irq(IRQ_KIRKWOOD_TS(dev->port),dev);
 ...
 * TSU ioctl()
 */
static long mvtsu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct mvtsu_dev *dev = filp->private_data;
	int ret = 0;
	u32 val;
...
int mvtsu_init(void)
{
	...
	mv_conf_mbus_windows((void __iomem *)TSU_PHYS_BASE, dram);
	TSU_DPRINT(TSU_DBG_INIT, ("\tTSU windows initialized successfully.\n"));

	/* Perform unit initialization.	*/
	if (mvTsuHalInit(cfg_core_clk, cfg_tsu_mode, NULL) != MV_OK) {
		printk("%s: mvTsuHalInit failed\n", __func__);
		result = -EINVAL;
		goto fail_init;
	}
