...
static ssize_t mv_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int off = 0, switch_num = 0;

	if (!capable(CAP_NET_ADMIN))
		return -EPERM;

	/* Read arguments */
	sscanf(buf, "%d", &switch_num);

	if (!strcmp(name, "stats"))
		mv_switch_stats_print(switch_num);
	else if (!strcmp(name, "status"))
		mv_switch_status_print(switch_num);
	else
...
static ssize_t mv_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	const char      *name = attr->attr.name;
	unsigned long   flags;
	int             err, switch_num, port, reg, type;
	unsigned int    v;
	MV_U16          val;
	...
	/* Read arguments */
	err = switch_num = port = reg = type = val = 0;
	sscanf(buf, "%d %d %d %d %x", &switch_num, &port, &reg, &type, &v);

	local_irq_save(flags);
	if (!strcmp(name, "reg_r")) {
		err = mv_switch_reg_read(switch_num, port, reg, type, &val);
	} else if (!strcmp(name, "reg_w")) {
		val = (MV_U16)v;
		err = mv_switch_reg_write(switch_num, port, reg, type, v);
	}
