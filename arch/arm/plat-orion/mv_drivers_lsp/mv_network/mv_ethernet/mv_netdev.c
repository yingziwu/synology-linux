...
#include "mv_xcat_pp/xcat98dx-switch.h"
#include "mv_xcat_pp/xcat98dx-core.h"
...
/*
 * mvMacAddr and mvMtu should be inited in machine init stage
 */
u8 mvMacAddr[CONFIG_MV_ETH_PORTS_NUM][6] = {{0}};
...
static int mac_addr_setup(char *str)
{
	int i;

	str2eaddr(mvMacAddr[0], str);

	for (i = 1; i < CONFIG_MV_ETH_PORTS_NUM; i++) {
		memcpy(mvMacAddr[i], mvMacAddr[0], ETH_ALEN);
		mvMacAddr[i][ETH_ALEN - 1] += i;
	}

	return 1;
}

__setup("eth_mac_addr=", mac_addr_setup);
...
spinlock_t          mii_lock = __SPIN_LOCK_UNLOCKED(mii_lock);
spinlock_t          nfp_lock = __SPIN_LOCK_UNLOCKED(nfp_lock);
...
static INLINE struct sk_buff*   eth_skb_alloc(mv_eth_priv *priv, MV_PKT_INFO* pPktInfo)
{
    ...
    if (!skb) {
        ETH_DBG( ETH_DBG_RX_FILL, ("port %d: rx_fill cannot allocate skb\n",priv->port) );
...
static INLINE void  eth_skb_free(mv_eth_priv *priv, MV_PKT_INFO* pPktInfo)
{
    ...
    if(pPktInfo->ownerId != ~0)
    {
        ...
        if ( (!priv->isGtw) || (priv->isGtw && priv->useDsaTag) )
...
static INLINE int eth_rx(struct net_device *dev, unsigned int work_to_do, int queue)
{
    ...
    while( work_done < work_to_do )
    {
        ...
        if(priv->isGtw)
        {
	        dev = mv_gtw_ingress_dev(data, priv);
            if (dev) {
        ...
        if ( (nfp) && !(priv->useDsaTag) ) /* TODO: NFP not supported if ingress interface uses DSA tag */
#else
        if (nfp) /* TODO: NFP not supported if ingress interface uses DSA tag */
#endif /* CONFIG_MV_GATEWAY */
        {
            int txq = ETH_DEF_TXQ;
            out_dev = NULL;
		    ...
		    mv_eth_priv *out_priv = MV_ETH_PRIV(out_dev);
                    ...
                    if (out_priv->isGtw)
                    {
                        /* TODO: NFP not supported if egress interface uses DSA tag */
                        if (!(out_priv->useDsaTag)) {
                            struct mv_vlan_cfg* vlan_cfg = MV_NETDEV_VLAN(out_dev);
                            *(unsigned short *)(pPktInfo->pFrags->bufVirtPtr) = vlan_cfg->header;
                        }
                        else {
                            printk("Error: NFP is not supported when working with DSA tag\n");
                        }
                    }
                    else
#endif /* CONFIG_MV_GATEWAY */
...
#if defined(CONFIG_MV_GATEWAY)
                        if ( (!out_priv->isGtw) || (out_priv->isGtw && out_priv->useDsaTag) )
#endif /* CONFIG_MV_GATEWAY */
                        {
                            pPktInfo->pFrags->bufPhysAddr -= ETH_MV_HEADER_SIZE;
                        }
		...
		if (boardId == XCAT98DX_ID && priv->xcatUseDsaTag == MV_TRUE) {
			memmove(skb->data + priv->dsaTagLen, skb->data, 2 * ETH_ALEN);
			skb_pull(skb, priv->dsaTagLen);
		}

#ifdef CONFIG_MV_GATEWAY
        if(priv->isGtw && priv->useDsaTag)
        {
        ...
        else
        {
            ...
            {
                /* TODO: IGMP not supported for DSA tag mode */
                if (!(priv->useDsaTag))
                {
                    ...
                    ETH_DBG( ETH_DBG_RX, ("Error: IGMP is not supported when working with DSA tag\n"));
                }
            }
...
#else
	    napi_complete(napi);
        local_irq_save(flags); /* should be after calling napi_complete() */
#endif
...
#error "If TSO supported - TX checksum offload must be supported too"
    ...
    printk("     o DSA mode support enabled\n");
#endif
...
static int mv_eth_set_port_params(mv_eth_priv *priv)
{
	MV_BOARD_MAC_SPEED board_mac_speed;
	MV_ETH_PORT_SPEED eth_port_speed;
	int ret_val = 0;

	/* Force link, speed and duplex if necessary
	 * (e.g. Switch is connected) based on board information
	 */
	board_mac_speed = mvBoardMacSpeedGet(priv->port);
	if (mvBoardMacSpeedToEthPortSpeed(board_mac_speed, &eth_port_speed)) {
		ret_val = -EINVAL;
		goto out;
	}

	if (eth_port_speed == MV_ETH_SPEED_AN) {
		...
		/* Force all eth port parameters */
		if (mvEthForceLinkModeSet(priv->hal_priv, 1, 0)) {
			printk(KERN_ERR "mvEthForceLinkModeSet failed\n");
			ret_val = -EINVAL;
		}
		if (mvEthSpeedDuplexSet(priv->hal_priv, eth_port_speed, MV_ETH_DUPLEX_FULL)) {
			printk(KERN_ERR "mvEthSpeedDuplexSet failed\n");
			ret_val = -EINVAL;
		}
...
int     mv_eth_start_internals(mv_eth_priv *priv, int mtu)
{
    ...
    err = mv_eth_set_port_params(priv);
    if (err)
	goto out;
...
static int eth_tso_tx(struct sk_buff *skb, struct net_device *dev, int txq)
{
	...
	frag_ptr = skb_frag_address(skb_frag_ptr);
        frag++;
    }
...
#ifdef CONFIG_MV_GATEWAY
	    /* Note: supports Marvell Header mode, not DSA tag mode */
        if (priv->isGtw)
        {
            /* TODO: TSO not supported for DSA tag mode */
            if (!(priv->useDsaTag))
            {
	            ...
	            *(unsigned short *)(extra_ptr) = vlan_cfg->header;
                ...
                printk("Error: TSO is not supported when working with DSA tag\n");
            }
        }
        else
#endif /* CONFIG_MV_GATEWAY */
...
static int eth_ufo_tx(struct sk_buff *skb, struct net_device *dev, int txq)
{
	...
	pPktInfo = (MV_PKT_INFO*)mvStackPop(priv->txPktInfoPool);
		...
		if (offset) {
            ...
            if(priv->isGtw)
            {
                /* TODO: UFO not supported for DSA tag mode */
                if (!(priv->useDsaTag))
                {
                    struct mv_vlan_cfg *vlan_cfg = MV_NETDEV_VLAN(dev);
		        ...
		        pPktInfo->pktSize = ETH_MV_HEADER_SIZE;
                    ...
                    printk("Error: UFO is not supported when working with DSA tag\n");
                }
            }
            else
#endif /* CONFIG_MV_GATEWAY */
		...
		while (fn < nr) {
			...
			data = skb_frag_address(skb_frag_ptr);
...
static int eth_tx( struct sk_buff *skb , struct net_device *dev )
{
    ...
    if (boardId == XCAT98DX_ID && priv->xcatUseDsaTag == MV_TRUE) {
	    /*
	     * Extended FROM_CPU DSA tag description:
	     * TrgTagged = 0 (bit 29)	==> packet is send via network port untagged
	     * Use_vidx = 1 (bit 18)	==> packet is sent to VLAN
	     * Vidx = 1 (bit[28:19])	==> packet is sent to Vidx 1
	     * VLAN = 1 (bit[11:0])	==> packet is sent to VLAN 1
	     */
	    unsigned char dsa[8] = {0x40,0x0C,0x10,0x01,0x00,0x00,0x07,0xFF};
	    unsigned int dsa_len = priv->dsaTagLen;

	    /* check if we have place inside skb for the DSA tag */
	    if (skb_headroom(skb) < dsa_len) {
		    /* allocate enough space for the DSA tag */
		    if (pskb_expand_head(skb, dsa_len, 0, GFP_ATOMIC)) {
			    printk("%s: pskb_expand_head() failed\n", __func__);
			    return -ENOMEM;
		    }
	    }
	    /* now we have enough space for the DSA tag */
	    memmove(skb->data - dsa_len, skb->data, 2 * ETH_ALEN);
	    memmove(skb->data + (2 * ETH_ALEN - dsa_len), dsa, dsa_len);
	    skb_push(skb, dsa_len);
    }
    ...
    if( skb_shinfo(skb)->nr_frags == 0 ) {
        ...
        for ( i = 0; i < skb_shinfo(skb)->nr_frags; i++ ) {
            ...
            p_buf_info->bufVirtPtr = skb_frag_address(frag);
            p_buf_info++;
        }
...
struct net_device* mv_netdev_init(mv_eth_priv *priv, int mtu, u8* mac)
{
    ...
    dev->irq = IRQ_KIRKWOOD_GE(priv->port);
    dev->mtu = mtu;
...
#endif
        if (priv->useDsaTag)
            dev->hard_header_len += MV_DSA_TAG_SIZE;
        else
...
void    mv_eth_netdev_print(struct net_device* dev)
{
    ...
    printk("features=0x%x, flags=0x%x, mtu=%u, size=%d\n",
    ...
    if(priv->isGtw)
    {
        ...
        if (vlan_cfg != NULL) {
            if (priv->useDsaTag)
            {
                printk("GTW: vlan_grp_id=0x%x, ports_mask[0]=0x%x, ports_mask[1]=0x%x, ports_link[0]=0x%x, ports_link[1]=0x%x, header=0x%04x\n",
                        vlan_cfg->vlan_grp_id, vlan_cfg->ports_mask[SWITCH_0], vlan_cfg->ports_mask[SWITCH_1],
            ...
            else
            {
                printk("GTW: vlan_grp_id=0x%x, ports_mask=0x%x, ports_link=0x%x, mv_header=0x%04x\n",
                        vlan_cfg->vlan_grp_id, vlan_cfg->ports_mask[SWITCH_0],
...
void print_skb(struct sk_buff* skb)
{
    ...
    for(i=0; i<skb_shinfo(skb)->nr_frags; i++)
    {
        printk("\t frag_%d. page=%p, page_offset=0x%x, size=%d\n",
            i, skb_frag_page(&skb_shinfo(skb)->frags[i]),
...
static int __init mv_eth_init_module( void )
{
	...
	if (boardId == XCAT98DX_ID) {
		/* DSA tag support */
		priv->xcatUseDsaTag = mvBoardUseDsaTagOnPort(port);
		if (priv->xcatUseDsaTag)
			priv->dsaTagLen = mvBoardDsaTagLenGet(port);
	}

	/* If ether address is *not* configured dynamically through __setup,
	 * leave it unchanged.
	 */
	if (!is_valid_ether_addr(mvMacAddr[port])) {
		if (mvEthMacAddrGet(port, mac_addr) != MV_OK) {
			printk("%s: mvEthMacAddrGet failed\n", __func__);
			return 1;
		}

		if (!is_valid_ether_addr(mac_addr)) {
			printk("%s: Error: read invalid MAC from port %d\n", __func__, port);
			return 1;
		}
    ...
    for (port = 0; port < mv_eth_ports_num; port++) {
        ...
        priv->isGtw = MV_TRUE;
        if (mvBoardNumSwitchesOnPortGet(port) > 1)
            priv->useDsaTag = MV_TRUE;
        else
            priv->useDsaTag = MV_FALSE;
...
#else
            ((mv_net_priv*)(netdev_priv(net_dev_ptr)))->vlan_cfg = &gtw_config.vlan_cfg[i];
#endif
            mv_net_devs[netdev] = net_dev_ptr;
            netdev++;
        }
...
int mv_eth_start(struct net_device *dev)
{
	...
	/*
	 * In xCat-98dx eth GbE1 can be connected _internally_
	 * to built-in-SoC Prestera switch CPU port (63)
	 */
	if (boardId == XCAT98DX_ID && priv->xcatUseDsaTag == MV_TRUE &&
						priv->port == 1) {
		xcat_config_internal_mii(priv);
		mvPpBridgeTeachNewAddr(dev->dev_addr);
		mvPpMruSetAllPorts(1600);
		setCpuAsVLANMember(MV_PP_DEV0, 1 /* VLAN */);
	}
