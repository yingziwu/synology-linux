...
void armadaxp_store_boot_info(void)
{
	...
	resume_pc = (int *)virt_to_phys(armada_cpu_resume);
	/*
	...
	/*
	 * Disable X-Bar window 12 which is opened in the BootROM
	 * by default to 0xF0000000
	 */
	writel(BOOTROM_INTER_REGS_PHYS_BASE + 0x200B0, store_addr++);
	 ...
	 */
	writel(BOOTROM_INTER_REGS_PHYS_BASE + 0x20080, store_addr++);
	writel(INTER_REGS_PHYS_BASE, store_addr++);

	/* Save AXI windows data */
	for (win = 0; win < 4; win++) {
		writel(INTER_REGS_PHYS_BASE + SDRAM_WIN_BASE_REG(win), store_addr++);
		writel(MV_REG_READ(SDRAM_WIN_BASE_REG(win)), store_addr++);

		writel(INTER_REGS_PHYS_BASE + SDRAM_WIN_CTRL_REG(win), store_addr++);
		writel(MV_REG_READ(SDRAM_WIN_CTRL_REG(win)), store_addr++);
	}
...
void armada_suspend(void)
{
	...
	cpu_suspend(0, armada_powerdown);
#ifdef DEBUG_DDR_SELF_REFRESH
	test_pattern();
#endif
	cpu_init();
