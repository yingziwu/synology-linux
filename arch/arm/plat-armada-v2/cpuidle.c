...
#if defined CONFIG_ARMADA_SUPPORT_DEEP_IDLE_CESA_USE
static MV_AHB_TO_MBUS_DEC_WIN ahbAddrDecWin;
#endif
...
{
	/* Restore CPU window 13 state */
	mvAhbToMbusWinSet(13, &ahbAddrDecWin);
}

void mv_cpuidle_set_cpu_win_state(void)
{
	...
	mvAhbToMbusWinGet(13, &ahbAddrDecWin);
...
void armadaxp_fabric_prepare_deepIdle(void)
{
	...
	MV_REG_WRITE(PM_CPU_BOOT_ADDR_REDIRECT(processor_id), virt_to_phys(armada_cpu_resume));
...
#ifdef CONFIG_ARMADA_XP_SW_SNOOP_DISABLE
