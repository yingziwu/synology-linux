...
static void board_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	...
	if (ctrl & NAND_CTRL_CHANGE) {        
		...
		switch(ctrl) {
			case NAND_CTRL_CLE: 
				this->IO_ADDR_W = (void __iomem *)((unsigned long)this->IO_ADDR_W | 1);
				 /* x8=>1, x16=>2 */
				break;
			case NAND_CTRL_ALE:
				this->IO_ADDR_W = (void __iomem *)((unsigned long)this->IO_ADDR_W | 2);
				/* x8=>2, x16=>4 */
				break;
		}				
	}
...
#ifdef  CONFIG_MTD_NAND_LNC_WITH_NOR
	/*
	 * XXX: Workaround for ECC layout mismatch between
	 *      u-boot and Linux. This is dangerous but allows
	 *      to boot both, u-boot and Linux rootfs from NAND.
	 *      Ultimately, u-boot's default ECC layout should be
	 *      matched with the Linux SOFT_BCH default or passed one.
	 */
	/* this->ecc.mode = NAND_ECC_SOFT_BCH; */
	this->ecc.mode = NAND_ECC_NONE;
	this->ecc.layout = NULL;
#else	/* !CONFIG_MTD_NAND_LNC_WITH_NOR */
#ifdef	CONFIG_MTD_NAND_LNC_8BYTE_READ
