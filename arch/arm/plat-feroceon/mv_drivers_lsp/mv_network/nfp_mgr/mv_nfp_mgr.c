...
struct nat_rule_hash_bucket {
	MV_FP_NAT_RULE *rule_chain;
};

static struct nat_rule_hash_bucket *mgr_nat_rule_db;
u32 mgr_nat_rule_db_size = 0;
...
/* global variables */
static struct rule_hash_bucket	*mgr_rule_db;
u32				mgr_rule_db_size = 0;
u32				mgr_rule_jhash_iv = 0;
...
static MV_FP_RULE* fp_rule_find(u32 sip, u32 dip)
{
	...
	hash = mv_jhash_3words(dip, sip, (u32)0, mgr_rule_jhash_iv);
	hash_tr = hash & (mgr_rule_db_size - 1);

	curr_rule = mgr_rule_db[hash_tr].rule_chain;
...
static MV_FP_NAT_RULE* fp_nat_rule_find(u32 sip, u32 dip, u16 sport, u16 dport, u8 proto) 
{
	...
	hash = mv_jhash_3words(sip, dip, (u32)((dport << 16) | sport), 
                            (u32)((mgr_rule_jhash_iv << 8) | proto));
	hash_tr = hash & (mgr_nat_rule_db_size - 1);

	for (curr_rule = mgr_nat_rule_db[hash_tr].rule_chain; curr_rule != NULL; curr_rule = curr_rule->next) {
...
static int fp_db_clear_and_update(void)
{
	...
	for (i = 0; i < mgr_rule_db_size; i++) {
		curr_rule = mgr_rule_db[i].rule_chain;
...
static void aging_timer_function(unsigned long data)
{
	...
	if( (curr_arp_rule == NULL) || 
        ((current_arp_rule_position*AGING_QUANTUM) >= mgr_arp_rule_db.db_size))
	...
	/* move pointer to rule to the next group of rules we want to handle */
	// liron, TODO: handle aging in quantas now that we use hash table DB
	...
	if ((curr_rule == NULL) || ((current_rule_position*AGING_QUANTUM) >= mgr_rule_db_size))
	...
	/* move pointer to point to the next group of rules we want to handle */
	// liron, TODO: handle aging in quantas now that we use hash table DB
	...
	if( (curr_nat_rule == NULL) || 
        	( (current_nat_rule_position*AGING_QUANTUM) >= mgr_nat_rule_db_size) )
			...
			curr_rule->mgmtInfo.new_count = mvFpRouteCountGet(curr_rule->routingInfo.srcIp, 
        		...
        		if(curr_rule->mgmtInfo.new_count != curr_rule->mgmtInfo.old_count)
        		{
            			...
            			else
            			{
                			...
                			curr_arp_rule = 
						fp_arp_rule_find(fp_eth_devs[curr_rule->routingInfo.inIfIndex].def_gtw_ip);
                			if(curr_arp_rule != NULL)
                			{
                    				curr_arp_rule->confirmed = 1;
                			}       		
...
int fp_is_route_confirmed(u32 src_ip, u32 dst_ip)
{
	...
	curr_rule = fp_rule_find(src_ip, dst_ip);
	if (curr_rule != NULL) 
	{
            	if(curr_rule->mgmtInfo.new_count != curr_rule->mgmtInfo.old_count)
            	{
...
int fp_mgr_init(void)
{
	...
	mgr_to_fp_chan = mvIpcChanCreate(MV_IPC_HOST_ID, MV_IPC_COPROCESSOR_ID,
                                        MV_IPC_RX_CB_MODE, MAX_MSG_SIZE, 64 /*mgr_rule_db_size*/);
	...
	fpdb_init_struct.rule_db_size = mgr_rule_db_size;
	...
	return 0;
#else
	/* Note at this stage the Manager DB is already initialized so mgr_rule_db_size is set */
	mvFpRuleDbInit(mgr_rule_db_size);
#endif /* CONFIG_MV_ETH_NFP_DUAL */
...
int fp_rule_db_init(u32 db_size)
{
	...
	mgr_rule_jhash_iv = mvOsRand();
	mgr_rule_db = (struct rule_hash_bucket *)kmalloc(sizeof(struct rule_hash_bucket)*db_size, GFP_ATOMIC);
	if (mgr_rule_db == NULL) {
		printk("fp_rule_db_init: Error allocating memory for NFP Manager Rule Database\n");
		return -ENOMEM;
	}
	mgr_rule_db_size = db_size;
	memset(mgr_rule_db, 0, sizeof(struct rule_hash_bucket)*db_size);
...
int fp_arp_db_init(u32 db_size)
{
	...
	mgr_arp_rule_db.db_size = db_size;
	return 0;
}
...
static int fp_rule_db_arp_update(u32 ip, const u8 *mac)
{
	...
	for (i = 0; i < mgr_rule_db_size; i++)
	{
		for (curr_rule = mgr_rule_db[i].rule_chain; curr_rule != NULL; curr_rule = curr_rule->next) 
    		{
...
int fp_rule_set(MV_FP_RULE *rule)
{
	...
	hash = mv_jhash_3words(rule->routingInfo.dstIp, rule->routingInfo.srcIp, (u32)0, mgr_rule_jhash_iv);
	hash_tr = hash & (mgr_rule_db_size - 1);

	if (mgr_rule_db[hash_tr].rule_chain == NULL) {
		/* There is no rule in this hash bucket yet */
		mgr_rule_db[hash_tr].rule_chain = new_rule;
	}
	else {
		/* Let's add this rule at the tail of the list */
		curr_rule = mgr_rule_db[hash_tr].rule_chain;		
...
int fp_rule_delete(u32 src_ip, u32 dst_ip, MV_FP_RULE_TYPE rule_type)
{
	...
	hash = mv_jhash_3words(dst_ip, src_ip, (u32)0, mgr_rule_jhash_iv);
	hash_tr = hash & (mgr_rule_db_size - 1);

	prev_rule = NULL;
	for(curr_rule = mgr_rule_db[hash_tr].rule_chain; 
		curr_rule != NULL; 
		prev_rule = curr_rule, curr_rule = curr_rule->next) {
		if( (curr_rule->routingInfo.srcIp == src_ip) && 
			...
			if (prev_rule == NULL) {
				mgr_rule_db[hash_tr].rule_chain = curr_rule->next;
			}
			else {
				prev_rule->next = curr_rule->next;
			}
...
{
	FP_MGR_DBG(FP_MGR_DBG_INIT, ("FP_MGR: Initializing NAT Rule DB, hash size = %d\n", db_size));
	mgr_nat_rule_db = (struct nat_rule_hash_bucket *)kmalloc(sizeof(struct nat_rule_hash_bucket)*db_size, GFP_ATOMIC);
	if (mgr_nat_rule_db == NULL) {
		printk("fp_nat_db_init: Error allocating memory for NFP Manager NAT Rule Database\n");
		return -ENOMEM;
	}
	mgr_nat_rule_db_size = db_size;
	memset(mgr_nat_rule_db, 0, sizeof(struct nat_rule_hash_bucket)*db_size);
#ifdef CONFIG_MV_ETH_NFP_DUAL
	return mvIpcChanMsgSend(mgr_to_fp_chan, MV_SERVICE_NFP_ID, MV_FP_NAT_DB_INIT_OPCODE, 
				&(mgr_nat_rule_db_size), sizeof(mgr_nat_rule_db_size));
#else
	return mvFpNatDbInit(mgr_nat_rule_db_size);
#endif /* CONFIG_MV_ETH_NFP_DUAL */
}
...
int fp_nat_db_clear(void)
{
	...
	for (i = 0; i < mgr_nat_rule_db_size; i++)
	{
		curr_rule = mgr_nat_rule_db[i].rule_chain;
		while (curr_rule != NULL) {
			tmp_rule = curr_rule;
			curr_rule = curr_rule->next;
			kfree(tmp_rule);
		}
		mgr_nat_rule_db[i].rule_chain = NULL;
	}
#ifdef CONFIG_MV_ETH_NFP_DUAL
...
static int fp_nat_db_clear_and_update(void)
{
	...
	for (i = 0; i < mgr_nat_rule_db_size; i++)
	{
		curr_nat_rule = mgr_nat_rule_db[i].rule_chain;
			        ...
			        int if_index, enum nf_nat_manip_type maniptype)
{
	...
	curr_rule = fp_nat_rule_find(src_ip, dst_ip, src_port, dst_port, proto);
	if (curr_rule != NULL) 
        {
		/* Updating existing rule */
		curr_rule->new_count = 1;
       		curr_rule->count = 1;
	...
	hash = mv_jhash_3words(src_ip, dst_ip, (u32)((dst_port << 16) | src_port), 
                            (u32)((mgr_rule_jhash_iv << 8) | proto));
	hash_tr = hash & (mgr_nat_rule_db_size - 1);

	if (mgr_nat_rule_db[hash_tr].rule_chain == NULL) {
		/* There is no rule in this hash bucket yet */
		mgr_nat_rule_db[hash_tr].rule_chain = new_rule;
	}
	else {
		/* Let's add this rule at the tail of the list */
		curr_rule = mgr_nat_rule_db[hash_tr].rule_chain;		
		while (curr_rule->next != NULL)
			curr_rule = curr_rule->next;
		curr_rule->next = new_rule;		
	}
...
int fp_nat_info_delete(u32 src_ip, u32 dst_ip, u16 src_port, u16 dst_port, u8 proto)
{
	...
	prev_rule = NULL;
	for (	curr_rule = mgr_nat_rule_db[hash_tr].rule_chain; 
		curr_rule != NULL; 
		prev_rule = curr_rule, curr_rule = curr_rule->next) {
		if (	curr_rule->srcIp == src_ip && 
			...
			if (prev_rule == NULL)
				mgr_nat_rule_db[hash_tr].rule_chain = curr_rule->next;	
			else
				prev_rule->next = curr_rule->next;
#ifdef CONFIG_MV_ETH_NFP_DUAL
			status = mvIpcChanMsgSend(mgr_to_fp_chan, MV_SERVICE_NFP_ID, 
...
int fp_is_nat_confirmed(u32 src_ip, u32 dst_ip, u16 src_port, u16 dst_port, u8 proto)
{
	...
	spin_lock_irqsave(&nfp_mgr_lock, flags);
	new_count = mvFpNatCountGet(src_ip, dst_ip, src_port, dst_port, proto);
	if (new_count != 0)        
       		confirmed = 1;
	spin_unlock_irqrestore(&nfp_mgr_lock, flags);
	return confirmed;
...
int fp_fdb_db_init(u32 db_size)
{
	...
	mgr_fdb_rule_db.db_size = db_size;
