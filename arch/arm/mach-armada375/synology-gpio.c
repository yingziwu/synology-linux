...
#define SYNO_LED_BLINKING	2
	...
	SYNO_SOC_HDD_LED_SET(int index, int status)
	{
		...
		int mode_gpio;
		int fail_led;
		int active = 0; //note: led low active

		#ifndef SYNO_TMP_DISABLE
...
static SYNO_GPIO gpio_375_1bay = {
	.ext_sata_led = {
...
static SYNO_GPIO gpio_375_2bay = {
	.ext_sata_led = {
		.hdd1_led_0 = GPIO_UNDEF,
		.hdd1_led_1 = GPIO_UNDEF,
		.hdd2_led_0 = GPIO_UNDEF,
...
static void
A375_1BAY_GPIO_init(SYNO_GPIO *global_gpio)
{
        *global_gpio = gpio_375_1bay;
}

static void
A375_2BAY_GPIO_init(SYNO_GPIO *global_gpio)
{
        *global_gpio = gpio_375_2bay;
}
...
void synology_gpio_init(void)
{
	...
	switch(boardId) {
        case SYNO_DS115_ID:
		A375_1BAY_GPIO_init(&generic_gpio);
		printk("Synology Armada375 1-bay GPIO Init\n");
		break;
	case SYNO_DS215j_ID:
		A375_2BAY_GPIO_init(&generic_gpio);
		printk("Synology Armada375 2-bay GPIO Init\n");
		break;
	default:
