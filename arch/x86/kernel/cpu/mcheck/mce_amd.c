...
static void threshold_restart_bank(void *_tr)
{
	...
	tr->reset = 0;
#endif /* SYNO_AMD_MCE_THRESHOLD_CLEAR */
}
...
#ifdef MY_DEF_HERE
		for (bank = 0; bank < mca_cfg.banks; ++bank) {
			if (!(per_cpu(bank_map, lcpu) & (1 << bank)))
				continue;

			first_block = per_cpu(threshold_banks, lcpu)[bank]->blocks;
			if (!first_block)
				continue;

			rdmsr_safe_on_cpu(lcpu, first_block->address, &lo, &hi);
			hi = (hi & ~(MASK_ERR_COUNT_HI | MASK_OVERFLOW_HI)) | (THRESHOLD_MAX - first_block->threshold_limit);
			err = wrmsr_safe_on_cpu(lcpu, first_block->address, lo, hi);

			list_for_each_entry_safe(block, tmp, &first_block->miscj, miscj) {
				rdmsr_safe_on_cpu(lcpu, block->address, &lo, &hi);
				hi = (hi & ~(MASK_ERR_COUNT_HI | MASK_OVERFLOW_HI)) | (THRESHOLD_MAX - block->threshold_limit);
				err = wrmsr_safe_on_cpu(lcpu, block->address, lo, hi);
			}
		}
#endif /* MY_DEF_HERE */
	}
