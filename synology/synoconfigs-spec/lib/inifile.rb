...
class IniFile
  ...
  def self.load( filename, opts = {} )
    raise Error, "file '#{filename}' not found" unless File.file? filename
  ...
  class Parser
    ...
    def process_property
      ...
      warn "Duplicated key '#{ANSI::Code.ansi(property, :yellow, :bright)}' found in [#{@section_name}]" if section.key?(property)
