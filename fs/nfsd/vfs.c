...
nfsd_synoclone(const char *srcPath, struct svc_rqst *rqstp, struct svc_fh *fhp)
{
	...
	if (0 != (err = nfsd_open(rqstp, fhp, S_IFREG, NFSD_MAY_WRITE, &dstFile))) {
		dprintk("nfsd_synoclone: cannot open destination file\n");
		goto out;
	}

	if (0 > (srcFd = do_sys_open(AT_FDCWD, srcPath, O_RDONLY | O_LARGEFILE, S_IRWXU))) {
	...
	if (0 != (err = (0 > btrfs_lazy_clone(dstFile, srcFd, 0, 0, 0)))) {
		dprintk("nfsd_synoclone: cannot open source file\n");
		goto old_fs;
	}
