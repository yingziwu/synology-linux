...
static int btrfs_ioctl_setflags(struct file *file, void __user *arg)
{
	...
	if (flags & FS_NOCOMP_FL) {
		...
		if (compress_type == BTRFS_COMPRESS_ZLIB)
			comp = "zlib";
		else if (compress_type == BTRFS_COMPRESS_ZSTD)
			comp = "zstd";
		else
			comp = "lzo";
#else
...
#ifdef MY_DEF_HERE
			   struct btrfs_qgroup_inherit *inherit, u64 copy_limit_from)
#else
			   struct btrfs_qgroup_inherit *inherit)
#endif
{
	...
	if (ret < 0)
		goto dec_and_free;
	reserve_usrquota_leafs = 1 + div_u64(reserve_usrquota_items, (u32)BTRFS_USRQUOTA_MAX_ITEMS_LEAF(root));
		      ...
		      u64 newer_than, unsigned long max_to_defrag)
{
	...
	u64 last_rec_pos = 0;
	/*
	 * 32bit platform does not support slow 64bit division.
	 * This is only log printing. Therefor, we change the times
	 * to power of 2 and use shift to avoid compiler error.
	 */
...
#ifdef MY_DEF_HERE
				     NULL, transid, readonly, inherit, copy_limit_from);
#else
...
#ifdef MY_DEF_HERE
					     transid, readonly, inherit, copy_limit_from);
#else
					     transid, readonly, inherit);
#endif /* MY_DEF_HERE */
#else /* MY_DEF_HERE */
					      ...
					      void __user *arg)
{
	...
	u64 flags;
	int ret = 0;
#if defined(MY_DEF_HERE) || defined(MY_DEF_HERE) || defined(MY_DEF_HERE) || defined(MY_DEF_HERE)
...
#ifdef MY_DEF_HERE
	ret = btrfs_usrquota_delsnap(trans, dest->fs_info, dest->root_key.objectid);
	if (ret) {
		btrfs_abort_transaction(trans, root, ret);
		err = ret;
		btrfs_err(root->fs_info, "failed to delete usrquota items, err=%d", err);
		goto out_end_trans;
	}
#endif
...
#ifdef MY_DEF_HERE
#ifdef MY_DEF_HERE
		ret = btrfs_clone(src, dst, loff, olen, len, dst_loff, NULL, 0, NULL);
#elif defined(MY_DEF_HERE)
		ret = btrfs_clone(src, dst, loff, olen, len, dst_loff, 0, NULL);
#else
		ret = btrfs_clone(src, dst, loff, olen, len, dst_loff, NULL);
#endif
#else /* !MY_DEF_HERE */
#ifdef MY_DEF_HERE
		ret = btrfs_clone(src, dst, loff, olen, len, dst_loff, NULL, 0);
#elif defined(MY_DEF_HERE)
		ret = btrfs_clone(src, dst, loff, olen, len, dst_loff, 0);
#else
...
static int btrfs_clone(struct inode *src, struct inode *inode,
		       ...
		       const u64 destoff, u64 *reserved_size, int full_clone,
		       struct btrfs_syno_clone_range *args)
#elif defined(MY_DEF_HERE)
		       const u64 off, const u64 olen, const u64 olen_aligned,
		       const u64 destoff, int full_clone,
		       struct btrfs_syno_clone_range *args)
#else
		       const u64 off, const u64 olen, const u64 olen_aligned,
		       const u64 destoff,
...
#endif /* MY_DEF_HERE */
{
	    ...
	    (BTRFS_I(inode)->flags & BTRFS_INODE_NODATASUM))
	{
		...
		if (0 == destoff && 0 == inode_get_bytes(inode)) {
			...
			goto out_checksum_flag_not_match;
		}
		up_write(&BTRFS_I(inode)->dio_sem);
	}
#endif /* MY_DEF_HERE */
	...
	if (root->fs_info->usrquota_enabled
		 ...
		 */
		ret = btrfs_usrquota_reserve(root, inode, inode->i_uid, reserve_size);
		if (ret)
			goto fail_usrquota;
	}
#endif /* MY_DEF_HERE */
...
#ifdef MY_DEF_HERE
#ifdef MY_DEF_HERE
	ret = btrfs_clone(src, inode, off, olen, len, destoff, &reserve_size, full_clone, args);
#elif defined(MY_DEF_HERE)
	ret = btrfs_clone(src, inode, off, olen, len, destoff, full_clone, args);
#else
	ret = btrfs_clone(src, inode, off, olen, len, destoff, args);
#endif
#else /* !MY_DEF_HERE */
#ifdef MY_DEF_HERE
	ret = btrfs_clone(src, inode, off, olen, len, destoff, &reserve_size, full_clone);
#elif defined(MY_DEF_HERE)
	ret = btrfs_clone(src, inode, off, olen, len, destoff, full_clone);
#else
...
static long btrfs_ioctl_usrquota_limit(struct file *file, void __user *arg)
{
	...
	ret = btrfs_usrquota_limit(trans, root->fs_info, rootid,
	                           limit_args->uid,  limit_args->rfer_soft, limit_args->rfer_hard);
