...
int btrfs_clear_syno_usage_tree(struct btrfs_fs_info *fs_info)
{
	...
	clean_tree_block(trans, tree_root->fs_info, syno_usage_root->node);
...
int btrfs_clear_syno_extent_usage_tree(struct btrfs_fs_info *fs_info)
{
	...
	clean_tree_block(trans, tree_root->fs_info, syno_extent_usage_root->node);
...
out:
	if (ret)
		btrfs_warn_rl(fs_info, "Failed to add subvol usage dummy item with root %llu err %d", root->objectid, ret);
	return ret;
}
...
out:
	...
	if (ret && ret != -EAGAIN) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn_rl(fs_info, "Failed to clear subvol usage item with root %llu err %d", root->objectid, ret);
	}
	return ret;
}
...
out:
	...
	if (trans)
		btrfs_end_transaction_throttle(trans, root);
	if (ret < 0 && ret != -EAGAIN)
		btrfs_warn_rl(fs_info, "Failed to clear unused subvol usage item with root %llu err %d", root->objectid, ret);
	return ret;
}
...
inconsistent:
	fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
	btrfs_warn_rl(fs_info, "Failed to fast rescan for syno usage, err:%d", ret);
	goto out;
}
...
inconsistent:
	fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
	btrfs_warn_rl(fs_info, "Failed to full rescan for syno usage, err:%d", ret);
	goto out;
}
...
static void __btrfs_syno_usage_fast_rescan(struct work_struct *work)
{
	...
	while (!list_empty(&fs_info->syno_usage_pending_fast_rescan_roots)) {
		...
		if (err) {
			if (err != -EAGAIN)
				btrfs_warn_rl(fs_info, "Failed to rescan syno suage with objectid:%llu, err:%d", root->objectid, err);
			goto next;
		}
...
next_slot:
		...
		if (IS_ERR(subvol_root)) {
			...
			if (ret != -ENOENT)
				btrfs_warn_rl(fs_info, "Failed to syno usage rescan, read fs root with objectid:%llu, err:%d", subvol_key.objectid, ret);
			goto next;
		}
next:
		btrfs_release_path(path);
...
next_slot:
		...
		if (IS_ERR(subvol_root)) {
			ret = PTR_ERR(subvol_root);
			if (ret != -ENOENT)
				btrfs_warn_rl(fs_info, "Failed to disable syno usage, read fs root with objectid:%llu, err:%d", subvol_key.objectid, ret);
			srcu_read_unlock(&fs_info->subvol_srcu, index);
			goto next;
		}
		btrfs_hold_fs_root(subvol_root);
...
int btrfs_syno_usage_enable(struct btrfs_fs_info *fs_info)
{
	...
	ret = __btrfs_create_syno_extent_usage_tree(trans, fs_info);
	if (ret) {
		btrfs_warn_rl(fs_info, "Failed to create syno extent usage tree, err:%d\n", ret);
		goto out_abort;
	}
