...
static int btrfs_clean_tree_by_root_throttle(struct btrfs_root *root)
{
	...
	while (1) {
		trans = btrfs_start_transaction_fallback_global_rsv(root, 1, 5);
		if (IS_ERR(trans)) {
			ret = PTR_ERR(trans);
			trans = NULL;
			goto out;
		...
		trans = NULL;
		if (syno_usage_need_stop(root->fs_info)) {
...
static int syno_usage_status_insert(struct btrfs_trans_handle *trans, struct btrfs_root *root,
						struct btrfs_path *path, struct btrfs_key *ins)
{
	...
	size = sizeof(struct btrfs_syno_usage_status_item);
				...
				struct btrfs_fs_info *fs_info)
{
	...
	if (!fs_info->syno_usage_enabled && fs_info->syno_usage_status.state != SYNO_USAGE_STATE_DISABLE)
		return 0;
	...
	if (ret < 0) {
		...
		btrfs_release_path(path);
		// insert new item
		ret = syno_usage_status_insert(trans, root, path, &key);
		goto out;
	}
...
out:
	...
	if (ret) {
		...
		btrfs_warn(fs_info, "Failed to update syno usage status");
	}
	return ret;
}

static int syno_usage_global_type_insert(struct btrfs_trans_handle *trans, struct btrfs_root *root,
						struct btrfs_path *path, struct btrfs_key *ins, u64 num_bytes)
{
	int ret;
	...
	size = sizeof(struct btrfs_syno_usage_global_type_item);
				...
				struct btrfs_fs_info *fs_info, int type, u64 num_bytes)
{
	...
	if (ret < 0) {
		...
		btrfs_release_path(path);
		// insert new item
		ret = syno_usage_global_type_insert(trans, root, path, &key, num_bytes);
		goto out;
	}
...
out:
	...
	if (ret) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to update syno usage global type with type:%d", type);
	}
	return ret;
}
...
static int syno_usage_root_status_insert(struct btrfs_trans_handle *trans, struct btrfs_root *root,
						struct btrfs_path *path, struct btrfs_key *ins, struct btrfs_syno_usage_root_status *syno_usage_root_status)
{
	int ret;
	...
	size = sizeof(struct btrfs_syno_usage_root_status_item);
				...
				struct btrfs_fs_info *fs_info, u64 root_objectid, struct btrfs_syno_usage_root_status *syno_usage_root_status)
{
	...
	if (ret < 0) {
		...
		btrfs_release_path(path);
		// insert new item
		ret = syno_usage_root_status_insert(trans, root, path, &key, syno_usage_root_status);
		goto out;
	}
...
out:
	...
	if (ret) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to update syno usage root status with root:%llu", root_objectid);
	}
	return ret;
}
...
out:
	...
	if (ret) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to remove syno usage root status with root:%llu", root_objectid);
	}
	return ret;
}

int btrfs_syno_usage_root_status_lookup(struct btrfs_fs_info *fs_info, u64 root_objectid, struct btrfs_syno_usage_root_status *ret_syno_usage_root_status)
{
	int ret;
	struct btrfs_root *root = fs_info->syno_usage_root;
	struct btrfs_path *path;
...
int __btrfs_create_syno_usage_tree(struct btrfs_trans_handle *trans, struct btrfs_fs_info *fs_info)
{
	int ret;
	struct btrfs_root *syno_usage_root;

	syno_usage_root = btrfs_create_tree(trans, fs_info,
					    BTRFS_SYNO_USAGE_TREE_OBJECTID);
...
int btrfs_create_syno_usage_tree(struct btrfs_fs_info *fs_info)
{
	...
	ret = __btrfs_create_syno_usage_tree(trans, fs_info);
	if (ret)
		goto abort;
...
int btrfs_clear_syno_usage_tree(struct btrfs_fs_info *fs_info)
{
	struct btrfs_trans_handle *trans = NULL;
	struct btrfs_root *tree_root = fs_info->tree_root;
	struct btrfs_root *syno_usage_root = fs_info->syno_usage_root;
	int ret;
	...
	ret = btrfs_clean_tree_by_root_throttle(syno_usage_root);
	if (ret)
		goto out;

	trans = btrfs_start_transaction_fallback_global_rsv(tree_root, 1, 5);
	if (IS_ERR(trans)) {
		ret = PTR_ERR(trans);
		trans = NULL;
		goto out;
	...
	fs_info->syno_usage_root = NULL;

	ret = btrfs_del_root(trans, tree_root, &syno_usage_root->root_key);
	if (ret)
		goto abort;

	list_del(&syno_usage_root->dirty_list);

	btrfs_tree_lock(syno_usage_root->node);
	clean_tree_block(trans, tree_root, syno_usage_root->node);
	btrfs_tree_unlock(syno_usage_root->node);
	btrfs_free_tree_block(trans, syno_usage_root, syno_usage_root->node, 0, 1);

	free_extent_buffer(syno_usage_root->node);
	free_extent_buffer(syno_usage_root->commit_root);
	kfree(syno_usage_root);
				  ...
				  int refs_to_mod)
{
	...
	if (refs_to_mod < 0 && refs < refs_to_mod) {
		...
		btrfs_warn(fs_info, "Failed to syno usage refs underflow for extent bytenr %llu refs %lld refs_to_mod %d", key.objectid, refs, refs_to_mod);
		refs = 0;
	} else {
		refs += refs_to_mod;
	}
	...
	if (refs == 0) {
		...
		if (type == inline_ref_type) {
			...
			if (fs_info->syno_usage_status.syno_usage_type_num_bytes[type] < key.offset) {
				btrfs_warn(fs_info, "Failed to syno usage underflow for type %u size %llu key [%llu %u %llu]",
 ...
 */
static noinline_for_stack
int __syno_extent_usage_lookup_inline_ref(struct btrfs_trans_handle *trans,
				 ...
				 int want, int insert)
{
	...
	if (ret == -ENOENT && insert) {
		if (item_size + sizeof(struct btrfs_syno_extent_usage_inline_ref) >=
				...
				u64 bytenr, u64 num_bytes, int refs_to_add)
{
	...
	extra_size = sizeof(struct btrfs_syno_extent_usage_item) + sizeof(struct btrfs_syno_extent_usage_inline_ref);
...
again:
	...
	ret = __syno_extent_usage_lookup_inline_ref(trans, root, path, &iref, want, 1);
	if (ret == 0) { /* update */
...
out:
	...
	if (ret) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to add syno usage extent item with bytenr:%llu", bytenr);
	}
	return ret;
}
				...
				u64 bytenr, u64 num_bytes, int refs_to_drop, bool remove)
{
	...
	ret = __syno_extent_usage_lookup_inline_ref(trans, root, path, &iref, want, 0);
	if (ret == 0) /* update */
...
out:
	...
	if (ret) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to free syno usage extent item with bytenr:%llu", bytenr);
	}
	return ret;
}

static int __btrfs_create_syno_extent_usage_tree(struct btrfs_trans_handle *trans, struct btrfs_fs_info *fs_info)
{
	int ret;
	struct btrfs_root *syno_extent_usage_root;

	syno_extent_usage_root = btrfs_create_tree(trans, fs_info,
					    BTRFS_SYNO_EXTENT_USAGE_TREE_OBJECTID);
...
int btrfs_create_syno_extent_usage_tree(struct btrfs_fs_info *fs_info)
{
	...
	ret = __btrfs_create_syno_extent_usage_tree(trans, fs_info);
	if (ret)
		goto abort;
...
int btrfs_clear_syno_extent_usage_tree(struct btrfs_fs_info *fs_info)
{
	...
	ret = btrfs_clean_tree_by_root_throttle(syno_extent_usage_root);
	if (ret)
		goto out;
	...
	fs_info->syno_extent_usage_root = NULL;

	ret = btrfs_del_root(trans, tree_root, &syno_extent_usage_root->root_key);
	if (ret)
		goto abort;

	list_del(&syno_extent_usage_root->dirty_list);

	btrfs_tree_lock(syno_extent_usage_root->node);
	clean_tree_block(trans, tree_root, syno_extent_usage_root->node);
	btrfs_tree_unlock(syno_extent_usage_root->node);
	btrfs_free_tree_block(trans, syno_extent_usage_root, syno_extent_usage_root->node,
			      0, 1);

	free_extent_buffer(syno_extent_usage_root->node);
	free_extent_buffer(syno_extent_usage_root->commit_root);
	kfree(syno_extent_usage_root);
...
static int syno_subvol_usage_update_ref_count(struct btrfs_trans_handle *trans,
				  struct btrfs_root *root, struct btrfs_path *path, struct btrfs_key *ins, int refs_to_mod, int *last_ref, u8 *type)
{
	struct btrfs_fs_info *fs_info = root->fs_info;
	struct extent_buffer *leaf;
	struct btrfs_syno_subvol_usage_item *ei;
	...
	if (refs_to_mod < 0 && refs < refs_to_mod) {
		btrfs_warn(fs_info, "Failed to syno usage refs underflow for subvol usage bytenr %llu refs %lld refs_to_mod %d", ins->offset, refs, refs_to_mod);
		refs = 0;
	} else {
		refs += refs_to_mod;
	}
...
static int __btrfs_syno_subvol_usage_add(struct btrfs_trans_handle *trans, struct btrfs_fs_info *fs_info,
...
again:
	...
	if (ret < 0) {
		...
		if (ret)
			goto out;
		ret = btrfs_syno_extent_usage_add(trans, root->fs_info, type, bytenr, num_bytes, 1);
		goto out;
	}
...
out:
	...
	if (ret) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to add syno usage subvol item with bytenr:%llu", bytenr);
	}
	return ret;
}
				...
				u64 root_objectid, u64 bytenr, u64 num_bytes, int refs_to_add)
{
	...
	if (IS_ERR(root)) {
		if (PTR_ERR(root) != -ENOENT) {
			ret = PTR_ERR(root);
			fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
			btrfs_warn(fs_info, "Failed to read root with objectid:%llu, err:%d", root_objectid, ret);
		}
	...
	ret = __btrfs_syno_subvol_usage_add(trans, fs_info, root, bytenr, num_bytes, refs_to_add);
	btrfs_release_fs_root(root);
out:
	return ret;
}

static int __btrfs_syno_subvol_usage_free(struct btrfs_trans_handle *trans, struct btrfs_fs_info *fs_info,
				struct btrfs_root *root, u64 bytenr, u64 num_bytes, int refs_to_drop)
{
	...
	if (last_ref) {
		btrfs_release_path(path);
		ret = btrfs_syno_extent_usage_free(trans, fs_info, type, bytenr, num_bytes, 1, 0);
	}
out:
	btrfs_free_path(path);
	trans->syno_usage = false;
	if (ret) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to free syno usage subvol item with bytenr:%llu", bytenr);
	}
	return ret;
}
				...
				u64 root_objectid, u64 bytenr, u64 num_bytes, int refs_to_drop)
{
	...
	ret = __btrfs_syno_subvol_usage_free(trans, fs_info, root, bytenr, num_bytes, refs_to_drop);
	btrfs_release_fs_root(root);
out:
	return ret;
}

static int rescan_wait(void *word)
{
	io_schedule();
	return 0;
}
						...
						struct btrfs_path *path, struct btrfs_key *ins)
{
	...
	ret = btrfs_insert_empty_item(trans, root, path, ins, root->nodesize / 2);
	if (ret)
		goto out;
...
out:
	if (ret)
		btrfs_warn(fs_info, "Failed to add subvol usage dummy item with root %llu err %d", root->objectid, ret);
	return ret;
}
...
int btrfs_syno_clear_subvol_usage_item_doing(struct btrfs_root *root)
{
	...
	while (1) {
		...
		ret = btrfs_syno_usage_root_status_update(trans, fs_info, root->objectid, &root->syno_usage_root_status);
		if (ret)
			goto out;

		btrfs_end_transaction_throttle(trans, fs_info->tree_root);
		trans = NULL;
...
out:
	...
	if (ret && ret != -EAGAIN) {
		fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
		btrfs_warn(fs_info, "Failed to clear subvol usage item with root %llu err %d", root->objectid, ret);
	}
	return ret;
}
...
next_slot:
		...
		if (root->syno_usage_root_status.flags & BTRFS_SYNO_USAGE_ROOT_FLAG_RESCAN_PROGRESS_ACCOUNTING) {
			spin_lock(&fs_info->syno_usage_lock);
			if (root->syno_usage_root_status.cur_full_rescan_size + root->nodesize > root->syno_usage_root_status.total_full_rescan_size) {
				root->syno_usage_root_status.total_full_rescan_size += root->nodesize;
				fs_info->syno_usage_status.total_full_rescan_size += root->nodesize;
			}
			root->syno_usage_root_status.cur_full_rescan_size += root->nodesize;
			fs_info->syno_usage_status.cur_full_rescan_size += root->nodesize;
			spin_unlock(&fs_info->syno_usage_lock);
		}
...
out:
	...
	if (trans)
		btrfs_end_transaction_throttle(trans, root);
	if (ret < 0 && ret != -EAGAIN)
		btrfs_warn(fs_info, "Failed to clear unused subvol usage item with root %llu err %d", root->objectid, ret);
	return ret;
}
...
static int syno_usage_fast_rescan_root(struct btrfs_root *root)
{
	...
	while(1) {
		...
		spin_lock(&root->syno_usage_lock);
		root->syno_usage_root_status.fast_rescan_progress = found_key;
		root->syno_usage_root_status.fast_rescan_progress.offset = found_key.offset + 1;
		key.offset = root->syno_usage_root_status.fast_rescan_progress.offset;
		spin_unlock(&root->syno_usage_lock);

		ret = btrfs_syno_extent_usage_add(trans, fs_info, root->syno_usage_root_status.new_type, bytenr, num_bytes, 1);
		if (ret)
			goto inconsistent;
		ret = btrfs_syno_extent_usage_free(trans, fs_info, root->syno_usage_root_status.type, bytenr, num_bytes, 1, 0);
		if (ret)
			goto inconsistent;
...
inconsistent:
	fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
	btrfs_warn(fs_info, "Failed to fast rescan for syno usage, err:%d", ret);
	goto out;
}
...
next_slot:
		...
		root->syno_usage_root_status.full_rescan_progress = found_key;
		root->syno_usage_root_status.full_rescan_progress.offset = found_key.offset + 1;
		...
		ret = __btrfs_syno_subvol_usage_add(trans, fs_info, root, disk_bytenr, disk_num_bytes, 1);
		if (ret)
			goto inconsistent;
...
inconsistent:
	fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
	btrfs_warn(fs_info, "Failed to full rescan for syno usage, err:%d", ret);
	goto out;
}
...
static void __btrfs_syno_usage_fast_rescan(struct work_struct *work)
{
	...
	while (!list_empty(&fs_info->syno_usage_pending_fast_rescan_roots)) {
		...
		if (err) {
			if (err != -EAGAIN)
				btrfs_warn(fs_info, "Failed to rescan syno suage with objectid:%llu, err:%d", root->objectid, err);
			goto next;
		}
...
next_slot:
		if (path->slots[0] >= btrfs_header_nritems(path->nodes[0])) {
			if (processed_count == 0) {
				...
				if (ret < 0)
					...
					break;
				spin_lock(&fs_info->syno_usage_lock);
				if (fs_info->syno_usage_status.extent_tree_cur_rescan_size + extent_root->nodesize > fs_info->syno_usage_status.extent_tree_total_rescan_size) {
				...
				if (fs_info->syno_usage_status.extent_tree_cur_rescan_size + extent_root->nodesize > fs_info->syno_usage_status.extent_tree_total_rescan_size) {
					fs_info->syno_usage_status.extent_tree_total_rescan_size += extent_root->nodesize;
					fs_info->syno_usage_status.total_full_rescan_size += extent_root->nodesize;
				}
				fs_info->syno_usage_status.extent_tree_cur_rescan_size += extent_root->nodesize;
				fs_info->syno_usage_status.cur_full_rescan_size += extent_root->nodesize;
				spin_unlock(&fs_info->syno_usage_lock);
			} else {
				btrfs_release_path(path);
				btrfs_end_transaction_throttle(trans, syno_extent_usage_root);
				trans = NULL;
				goto next;
			}
		}
		...
		ret = btrfs_syno_extent_usage_add(trans, fs_info, SYNO_USAGE_TYPE_RO_SNAPSHOT, bytenr, num_bytes, 0);
		if (ret)
			goto inconsistent;
...
inconsistent:
	fs_info->syno_usage_status.flags |= BTRFS_SYNO_USAGE_FLAG_INCONSISTENT;
	btrfs_warn(fs_info, "Failed to extent rescan for syno usage, err:%d", ret);
	goto out;
}
...
next:
	...
	if (!ret && (fs_info->syno_usage_status.state == SYNO_USAGE_STATE_RESCAN || fs_info->syno_usage_status.state == SYNO_USAGE_STATE_RESCAN_PAUSE) &&
		(btrfs_comp_cpu_keys(&rescan_finish_key, &fs_info->syno_usage_status.extent_rescan_progress) == 0)) {
		fs_info->syno_usage_status.cur_full_rescan_size += fs_info->syno_usage_status.extent_tree_total_rescan_size - fs_info->syno_usage_status.extent_tree_cur_rescan_size;
		fs_info->syno_usage_status.extent_tree_cur_rescan_size = 0;
		fs_info->syno_usage_status.extent_tree_total_rescan_size = 0;
		fs_info->syno_usage_status.cur_full_rescan_size = fs_info->syno_usage_status.total_full_rescan_size;
		fs_info->syno_usage_status.state = SYNO_USAGE_STATE_ENABLE;
	}

	if (ret && ret != -EAGAIN && (fs_info->syno_usage_status.state == SYNO_USAGE_STATE_RESCAN)) {
		fs_info->syno_usage_status.state = SYNO_USAGE_STATE_RESCAN_ERROR;
		fs_info->syno_usage_status.error_code = ret;
	}
...
static void __syno_usage_rescan_resume(struct btrfs_fs_info *fs_info)
{
	...
	while (1) {
		...
		for (i = 0; i < ret; i++) {
			/* Avoid to hold roots in dead_roots */
			if (btrfs_root_refs(&gang[i]->root_item) == 0) {
				gang[i] = NULL;
				continue;
			}
			/* hold all the search result for later use */
			btrfs_hold_fs_root(gang[i]);
		}
		spin_unlock(&fs_info->fs_roots_radix_lock);
		srcu_read_unlock(&fs_info->subvol_srcu, index);
		...
		for (i = 0; i < ret; i++) {
			...
			spin_lock(&fs_info->syno_usage_full_rescan_lock);
			spin_lock(&fs_info->syno_usage_fast_rescan_lock);
			if (subvol_root->syno_usage_enabled &&
				...
				btrfs_hold_fs_root(subvol_root);
...
next_slot:
		...
		if (found_key.type != SYNO_BTRFS_USAGE_ROOT_STATUS_KEY) {
			path->slots[0]++;
			goto next_slot;
		}
		...
		if (btrfs_syno_usage_root_status_new_type(leaf, ei) == SYNO_USAGE_TYPE_NONE || btrfs_syno_usage_root_status_new_type(leaf, ei) == SYNO_USAGE_TYPE_RO_SNAPSHOT) {
			path->slots[0]++;
			goto next_slot;
		}
		btrfs_release_path(path);

		subvol_key.objectid = found_key.objectid;
		subvol_key.type = BTRFS_ROOT_ITEM_KEY;
		subvol_key.offset = (u64)-1;

		subvol_root = btrfs_read_fs_root_no_name(fs_info, &subvol_key);
		if (IS_ERR(subvol_root)) {
			ret = PTR_ERR(subvol_root);
			if (ret != -ENOENT)
				btrfs_warn(fs_info, "Failed to syno usage rescan, read fs root with objectid:%llu, err:%d", subvol_key.objectid, ret);
			goto next;
		}
next:
		btrfs_release_path(path);
...
static void __btrfs_syno_usage_rescan(struct work_struct *work)
{
	...
	if (!ret && !fs_info->syno_usage_rescan_preload) {
		ret = __syno_usage_rescan_preload(fs_info);
		if (!ret)
			fs_info->syno_usage_rescan_preload = 1;
	}
	if (!ret && !fs_info->syno_usage_rescan_check_all) {
		__syno_usage_rescan_resume(fs_info);
		fs_info->syno_usage_rescan_check_all = 1;
	}

	if (!list_empty(&fs_info->syno_usage_pending_fast_rescan_roots) && !work_busy(&fs_info->syno_usage_fast_rescan_work)) {
		queue_work(system_unbound_wq, &fs_info->syno_usage_fast_rescan_work);
	}
	if (!list_empty(&fs_info->syno_usage_pending_full_rescan_roots) && !work_busy(&fs_info->syno_usage_full_rescan_work)) {
...
{
	if (!subvol_root->fs_info->syno_usage_enabled || !is_fstree(subvol_root->objectid) || subvol_root->objectid > BTRFS_LAST_FREE_OBJECTID)
		return;

	spin_lock(&subvol_root->syno_usage_lock);
	if (!subvol_root->syno_usage_enabled) {
		subvol_root->syno_usage_root_status.type = SYNO_USAGE_TYPE_NONE;
		subvol_root->syno_usage_root_status.new_type = SYNO_USAGE_TYPE_NONE;
		subvol_root->syno_usage_root_status.state = SYNO_USAGE_ROOT_STATE_RESCAN;
		subvol_root->syno_usage_root_status.flags = 0;
		if (btrfs_root_readonly(subvol_root))
			subvol_root->syno_usage_root_status.flags |= BTRFS_SYNO_USAGE_ROOT_FLAG_READONLY;
		subvol_root->syno_usage_root_status.flags |= BTRFS_SYNO_USAGE_ROOT_FLAG_FAST_RESCAN | BTRFS_SYNO_USAGE_ROOT_FLAG_FULL_RESCAN;
		subvol_root->syno_usage_root_status.num_bytes = 0;
		subvol_root->syno_usage_root_status.drop_progress.objectid = 0;
		subvol_root->syno_usage_root_status.drop_progress.type = 0;
		subvol_root->syno_usage_root_status.drop_progress.offset = 0;
		subvol_root->syno_usage_root_status.fast_rescan_progress.objectid = 0;
		subvol_root->syno_usage_root_status.fast_rescan_progress.type = 0;
		subvol_root->syno_usage_root_status.fast_rescan_progress.offset = 0;
		subvol_root->syno_usage_root_status.full_rescan_progress.objectid = 0;
		subvol_root->syno_usage_root_status.full_rescan_progress.type = 0;
		subvol_root->syno_usage_root_status.full_rescan_progress.offset = 0;
		subvol_root->syno_usage_root_status.cur_full_rescan_size = 0;
		subvol_root->syno_usage_root_status.total_full_rescan_size = 0;
		subvol_root->syno_usage_root_status.total_syno_subvol_usage_items = 0;
		subvol_root->syno_usage_enabled = 1;
	}
	spin_unlock(&subvol_root->syno_usage_lock);
}

int btrfs_syno_usage_disable_clear_subvol_usage_item(struct btrfs_root *root)
{
	struct btrfs_fs_info *fs_info = root->fs_info;
	struct btrfs_trans_handle *trans = NULL;
	struct btrfs_path *path;
...
next_slot:
		...
		if (root->syno_usage_root_status.flags & BTRFS_SYNO_USAGE_ROOT_FLAG_READONLY) {
			...
			root->fs_info->syno_usage_status.total_full_rescan_size += root->syno_usage_root_status.total_syno_subvol_usage_items;
...
static int __syno_usage_disable_clear_fs_roots(struct btrfs_fs_info *fs_info)
{
	int ret;
	struct btrfs_root *root = fs_info->syno_usage_root;
	struct btrfs_path *path = NULL;
...
next_slot:
		...
		if (IS_ERR(subvol_root)) {
			ret = PTR_ERR(subvol_root);
			if (ret != -ENOENT)
				btrfs_warn(fs_info, "Failed to disable syno usage, read fs root with objectid:%llu, err:%d", subvol_key.objectid, ret);
			srcu_read_unlock(&fs_info->subvol_srcu, index);
			goto next;
		}
		btrfs_hold_fs_root(subvol_root);
		...
		ret = btrfs_syno_usage_disable_clear_subvol_usage_item(subvol_root);
		btrfs_release_fs_root(subvol_root);
		if (ret)
			goto out;
next:
		if (syno_usage_need_stop(fs_info)) {
...
int btrfs_syno_usage_disable(struct btrfs_fs_info *fs_info)
{
	...
	ret = __syno_usage_disable_clear_fs_roots(fs_info);
	if (ret)
		goto out;

	ret = btrfs_clear_syno_extent_usage_tree(fs_info);
	if (ret)
		goto out;
...
int btrfs_syno_usage_enable(struct btrfs_fs_info *fs_info)
{
	...
	ret = __btrfs_create_syno_usage_tree(trans, fs_info);
	if (ret) {
		btrfs_warn(fs_info, "Failed to create syno usage tree, err:%d", ret);
		goto out_abort;
	}

	ret = __btrfs_create_syno_extent_usage_tree(trans, fs_info);
	if (ret) {
		btrfs_warn(fs_info, "Failed to create syno extent usage tree, err:%d\n", ret);
		goto out_abort;
	}
