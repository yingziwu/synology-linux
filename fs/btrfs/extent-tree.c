			 ...
			 u64 root_objectid, u64 owner, u64 offset, int for_cow)
{
	...
	if (owner < BTRFS_FIRST_FREE_OBJECTID) {
		...
		ret = btrfs_add_delayed_data_ref(fs_info, trans, bytenr,
					...
					BTRFS_ADD_DELAYED_REF, NULL, for_cow, 0, 0);
#else
					BTRFS_ADD_DELAYED_REF, NULL, for_cow);
#endif
	}
	...
	while (!list_empty(&trans->qgroup_ref_list)) {
		...
		if (!ret1)
			ret1 = btrfs_usrquota_account_ref(
			           trans, fs_info, qgroup_update->node,
			           qgroup_update->extent_op,
			           qgroup_update->is_uid_valid, qgroup_update->uid);
#endif
		kfree(qgroup_update);
	}
...
{
	return __btrfs_mod_ref(trans, root, buf, full_backref, 0, for_cow, rootid);
}
#endif
...
int btrfs_delalloc_reserve_metadata(struct inode *inode, u64 num_bytes)
{
	...
	if (root->fs_info->usrquota_enabled) {
		ret = btrfs_usrquota_reserve(root, inode->i_uid, num_bytes);
		if (ret) {
			if (root->fs_info->quota_enabled) {
...
#ifdef CONFIG_BTRFS_FS_SYNO_USRQUOTA
		if (root->fs_info->usrquota_enabled) {
			btrfs_usrquota_free(root, inode->i_uid, num_bytes);
		}
#endif
...
int btrfs_free_extent_uid(struct btrfs_trans_handle *trans, struct btrfs_root *root,
                          ...
                          u64 owner, u64 offset, int for_cow, uid_t uid)
{
	int ret;
	struct btrfs_fs_info *fs_info = root->fs_info;
				     ...
				     u64 offset, struct btrfs_key *ins)
{
	...
	ret = btrfs_add_delayed_data_ref(root->fs_info, trans, ins->objectid,
					 ...
					 root_objectid, owner, offset,
#ifdef CONFIG_BTRFS_FS_SYNO_USRQUOTA
					 BTRFS_ADD_DELAYED_EXTENT, NULL, 0, 0, 0);
#else
			 ...
			 int for_reloc)
{
	...
	if (fs_info->quota_enabled) {
		...
		if (0 != ret) {
			if (-ENOENT == ret) {
				printk(KERN_ERR "cannot find qgroup item, qgroupid=%llu !\n", qgroupid);
			} else if (-EBUSY == ret) {
				printk(KERN_ERR "cannot remove qgroup item, it relates to other qgroup, qgroupid=%llu !\n", qgroupid);
			} else {
				btrfs_abort_transaction(trans, tree_root, ret);
				err = ret;
				goto out_end_trans;
