...
#ifdef MY_ABC_HERE
				int no_quota, struct btrfs_delayed_ref_node *node);
#else
				int no_quota);
#endif
...
#ifdef MY_ABC_HERE
				      struct btrfs_key *ins, int ref_mod, struct btrfs_delayed_ref_node *node);
#else
...
#ifdef MY_ABC_HERE
	if (btrfs_test_opt(fs_info->extent_root, SYNO_ALLOCATOR))
...
u64 get_bg_offset_with_free_space_bytes(struct btrfs_fs_info *fs_info)
{
	...
	for (; node && visit_cnt < 1000; node = rb_next(node), visit_cnt++) {
		block_group = rb_entry(node, struct btrfs_block_group_cache, syno_allocator.bytes_index);
		...
		if (block_group->key.offset < block_group->syno_allocator.last_bytes) {
		...
		usage = block_group->key.offset - block_group->syno_allocator.last_bytes;
		...
		if (usage < min_usage) {
			btrfs_info(fs_info, "Balance-candidate has changed from %llu(usage=%llu) to %llu(usage=%llu),"
						" visit_cnt=%d",
						result, min_usage, block_group->key.objectid, usage, visit_cnt);
			min_usage = usage;
			result = block_group->key.objectid;
		}
loop:
		btrfs_put_block_group(block_group);
	}
...
#ifdef MY_ABC_HERE
				 u64 parent, bool *has_clone_range, u64 root_objectid,
#else
				 u64 parent, u64 root_objectid,
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
	if (has_clone_range && *has_clone_range == 0)
...
static int btrfs_issue_discard(struct block_device *bdev, u64 start, u64 len,
			       u64 *discarded_bytes, enum trim_act act)
#else /* MY_ABC_HERE */
static int btrfs_issue_discard(struct block_device *bdev, u64 start, u64 len,
			       u64 *discarded_bytes)
#endif /* MY_ABC_HERE */
...
#endif /* MY_ABC_HERE */
{
	...
	if (!ret) {
		...
		for (i = 0; i < bbio->num_stripes; i++, stripe++) {
			u64 bytes;
#ifdef MY_ABC_HERE
			if (!stripe->dev->can_discard && (act != TRIM_SEND_HINT))
				continue;
...
#endif
{
...
#ifdef MY_ABC_HERE
	if (!is_fstree(root_objectid) || !BTRFS_ANY_QUOTA_ENABLED(fs_info))
#else
...
#ifdef MY_ABC_HERE
	search_key.objectid = root_objectid;
	...
	if (!ret && !no_quota) {
#ifdef MY_ABC_HERE
		ASSERT(BTRFS_ANY_QUOTA_ENABLED(fs_info));
#else
				...
				int insert_reserved)
{
	...
	if (ref->syno_usage == 1) {
		if (node->action == BTRFS_ADD_DELAYED_REF)
			ret = btrfs_syno_subvol_usage_add(trans, root->fs_info, ref_root, node->bytenr, node->num_bytes, node->ref_mod);
		else
			ret = btrfs_syno_subvol_usage_free(trans, root->fs_info, ref_root, node->bytenr, node->num_bytes, node->ref_mod);
	} else if (ref->syno_usage == 2 && node->action == BTRFS_ADD_DELAYED_REF) {
		ret = btrfs_syno_extent_usage_add(trans, root->fs_info, SYNO_USAGE_TYPE_RO_SNAPSHOT, node->bytenr, node->num_bytes, 0);
	}
out:
#endif /* MY_ABC_HERE */
	return ret;
...
#ifdef MY_ABC_HERE
	int chown = nr == BTRFS_USRQUOTA_DELAYED_REF_SCAN;
#endif
#ifdef MY_ABC_HERE
	...
	while (1) {
		if (!locked_ref) {
			...
			if (!chown)
				...
				locked_ref = btrfs_select_data_ref_head(trans);
				if (IS_ERR(locked_ref) && PTR_ERR(locked_ref) == -EAGAIN) {
					spin_unlock(&delayed_refs->lock);
					locked_ref = NULL;
					cond_resched();
					continue;
...
#endif /* MY_ABC_HERE */
#ifdef MY_ABC_HERE
	u64 num_syno_usage_heads = trans->transaction->delayed_refs.num_syno_usage_heads_ready;
	u64 num_syno_usage_entries = atomic_read(&trans->transaction->delayed_refs.num_syno_usage_entries);
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
	num_heads += num_syno_usage_heads + num_syno_usage_entries;
#endif /* MY_ABC_HERE */
				       ...
				       struct btrfs_root *root)
{
	...
	if (num_entries * avg_runtime >= NSEC_PER_SEC)
		return 1;
	if (btrfs_check_space_for_delayed_refs(trans, root, true))
		return 1;
...
static void delayed_ref_async_start(struct btrfs_work *work)
{
	...
	struct btrfs_delayed_ref_root *delayed_refs;
	unsigned long count = 0, processed_count, batch = 32;
...
again:
	...
	trans->sync = true;
	processed_count = 0;
	btrfs_run_delayed_refs_and_get_processed(trans, async->root, batch, &processed_count);
	...
	/* Deduct count times that have been executed */
	if (count >= batch)
		count -= batch;
	else
		count = 0;

	/* Check if there is any remaining count */
	if (count) {
		cond_resched();
		goto again;
	}
...
#endif /* MY_ABC_HERE */
{
	...
	int run_most = 0;
#ifdef MY_ABC_HERE
	int chown = count == BTRFS_USRQUOTA_DELAYED_REF_SCAN;
...
#ifdef MY_ABC_HERE
	if (btrfs_super_cache_generation(root->fs_info->super_copy) == (u64)-1 &&
...
#ifdef MY_ABC_HERE
		/*
		 * When tree log replay, avoid free space inode race
		 * logged EXTENT_DATA use same extent item posision,
		 * lead to tree log replay failed.
		 */
...
#ifdef MY_ABC_HERE
	spin_lock_init(&found->syno_allocator.lock);
	init_rwsem(&found->syno_allocator.allocation_sem);
	found->syno_allocator.free_space_bytes = RB_ROOT_CACHED;
	found->syno_allocator.free_space_max_length = RB_ROOT_CACHED;
	found->syno_allocator.free_space_max_length_with_extent = RB_ROOT_CACHED;
	found->syno_allocator.preload = RB_ROOT_CACHED;
	mutex_init(&found->syno_allocator.syno_allocator_mutex);
	found->syno_allocator.force_cluster_disable = true;
	found->syno_allocator.cache_bg = NULL;
	found->syno_allocator.cache_offset = 0;
#ifdef MY_ABC_HERE
	found->syno_allocator.log_bg_offset = 0;
#endif /* MY_ABC_HERE */
	atomic64_set(&found->syno_allocator.fallback_relink_count, 0);
	atomic64_set(&found->syno_allocator.fallback_full_scan_count, 0);
#endif /* MY_ABC_HERE */
			  ...
			  u64 *rsv_start, u64 *rsv_size)
{
	...
	int index;
	int ret = 0;
	flags = btrfs_get_alloc_profile(root, 0);
	...
	list_for_each_entry(block_group, &space_info->block_groups[index], list) {
		...
		if (block_group->cached == BTRFS_CACHE_FINISHED) {
			...
			if (ctl->free_space == block_group->key.offset) {
				*rsv_start = block_group->key.objectid;
				*rsv_size = block_group->key.offset;
			...
			spin_unlock(&ctl->tree_lock);
		} else if (btrfs_block_group_used(&block_group->item) == 0) {
			*rsv_start = block_group->key.objectid;
			*rsv_size = block_group->key.offset;
			spin_unlock(&block_group->lock);
			up_read(&space_info->groups_sem);
	...
	check_system_chunk(trans, fs_info->extent_root, flags, true);
	ret = btrfs_alloc_chunk_with_info(trans, fs_info->extent_root, flags,
				  rsv_start, rsv_size);
...
out:
	if (!ret) {
		fs_info->log_tree_rsv_start = *rsv_start;
		fs_info->log_tree_rsv_size = *rsv_size;
	}
	return ret;
}
#endif /* MY_ABC_HERE */
...
#endif /* MY_ABC_HERE */
#ifdef MY_ABC_HERE
	u64 disk_total_bytes = btrfs_super_total_bytes(fs_info->super_copy);
	u64 reserved_for_metadata;
#endif /* MY_ABC_HERE */
...
again:
	...
	if (flags & BTRFS_BLOCK_GROUP_DATA && fs_info->metadata_ratio) {
		reserved_for_metadata = disk_total_bytes;
		...
		if (space_info->total_bytes > (disk_total_bytes - reserved_for_metadata)) {
			ret = -ENOSPC;
			goto syno_data_limit;
		}
	}
#endif /* MY_ABC_HERE */
...
static void btrfs_async_reclaim_metadata_space(struct work_struct *work)
{
	...
	to_reclaim = btrfs_calc_reclaim_metadata_size(fs_info->fs_root,
						      space_info);
	if (!to_reclaim && atomic_read(&fs_info->syno_metadata_reserve_pending))
	...
	do {
		...
		flush_state++;
#ifdef MY_ABC_HERE
		if (atomic_read(&fs_info->syno_metadata_reserve_pending))
			continue;
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
static void btrfs_async_data_flush(struct work_struct *work)
{
	struct btrfs_fs_info *fs_info;
	struct btrfs_space_info *space_info;
	...
	fs_info = container_of(work, struct btrfs_fs_info, async_data_flush_work);
	space_info = __find_space_info(fs_info, BTRFS_BLOCK_GROUP_METADATA);
...
again:
	...
	if (fs_info->closing || test_bit(BTRFS_FS_STATE_REMOUNTING, &fs_info->fs_state))
		goto out;
	...
	if (atomic_read(&fs_info->syno_metadata_reserve_pending))
		goto skip_check;
	spin_lock(&space_info->lock);
	to_reclaim = btrfs_calc_reclaim_metadata_size(fs_info->fs_root, space_info);
	if (!to_reclaim) {
		spin_unlock(&space_info->lock);
		goto out;
	}
...
select_inode:
	...
	if (
#ifdef MY_ABC_HERE
		atomic_read(&fs_info->syno_metadata_reserve_pending) ||
#endif /* MY_ABC_HERE */
		btrfs_need_do_async_reclaim(space_info, fs_info, FLUSH_DELALLOC)) {
#ifdef MY_ABC_HERE
...
{
	INIT_WORK(work, btrfs_async_data_flush);
}
#endif /* MY_ABC_HERE */
...
static bool btrfs_check_need_async_meta_flush(struct btrfs_fs_info *fs_info, bool worker)
{
	...
	if (!worker && work_busy(&fs_info->async_metadata_flush_work))
		goto out;

	if (!writeback_in_progress(&fs_info->bdi))
		goto out;

	if (percpu_counter_compare(&fs_info->dirty_metadata_bytes, BTRFS_DIRTY_METADATA_THRESH) <= 0)
		goto out;
...
static void btrfs_async_metadata_flush(struct work_struct *work)
{
	struct btrfs_fs_info *fs_info;
	struct blk_plug plug;
	...
	fs_info = container_of(work, struct btrfs_fs_info, async_metadata_flush_work);
...
{
	INIT_WORK(work, btrfs_async_metadata_flush);
}
...
void btrfs_syno_btree_balance_dirty(struct btrfs_fs_info *fs_info, bool throttle)
{
	...
	if (async_meta_flush)
		queue_work(system_unbound_wq, &fs_info->async_metadata_flush_work);
				  ...
				  enum btrfs_reserve_flush_enum flush)
{
	...
	unsigned long resched_interval;
	bool pending = false;

	if (flush == BTRFS_RESERVE_FLUSH_ALL) {
		bl_resched_timeout = true;
		resched_interval = msecs_to_jiffies(5);
	}
#endif /* MY_ABC_HERE */
...
again:
	...
	if (ret && flush != BTRFS_RESERVE_NO_FLUSH) {
		...
		if (!root->fs_info->log_root_recovering &&
			...
			if (!work_busy(&root->fs_info->async_data_flush_work) &&
				!list_empty(&root->fs_info->syno_dirty_lru_inodes))
				queue_work(system_unbound_wq, &root->fs_info->async_data_flush_work);
		}
#else
		if (!root->fs_info->log_root_recovering &&
#ifdef MY_ABC_HERE
...
#ifdef MY_ABC_HERE
	need_async_reclaim = 0;
	if (ret && bl_resched_timeout
#ifdef MY_ABC_HERE
		&& !root->fs_info->avoid_fs_root_null_pointer_dereference
#endif /* MY_ABC_HERE */
		)
...
#ifdef MY_ABC_HERE
	if (bl_resched_timeout && need_async_reclaim &&
		...
		!root->fs_info->avoid_fs_root_null_pointer_dereference &&
			...
			(
#ifdef MY_ABC_HERE
			atomic64_read(&root->fs_info->syno_ordered_extent_nr) ||
#endif /* MY_ABC_HERE */
			percpu_counter_sum_positive(&root->fs_info->delalloc_bytes)
			)
		) {
		if (!pending) {
			atomic_inc(&root->fs_info->syno_metadata_reserve_pending);
			pending = true;
		}
		queue_work(system_unbound_wq, &root->fs_info->async_reclaim_work);
#ifdef MY_ABC_HERE
		if (!list_empty(&root->fs_info->syno_dirty_lru_inodes))
			queue_work(system_unbound_wq, &root->fs_info->async_data_flush_work);
#endif /* MY_ABC_HERE */
		if (!timeout)
			timeout = jiffies + HZ * 120;
...
#ifdef MY_ABC_HERE
		if (pending)
			atomic_dec(&root->fs_info->syno_metadata_reserve_pending);
#endif /* MY_ABC_HERE */
	}
	return ret;
}
...
#ifdef MY_ABC_HERE
	if (root != root->fs_info->chunk_root && trans->syno_usage)
		return &root->fs_info->global_block_rsv;
#endif /* MY_ABC_HERE */
#ifdef MY_ABC_HERE
	if (root != root->fs_info->chunk_root && trans->cleaner)
		return &root->fs_info->cleaner_block_rsv;
#endif /* MY_ABC_HERE */
...
#endif /* MY_ABC_HERE */
	num_bytes = max_t(u64, num_bytes, 0x01000000);
...
static void init_cleaner_block_rsv(struct btrfs_fs_info *fs_info)
{
	...
	used = sinfo->bytes_used + sinfo->bytes_reserved +
		sinfo->bytes_pinned + sinfo->bytes_readonly +
		sinfo->bytes_may_use;
	...
	block_rsv->reserved += num_bytes;
	sinfo->bytes_may_use += num_bytes;
	trace_btrfs_space_reservation(fs_info, "space_info", sinfo->flags, num_bytes, 1);
	block_rsv->full = 1;
...
static void init_global_block_rsv(struct btrfs_fs_info *fs_info)
{
	...
	if (fs_info->syno_usage_root)
		fs_info->syno_usage_root->block_rsv = &fs_info->global_block_rsv;
	if (fs_info->syno_extent_usage_root)
		fs_info->syno_extent_usage_root->block_rsv = &fs_info->global_block_rsv;
#endif /* MY_ABC_HERE */
...
{
#ifdef MY_ABC_HERE
	block_rsv_release_bytes(fs_info, &fs_info->cleaner_block_rsv, NULL, (u64)-1);
#endif /* MY_ABC_HERE */
				      ...
				      u64 qgroup_reserved)
{
	...
	if (qgroup_reserved)
#ifdef MY_ABC_HERE
		btrfs_qgroup_free_add_delay(root, qgroup_reserved);
#else
		btrfs_qgroup_free(root, qgroup_reserved);
#endif
}
...
#ifdef MY_ABC_HERE
	if (atomic_read(&BTRFS_I(inode)->syno_uq_refs) == 0 &&
		test_and_clear_bit(BTRFS_INODE_USRQUOTA_META_RESERVED,
	  					&BTRFS_I(inode)->runtime_flags)) {
			drop_inode_space++;
		}
#endif /* MY_ABC_HERE */
...
int btrfs_delalloc_reserve_metadata(struct inode *inode, u64 num_bytes)
{
	...
	unsigned dropped;
#ifdef MY_ABC_HERE
	int syno_usrquota_extra_reserve = 0;
#endif /* MY_ABC_HERE */
	...
	if (btrfs_usrquota_fast_chown_enable(inode) &&
		!test_bit(BTRFS_INODE_USRQUOTA_META_RESERVED, &BTRFS_I(inode)->runtime_flags)) {
		nr_extents++;
		syno_usrquota_extra_reserve = 1;
	}
#endif /* MY_ABC_HERE */
	...
	if (root->fs_info->usrquota_enabled) {
		ret = btrfs_usrquota_reserve(root, inode, inode->i_uid, num_bytes);
		if (ret) {
			if (root->fs_info->quota_enabled) {
#ifdef MY_ABC_HERE
...
#ifdef MY_ABC_HERE
		if (root->fs_info->usrquota_enabled) {
			btrfs_usrquota_free(root, inode, inode->i_uid, num_bytes);
		}
#endif /* MY_ABC_HERE */
		goto out_fail;
	}
...
#ifdef MY_ABC_HERE
	if (syno_usrquota_extra_reserve) {
		set_bit(BTRFS_INODE_USRQUOTA_META_RESERVED,
			&BTRFS_I(inode)->runtime_flags);
		nr_extents--;
	}
#endif /* MY_ABC_HERE */
 ...
 */
#ifdef MY_ABC_HERE
static void __btrfs_delalloc_release_metadata(struct inode *inode, u64 num_bytes, int add_delay)
#else
...
#endif
{
...
#ifdef MY_ABC_HERE
#ifdef MY_ABC_HERE
		__btrfs_qgroup_free(root, num_bytes, add_delay);
#else
		__btrfs_qgroup_free(root, num_bytes +
					dropped * root->leafsize, add_delay);
#endif /* MY_ABC_HERE */
#else
#ifdef MY_ABC_HERE
		btrfs_qgroup_free(root, num_bytes);
	...
	if (root->fs_info->usrquota_enabled) {
#ifdef MY_ABC_HERE
		__btrfs_usrquota_free(root, inode, inode->i_uid, num_bytes, add_delay);
#else
		btrfs_usrquota_free(root, inode, inode->i_uid, num_bytes);
#endif /* MY_ABC_HERE */
	}
...
void btrfs_delalloc_release_metadata(struct inode *inode, u64 num_bytes)
{
	__btrfs_delalloc_release_metadata(inode, num_bytes, 0);
}

void btrfs_delalloc_release_metadata_add_delay(struct inode *inode, u64 num_bytes)
{
	__btrfs_delalloc_release_metadata(inode, num_bytes, 1);
}
#endif
...
#ifdef MY_ABC_HERE
static int should_add_to_unused_bgs(struct btrfs_fs_info *fs_info,
			        struct btrfs_block_group_cache *cache)
{
	if (cache->key.objectid == fs_info->log_tree_rsv_start)
		return 0;
	return 1;
}
#endif /* MY_ABC_HERE */
			      ...
			      u64 num_bytes, int alloc)
{
	...
	while (total) {
		...
		if (alloc) {
			 ...
			 */
#ifdef MY_ABC_HERE
			if (old_val == 0 && should_add_to_unused_bgs(info, cache)) {
#else
			if (old_val == 0) {
#endif /* MY_ABC_HERE */
	...
	while (1) {
		...
		if (btrfs_test_opt(root, DISCARD))
			ret = btrfs_discard_extent(root, start,
						   end + 1 - start, NULL, TRIM_SEND_TRIM);
#else /* MY_ABC_HERE */
		if (btrfs_test_opt(root, DISCARD))
			ret = btrfs_discard_extent(root, start,
						   end + 1 - start, NULL);
...
#endif
{
	...
	bool skinny_metadata = btrfs_fs_incompat(root->fs_info,
						 SKINNY_METADATA);
#ifdef MY_ABC_HERE
	bool has_clone_range = (extent_op ? !!(extent_op->flags_to_set &
						 BTRFS_EXTENT_FLAG_HAS_CLONE_RANGE) : 0);
#endif /* MY_ABC_HERE */

#ifdef MY_ABC_HERE
	if (!BTRFS_ANY_QUOTA_ENABLED(info) || !is_fstree(root_objectid))
#else
	...
	search_key.type = BTRFS_ROOT_ITEM_KEY;
	search_key.offset = (u64)-1;
	search_root = btrfs_read_fs_root_no_name(info, &search_key);
	if (!IS_ERR(search_root) && btrfs_root_disable_quota(search_root))
		no_quota = 1;
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
		ret = btrfs_syno_extent_usage_free(trans, info, 0, bytenr, num_bytes, refs_to_drop, 1);
		if (ret) {
			btrfs_abort_transaction(trans, extent_root, ret);
			goto out;
		}
...
#ifdef MY_ABC_HERE
		if (has_clone_range) {
			ret = check_root_inode_ref(trans, info, bytenr, 0,
					    root_objectid, owner_objectid, (u64)-1, 1);
			if (ret < 0) {
				btrfs_abort_transaction(trans, root, ret);
				goto out;
			} else if (ret > 0) {
...
#ifdef MY_ABC_HERE
#define MIN_DATA_CLUSTER_EMPTY_SIZE (128 * 1024 * 1024)
#endif /* MY_ABC_HERE */
				     ...
				     u64 flags, int delalloc)
{
	 ...
	 */
	if (orig_root->root_key.objectid == BTRFS_TREE_LOG_OBJECTID &&
		root->fs_info->log_tree_rsv_start) {
		use_cluster = false;
		hint_byte = root->fs_info->log_tree_rsv_start;
	}
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
			/*
			 * Reserved block group for tree log is not allowed to use cluster allocation
			 */
			if (block_group->key.objectid == root->fs_info->log_tree_rsv_start) {
				reserve_bytes = block_group->key.offset;
			} else if ((flags & BTRFS_BLOCK_GROUP_METADATA) && loop < LOOP_NO_RESERVE) {
#else
		...
		if (unlikely(last_ptr)) {
			...
			if (loop >= LOOP_NO_CLUSTER) {
				if (set_fragmented) {
					...
					spin_unlock(&last_ptr->lock);
					btrfs_warn(root->fs_info, "Set cluster fragmented, flags = %llu\n", flags);
				}

				/*
				 * In case someone already got cluster before
				 * we set last_ptr->fragmented = 1.
				 */
...
#ifdef MY_ABC_HERE
static int btrfs_legacy_allocator(struct btrfs_root *root,
			 u64 num_bytes, u64 min_alloc_size,
			 u64 empty_size, u64 hint_byte,
			 struct btrfs_key *ins, int is_data, int delalloc)
{
	...
	ret = __btrfs_reserve_extent(root, num_bytes, min_alloc_size, empty_size, hint_byte, ins, is_data, delalloc);
...
struct syno_allocation_ctl {
	...
	struct btrfs_block_group_cache *found_bg;
	u64 found_offset;
	u64 found_len;
	int stage;
...
static void syno_allocation_calc_data_ratio(struct syno_allocation_ctl *ctl)
{
	...
	struct list_head *head = &ctl->fs_info->space_info;
	struct btrfs_space_info *found;
	struct btrfs_space_info *space_info;
...
static int syno_alloction_prepare(struct syno_allocation_ctl *ctl)
{
	...
	WARN_ON(ctl->num_bytes < ctl->root->sectorsize);
...
static noinline void
syno_wait_block_group_cache_done(struct syno_allocation_ctl *ctl, struct btrfs_block_group_cache *cache, bool force)
{
	struct btrfs_caching_control *caching_ctl;
	...
	if (force) {
		...
		if (time_after(ctl->expire, cur)) {
			...
			wait_event_timeout(caching_ctl->wait, block_group_cache_done(cache), delay);
		}
	}
	put_caching_control(caching_ctl);
out:
...
static void syno_allocator_candidate_compare(struct syno_allocation_ctl *ctl, struct btrfs_block_group_cache *block_group, u64 offset, u64 len)
{
	bool release = true;

	if (offset < block_group->key.objectid ||
		offset + len > block_group->key.objectid + block_group->key.offset) {
		WARN_ON_ONCE(1);
		goto out;
	}
...
static bool syno_allocation_precheck_block_group(struct syno_allocation_ctl *ctl, struct btrfs_block_group_cache *block_group, bool check_meta_reserve_ratio_high)
{
...
static void syno_allocation_with_block_group(struct syno_allocation_ctl *ctl, struct btrfs_block_group_cache *block_group, u64 search_start, bool check_meta_reserve_ratio_high)
{
	...
	if (btrfs_test_opt(ctl->root, SPACE_CACHE))
		delalloc = 1;
	...
	if ((search_start < block_group->key.objectid) ||
		(search_start >= block_group->key.objectid + block_group->key.offset))
		search_start = block_group->key.objectid;

	use_bytes_index = (search_start == block_group->key.objectid);
again:
	max_extent_size = 0;
	...
	if (!offset) {
		if (!final_tried && max_extent_size) {
			...
			num_bytes = round_down(num_bytes, ctl->root->sectorsize);
	...
	align_offset = ALIGN(offset, ctl->root->stripesize);
	align_len = num_bytes;
	/* adjust align len */
	if (align_offset + align_len >
	    block_group->key.objectid + block_group->key.offset) {
		align_len = block_group->key.objectid + block_group->key.offset - align_offset;
	}
...
retry:
	...
	for (; node; node = rb_next(node)) {
		if (root == &space_info->syno_allocator.free_space_bytes)
			...
			block_group = rb_entry(node, struct btrfs_block_group_cache, syno_allocator.max_length_index);
		else if (root == &space_info->syno_allocator.free_space_max_length_with_extent)
			block_group = rb_entry(node, struct btrfs_block_group_cache, syno_allocator.max_length_with_extent_index);
		else
			BUG();
		btrfs_get_block_group(block_group);
		cached = block_group_cache_done(block_group);
		if (!caching && unlikely(!cached)) {
			spin_unlock(&space_info->syno_allocator.lock);
			err = cache_block_group(block_group, 0);
			if (err >= 0)
		...
		spin_unlock(&space_info->syno_allocator.lock);
		/* use bytes index */
		syno_allocation_with_block_group(ctl, block_group, block_group->key.objectid, true);
		spin_lock(&space_info->syno_allocator.lock);
		btrfs_put_block_group(block_group);
		break;
loop:
...
{
	int err;
	int index = __get_raid_index(ctl->flags);
	struct btrfs_space_info *space_info;
	struct btrfs_block_group_cache *block_group;
	bool cached;
...
search:
	list_for_each_entry(block_group, &space_info->block_groups[index], list) {
		...
		syno_allocation_with_block_group(ctl, block_group, block_group->key.objectid, false);
...
do_alloc:
	...
	ret = do_chunk_alloc(trans, root, ctl->flags, CHUNK_ALLOC_FORCE);
	if (ret < 0 && ret != -ENOSPC)
		btrfs_abort_transaction(trans, root, ret);
	else if (ret > 0)
		ret = 0;
...
static int syno_allocation(struct syno_allocation_ctl *ctl)
{
	...
	u64 free_bytes, fs_min_alloc_size;
	static bool __warned = false;
#ifdef MY_ABC_HERE
	bool use_log_bg = false;
#endif /* MY_ABC_HERE */
	int preload_nr_retry = 3;
	int alloc_type = (ctl->flags & BTRFS_BLOCK_GROUP_DATA) ?
		RESERVE_ALLOC_NO_ACCOUNT : RESERVE_ALLOC;
	bool data_match;
...
again:
	...
	if (space_info->syno_allocator.cache_bg) {
		...
		if (ctl->found_bg && ctl->found_len >= ctl->num_bytes)
			goto found;
		/* use bytes index */
		syno_allocation_with_block_group(ctl, space_info->syno_allocator.cache_bg, space_info->syno_allocator.cache_bg->key.objectid, false);
		if (ctl->found_bg && ctl->found_len >= ctl->num_bytes)
			goto found;
		spin_lock(&space_info->syno_allocator.lock);
	}
	...
	if ((node = rb_first_cached(&space_info->syno_allocator.preload))) {
		check_preload = true;
		block_group = rb_entry(node, struct btrfs_block_group_cache, syno_allocator.preload_index);
		if (ctl->stage < SYNO_ALLOCATION_STAGE_CACHING_FORCE_WAIT) {
			if (--preload_nr_retry < 0)
				goto skip_preload;
			spin_lock(&block_group->lock);
			free_bytes = block_group->key.offset - btrfs_block_group_used(&block_group->item);
			spin_unlock(&block_group->lock);
			if (!syno_allocation_precheck_free_space(ctl, block_group->key.offset, free_bytes, true))
				goto skip_preload;
		}
		btrfs_get_block_group(block_group);
		cached = block_group_cache_done(block_group);
...
skip_preload:
	...
	if (ctl->stage == SYNO_ALLOCATION_STAGE_RELINK_RETRY) {
		relink_nr_retry--;
		if ((ctl->found_bg && ctl->found_len >= ctl->min_alloc_size) ||
			(ctl->is_data && ctl->min_alloc_size != ctl->root->sectorsize) ||
			relink_nr_retry < 0) {
			ctl->stage++;
		} else {
...
found:
	...
	ret = btrfs_update_reserved_bytes(ctl->found_bg, ctl->found_len, alloc_type, ctl->delalloc);
	if (ret == -EAGAIN) {
		/* release candidate */
		btrfs_add_free_space(ctl->found_bg, ctl->found_offset, ctl->found_len);
		atomic_dec(&ctl->found_bg->syno_allocator.refs);
	...
	trace_btrfs_reserve_extent(ctl->root, ctl->found_bg, ctl->found_offset, ctl->found_len);
...
skip_cache_found_bg:
	...
	ret = 0;
out:
	fs_min_alloc_size = ctl->is_data ? ctl->root->sectorsize : ctl->root->nodesize;
	if (ret && ctl->min_alloc_size == fs_min_alloc_size && !__warned) {
		...
		dump_space_info(space_info, ctl->num_bytes, 0);
		WARN_ON_ONCE(1);
		__warned = true;
	}
	if (ctl->found_bg) {
 ...
 */
static int btrfs_syno_allocator(struct btrfs_root *root,
			 u64 num_bytes, u64 min_alloc_size,
			 u64 empty_size, u64 hint_byte,
			 struct btrfs_key *ins, int is_data, int delalloc)
{
	...
	ctl.flags = btrfs_get_alloc_profile(ctl.root, ctl.is_data);
	ctl.space_info = __find_space_info(ctl.fs_info, ctl.flags);
	if (!ctl.space_info) {
	...
	if (ctl.space_info->syno_allocator.force_cluster_disable) {
		...
		cluster = fetch_cluster_info(ctl.root, ctl.space_info, &empty_cluster);
		if (cluster) {
			spin_lock(&cluster->refill_lock);
			 ...
			 struct btrfs_key *ins, int is_data, int delalloc)
{
	if (btrfs_test_opt(root, SYNO_ALLOCATOR))
		return btrfs_syno_allocator(root, num_bytes, min_alloc_size, empty_size, hint_byte, ins, is_data, delalloc);
	return btrfs_legacy_allocator(root, num_bytes, min_alloc_size, empty_size, hint_byte, ins, is_data, delalloc);
}
...
#ifdef MY_ABC_HERE
		if (btrfs_test_opt(root, DISCARD))
			ret = btrfs_discard_extent(root, start, len, NULL, TRIM_SEND_TRIM);
#else /* MY_ABC_HERE */
		if (btrfs_test_opt(root, DISCARD))
...
#ifdef MY_ABC_HERE
				      BTRFS_QGROUP_OPER_ADD_EXCL, 0, node);
#else
...
#endif
{
	...
	search_key.type = BTRFS_ROOT_ITEM_KEY;
	search_key.offset = (u64)-1;
	search_root = btrfs_read_fs_root_no_name(root->fs_info, &search_key);
#endif /* MY_ABC_HERE */

#ifdef MY_ABC_HERE
	ref.objectid = owner;
	ref.uid = uid;
	ref.node.type = BTRFS_EXTENT_DATA_REF_KEY;
	ref.node.is_head = 0;
	ref.inode = syno_usrquota_inode_get(inode);
#endif /* MY_ABC_HERE */
#ifdef MY_ABC_HERE
	ref.node.no_quota = (!is_fstree(root_objectid) || !root->fs_info->quota_enabled) || (!IS_ERR(root) && btrfs_root_disable_quota(search_root));
#else
	ref.node.no_quota = (!is_fstree(root_objectid) || !root->fs_info->quota_enabled);
#endif /* MY_ABC_HERE */
	...
	ret = alloc_reserved_file_extent(trans, root, 0, root_objectid,
					 0, owner, offset, ins, 1, &ref.node);
#else
	ret = alloc_reserved_file_extent(trans, root, 0, root_objectid,
					 0, owner, offset, ins, 1);
#endif
#ifdef MY_ABC_HERE
	syno_usrquota_inode_put(ref.inode);
#endif /* MY_ABC_HERE */
	btrfs_put_block_group(block_group);
	return ret;
}
		      ...
		      u64 bytenr, int level, u64 owner)
{
	...
	write_extent_buffer_fsid(buf, root->fs_info->fs_devices->metadata_uuid);
	write_extent_buffer_chunk_tree_uuid(buf, root->fs_info->chunk_tree_uuid);
	if (root->root_key.objectid == BTRFS_TREE_LOG_OBJECTID) {
...
#ifdef MY_ABC_HERE
	if (block_rsv == &root->fs_info->cleaner_block_rsv) {
...
#ifdef MY_ABC_HERE
				/* we don't calculate for metadata, so change no_quota to 1 */
				1
#else
				0
#endif /* MY_ABC_HERE */
...
{
	u64 num_bytes = btrfs_calc_trans_metadata_size(fs_info->tree_root, 1);
	u64 total_bytes;
	int err;
	...
	while(total_bytes > 0 && max_items > 0) {
		err = reserve_metadata_bytes(fs_info->extent_root, block_rsv, num_bytes, BTRFS_RESERVE_FLUSH_ALL);
		if (err)
			goto out;
			 ...
			 int for_reloc)
{
	...
	while (1) {
		...
		if (
#ifdef MY_ABC_HERE
			/* Must be before btrfs_should_end_transaction */
			true ||
#endif /* MY_ABC_HERE */
			btrfs_should_end_transaction(trans, tree_root)
#ifdef MY_ABC_HERE
			|| (!for_reloc && btrfs_comp_cpu_keys(&drop_key, &syno_usage_dummy_key) == 0)
#endif /* MY_ABC_HERE */
		    || (!for_reloc && btrfs_need_cleaner_sleep(root))
#ifdef MY_ABC_HERE
		    || (!for_reloc && !root->fs_info->snapshot_cleaner)
#endif /* MY_ABC_HERE */
			) {
			...
			ret = btrfs_run_delayed_refs(trans, tree_root, updates * 2);
			if (ret) {
				btrfs_abort_transaction(trans, tree_root, ret);
				err = ret;
				goto out_end_trans;
...
#ifdef MY_ABC_HERE
	if (err) {
		btrfs_abort_transaction(trans, tree_root, err);
		goto out_end_trans;
	}
#else /* MY_ABC_HERE */
	if (err)
...
#ifdef MY_ABC_HERE
	ret = btrfs_syno_usage_root_status_remove(trans, fs_info, root->objectid);
	if (ret) {
		btrfs_abort_transaction(trans, tree_root, ret);
		err = ret;
		goto out_end_trans;
	...
	if (!list_empty(&root->root_list)) {
		btrfs_warn(root->fs_info, "root_list not empty when drop snapshot");
		WARN_ON_ONCE(1);
		spin_lock(&root->fs_info->trans_lock);
...
out:
	...
	if (!for_reloc && root_dropped == false &&
		...
		btrfs_warn(root->fs_info, "root_list not empty when drop snapshot early exit");
		WARN_ON_ONCE(1);
		spin_lock(&fs_info->trans_lock);
		list_del_init(&root->root_list);
		spin_unlock(&fs_info->trans_lock);
...
int btrfs_clean_block_group_cache_tree(struct btrfs_fs_info *fs_info)
{
	...
	clean_tree_block(trans, tree_root, block_group_cache_root->node);
...
int btrfs_check_syno_block_group_cache_tree(struct btrfs_fs_info *fs_info)
{
	...
	/* Not really possible */
	if (!chunk_em) {
		btrfs_err(fs_info, "Failed to find first extent mapping");
		ret = -ENOENT;
		goto out;
	}
...
#ifdef MY_ABC_HERE
		cache->syno_allocator.space_info = __find_space_info(cache->fs_info, cache->flags);
		if (!cache->syno_allocator.space_info) {
			btrfs_err(cache->fs_info, "No space info for %llu", cache->flags);
			btrfs_put_block_group(cache);
			ret = -EINVAL;
			goto error;
		}
...
#ifdef MY_ABC_HERE
		btrfs_syno_allocator_preload_block_group(cache, found_key.offset - btrfs_block_group_used(&cache->item));
		btrfs_syno_allocator_relink_block_group(cache);
#endif /* MY_ABC_HERE */
		/*
...
#ifdef MY_ABC_HERE
		if (cache->key.objectid == root->fs_info->log_tree_rsv_start)
			root->fs_info->log_tree_rsv_size = cache->key.offset;
#endif /* MY_ABC_HERE */
		if (btrfs_chunk_readonly(root, cache->key.objectid)) {
			set_block_group_ro(cache, 1);
#ifdef MY_ABC_HERE
		} else if (btrfs_block_group_used(&cache->item) == 0 && should_add_to_unused_bgs(root->fs_info, cache)) {
#else
		} else if (btrfs_block_group_used(&cache->item) == 0) {
#endif /* MY_ABC_HERE */
			spin_lock(&info->unused_bgs_lock);
...
#ifdef MY_ABC_HERE
	if(fs_info->metadata_cache_enable && has_metadata_group && !ret) {
			     ...
			     struct extent_map *em)
{
	...
	if (block_group->has_caching_ctl) {
...
#ifdef MY_ABC_HERE
		spin_lock(&root->fs_info->caching_block_groups_lock);
#endif /* MY_ABC_HERE */
		if (!caching_ctl) {
			struct btrfs_caching_control *ctl;
		...
		if (caching_ctl)
			list_del_init(&caching_ctl->list);
#ifdef MY_ABC_HERE
		spin_unlock(&root->fs_info->caching_block_groups_lock);
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
/*
 * This function has almost same logic as btrfs_trim_free_extents
 * @device: device which want to send unused hints
 * @start: input start position and return next start position
 * @minlen: min free space length
 * @hint_bytes: total bytes we actually send as hints
 */
static int btrfs_hint_free_extents(struct btrfs_device *device, u64 *start,
				   u64 minlen, u64 *hint_bytes)
{
	struct btrfs_fs_info *fs_info = device->dev_root->fs_info;
	struct btrfs_transaction *trans;
	u64 len = 0;
	...
	/* Send 4G data at most to avoid too much bio split */
	if (len > (UINT_MAX & (~511)))
		len = (UINT_MAX & (~511));

	ret = btrfs_issue_discard(device->bdev, *start, len, &bytes, TRIM_SEND_HINT);
	up_read(&fs_info->commit_root_sem);
	mutex_unlock(&fs_info->chunk_mutex);
...
btrfs_next_hint_device(struct list_head *devices, u8 *uuid)
{
	struct btrfs_device *dev;
	struct btrfs_device *next = NULL;

	/*
	* find a device which has the smallest uuid but
	* larger than current one
	*/
	list_for_each_entry(dev, devices, dev_alloc_list) {
		/* must larger than current uuid */
		if (memcmp(dev->uuid, uuid, BTRFS_UUID_SIZE) <= 0)
			continue;

		/* find the smallest */
		if (!next || memcmp(dev->uuid, next->uuid, BTRFS_UUID_SIZE) < 0)
			next = dev;
	}
	return next;
}

static int btrfs_hint_unused_chunk_space(struct btrfs_fs_devices *fs_devices,
				   u64 minlen,
				   u64 *hint_bytes)
{
	struct btrfs_device *dev, *working_dev;
	...
	*hint_bytes = 0;
	...
	/* start from the device who has smallest uuid */
	mutex_lock(&fs_devices->device_list_mutex);
	devices = &fs_devices->alloc_list;
	...
	working_dev = list_first_entry(devices, struct btrfs_device, dev_alloc_list);
	list_for_each_entry(dev, devices, dev_alloc_list) {
		if (memcmp(dev->uuid, working_dev->uuid, BTRFS_UUID_SIZE) < 0)
			working_dev = dev;
	}
	...
	while (1) {
		...
		list_for_each_entry(dev, devices, dev_alloc_list) {
			if (dev == working_dev) {
				ret = btrfs_hint_free_extents(working_dev,
							      &start,
							      minlen,
							      hint_bytes);
				break;
		...
		if (ret == -ENOSPC) {
			/* no spaces, go next device */
			working_dev = btrfs_next_hint_device(devices, uuid);
			start = 0;
			ret = 0;
		}
		mutex_unlock(&fs_devices->device_list_mutex);

		if (ret || !working_dev) /* error or no more device */
			break;
...
#endif /* MY_ABC_HERE */
{
	...
	if (act == TRIM_SEND_HINT) {
		...
		ret = btrfs_hint_unused_chunk_space(fs_info->fs_devices,
							range->minlen, &group_trimmed);
		if (ret)
			return ret;
