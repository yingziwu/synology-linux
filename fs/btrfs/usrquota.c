...
struct btrfs_usrquota {
	...
	u64 uq_reserved;
#ifdef MY_ABC_HERE
	u64 uq_delayed_free; /* wait quota to be added to rfer_used in accounting function */
	bool uq_not_accounting;
#endif
...
#ifdef MY_ABC_HERE
	if (sgn == 1 && usrquota->uq_not_accounting) {
		if (usrquota->uq_delayed_free > oper->num_bytes)
			usrquota->uq_delayed_free -= oper->num_bytes;
		else {
			usrquota->uq_delayed_free = 0;
			usrquota->uq_not_accounting = 0;
		}
		if (binode && (binode->flags & BTRFS_INODE_UQ_REF_USED)) {
			if (binode->syno_uq_delayed_free > oper->num_bytes) {
				binode->syno_uq_delayed_free -= oper->num_bytes;
			} else {
				binode->syno_uq_delayed_free = 0;
			}
		}
	}
#endif
...
#ifdef MY_ABC_HERE
int __btrfs_usrquota_free(struct btrfs_root *root, struct inode *inode, uid_t uid, u64 num_bytes, int add_delay)
#else
...
#ifdef MY_ABC_HERE
	if (usrquota->uq_not_accounting && 1 == add_delay) {
		usrquota->uq_delayed_free += num_bytes;
		...
		if (binode && (binode->flags & BTRFS_INODE_UQ_REF_USED)) {
			binode->syno_uq_delayed_free += num_bytes;
		...
		if (usrquota->uq_delayed_free < num_bytes) {
			usrquota->uq_delayed_free = 0;
		} else {
			usrquota->uq_delayed_free -= num_bytes;
		}
		if (binode && (binode->flags & BTRFS_INODE_UQ_REF_USED)) {
			if (binode->syno_uq_delayed_free < num_bytes) {
				binode->syno_uq_delayed_free = 0;
			} else {
				binode->syno_uq_delayed_free -= num_bytes;
			}
...
#ifdef MY_ABC_HERE
		if (usrquota_orig->uq_delayed_free < binode->syno_uq_delayed_free) {
			binode->syno_uq_delayed_free = usrquota_orig->uq_delayed_free;
		}
		usrquota_orig->uq_delayed_free -= binode->syno_uq_delayed_free;
		usrquota_dest->uq_delayed_free += binode->syno_uq_delayed_free;
#endif /* MY_ABC_HERE */
	}
	inode->i_uid = new_uid;
