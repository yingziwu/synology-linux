			    ...
			    struct btrfs_device *device)
{
	...
	write_extent_buffer(leaf, root->fs_info->fs_devices->metadata_uuid,
			    ptr, BTRFS_FSID_SIZE);
	btrfs_mark_buffer_dirty(leaf);
...
again:
	...
	while (1) {
		...
		if (bctl->fast_key_offset && found_key.offset != key.offset - 1) {
			mutex_unlock(&fs_info->delete_unused_bgs_mutex);
			break;
		}
#endif /* MY_ABC_HERE */
		  ...
		  struct btrfs_ioctl_balance_args *bargs)
{
	 ...
	 *   Caught by btrfs_should_cancel_balance() and got -ECANCELED.
...
#endif /* MY_ABC_HERE */
{
	...
	if (start > U64_MAX - (64ULL* SZ_1G))
		start = find_free_chunk_overflow(info, num_bytes);
#endif /* MY_ABC_HERE */
...
#endif /* MY_ABC_HERE */
{
	...
	if (dev_replace_is_ongoing && mirror_num == map->num_stripes + 1 &&
		...
		ret = __btrfs_map_block(fs_info, REQ_GET_READ_MIRRORS,
			     logical, &tmp_length, &tmp_bbio, 0, NULL, tree_log);
#else
		ret = __btrfs_map_block(fs_info, REQ_GET_READ_MIRRORS,
...
again:
#ifdef MY_ABC_HERE
	bio = btrfs_bio_alloc(dev->bdev, physical >> 9, nr_vecs, GFP_NOFS, rw);
#else
			...
			struct btrfs_dev_item *dev_item)
{
	...
	if (memcmp(fs_uuid, root->fs_info->fs_devices->metadata_uuid, BTRFS_FSID_SIZE)) {
...
static void readahead_tree_node_children(struct extent_buffer *node)
{
	...
	blocksize = btrfs_level_size(node->fs_info->chunk_root, 0);
	...
	for (i = 0; i < nr_items; i++) {
		...
		readahead_tree_block(node->fs_info->chunk_root, start, blocksize, gen);
	}
}
