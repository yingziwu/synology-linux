...
static void add_cksumfailed_file(struct btrfs_fs_info *fs_info, struct btrfs_root *root, struct inode *inode)
{
	...
	if (len == sizeof(rec)) {
		if (rec.sub_vol == root->root_key.objectid && rec.ino == inode->i_ino) {
			goto out;
		}
	} else {
		WARN_ON(0 != len);
	...
	rec.sub_vol = root->root_key.objectid;
	rec.ino = inode->i_ino;
...
#ifdef MY_ABC_HERE
			if (-EBADF == ret) {
				add_cksumfailed_file(fs_info, root, inode);
			  ...
			  unsigned long len)
{
	...
	ret = SYNOUnicodeUTF8Strcmp((char *)ptrv, buf, len_ptrv, len_copy, 0);
	return ret;
}
#endif
