				    ...
				    const u64 generation)
{
	...
	ino_key.objectid = i_ino;
	ino_key.type = BTRFS_INODE_ITEM_KEY;
	ino_key.offset = 0;

	inode = btrfs_iget(sb, &ino_key, root, NULL);
				    ...
				    u64 subvol_id)
{
	...
	while (1) {
		trans = btrfs_start_transaction_fallback_global_rsv(feat_root, 1, 5);
		if (IS_ERR(trans))
			goto out;
		...
		ret = btrfs_end_transaction_throttle(trans, feat_root);
		if (ret)
			goto out;
		trans = NULL;
	}
...
end_trans:
	if (trans)
		btrfs_end_transaction_throttle(trans, feat_root);
out:
	btrfs_release_path(path);
	return ret;
}
				...
				const int insert_cnt)
{
	...
	ret = btrfs_search_slot(trans, feat_root, &key, path,
		sizeof(struct btrfs_rbd_meta_file_subvol_record_item), 1);
	if (ret < 0)
		goto out;
	else if (ret) {
		if (insert_cnt < 0) {
