			  ...
			  int slot)
{
	...
	while (cur < item_size) {
		...
		if (name_len + data_len > BTRFS_MAX_XATTR_SIZE(fs_info->tree_root)) {
			dir_item_err(leaf, slot,
			"dir item name and data len too long, have %u max %zu",
				name_len + data_len,
				BTRFS_MAX_XATTR_SIZE(fs_info->tree_root));
			return -EUCLEAN;
		}
				 ...
				 struct btrfs_key *key, int slot)
{
	...
	if (btrfs_item_size_nr(leaf, slot) < sizeof(struct btrfs_chunk)) {
		chunk_err(leaf, chunk, key->offset,
			"invalid chunk item size: have %u expect [%zu, %lu)",
...
"invalid key type, METADATA_ITEM type invalid when SKINNY_METADATA feature disabled");
	...
	if (item_size < sizeof(*ei)) {
		extent_err(leaf, slot,
			   "invalid item size, have %u expect [%zu, %lu)",
			   item_size, sizeof(*ei),
...
static void fix_node_key(struct btrfs_root *root, struct extent_buffer *node, int slot)
{
	...
	eb = read_tree_block(root, bytenr, generation, btrfs_header_level(node) - 1, NULL);
	if (IS_ERR(eb))
		goto can_fix;
...
static int check_leaf(struct extent_buffer *leaf, bool check_item_data)
{
	...
	"bad key order, prev (%llu %u %llu) current (%llu %u %llu)",
		 ...
		 */
		if (slot == 0)
			item_end_expected = BTRFS_LEAF_DATA_SIZE(fs_info->tree_root);
		else
			item_end_expected = btrfs_item_offset_nr(leaf,
								 slot - 1);
#ifdef MY_ABC_HERE
		if (btrfs_item_end_nr(leaf, slot) != item_end_expected) {
				if (fix_item_offset_size(fs_info->tree_root, leaf, slot)) {
					generic_err(leaf, slot,
				...
				btrfs_warn(fs_info, "corrupt leaf fixed, slot offset bad, "
		...
		if (btrfs_item_end_nr(leaf, slot) >
		    BTRFS_LEAF_DATA_SIZE(fs_info->tree_root)) {
			generic_err(leaf, slot,
			"slot end outside of leaf, have %u expect range [0, %lu]",
				btrfs_item_end_nr(leaf, slot),
				BTRFS_LEAF_DATA_SIZE(fs_info->tree_root));
			return -EUCLEAN;
		}
		...
		if (btrfs_comp_cpu_keys(&prev_key, &key) >= 0) {
			 ...
			 */
			fix_item_key(fs_info, leaf, slot, prev_key);
			fix_item_key(fs_info, leaf, slot + 1, key);
			...
			btrfs_warn(fs_info, "corrupt leaf fixed, bad key order, "
					"block=%llu, root=%llu, slot=%d", btrfs_header_bytenr(leaf), btrfs_header_owner(leaf), slot);
			// Need this, so we fix it in repair_eb_io_failure().
			set_bit(EXTENT_BUFFER_CORRUPT, &leaf->bflags);
		}
...
int btrfs_check_node(struct extent_buffer *node)
{
	...
	if (nr == 0 || nr > BTRFS_NODEPTRS_PER_BLOCK(fs_info->tree_root)) {
		btrfs_crit(fs_info,
...
#ifdef MY_ABC_HERE
			fix_node_key(fs_info->tree_root, node, slot);
			fix_second = 0;
fix_next:
			btrfs_node_key_to_cpu(node, &key, slot);
			btrfs_node_key_to_cpu(node, &next_key, slot + 1);
			...
			if (btrfs_comp_cpu_keys(&key, &next_key) >= 0) {
				if (!fix_second) {
					fix_second = 1;
					fix_node_key(fs_info->tree_root, node, slot + 1);
					goto fix_next;
				} else {
					generic_err(node, slot,
			...
			btrfs_warn(fs_info, "corrupt node fixed, bad key order, "
