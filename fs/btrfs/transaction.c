...
loop:
	...
	spin_lock_init(&cur_trans->qgroup_ref_lock);
	INIT_LIST_HEAD(&cur_trans->qgroup_ref_list);
#endif
#ifdef MY_ABC_HERE
...
#ifdef MY_ABC_HERE
	ret = btrfs_usrquota_mksnap(trans, fs_info, root->root_key.objectid, objectid,
	                                       pending->readonly, pending->copy_limit_from);
	if (ret) {
