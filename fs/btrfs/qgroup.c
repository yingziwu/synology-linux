...
#ifdef MY_ABC_HERE
	qgroup->rfer += sgn * node->num_bytes;
	qgroup->rfer_cmpr += sgn * node->num_bytes;
		   ...
		   struct extent_buffer *scratch_leaf)
{
	...
	int ret;
#ifdef MY_ABC_HERE
	/* FIXME use a search tree to reduce memory consumption */
	LIST_HEAD(roots_no_dedup);
	struct u64_list *cur_node;
	struct u64_list *next_node;
	struct btrfs_qgroup *qg;
#endif
...
#ifdef MY_ABC_HERE
		ret = btrfs_find_all_roots_nodedup(trans, fs_info, found.objectid,
					   tree_mod_seq_elem.seq, &roots_no_dedup);
		if (0 > ret) {
			goto out;
		}

		spin_lock(&fs_info->qgroup_lock);
		list_for_each_entry_safe(cur_node, next_node, &roots_no_dedup, list) {
			qg = find_qgroup_rb(fs_info, cur_node->val1);
			if (qg) {
				qg->rfer += num_bytes;
