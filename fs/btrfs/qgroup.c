...
#ifdef MY_ABC_HERE
	qqa->reserved = qgroup->reserved + qgroup->delayed_free;
#else
...
#endif /* MY_ABC_HERE */
{
	...
	oper->elem.seq = 0;
#ifdef MY_ABC_HERE
	oper->ref_type = node->type;
	if (node->type == BTRFS_EXTENT_DATA_REF_KEY ||
			node->type == BTRFS_SHARED_DATA_REF_KEY) {
		ref = btrfs_delayed_node_to_data_ref(node);
		oper->ram_bytes = ref->ram_bytes;
#ifdef MY_ABC_HERE
		oper->objectid = ref->objectid;
		oper->uid = ref->uid;
		oper->inode = syno_usrquota_inode_get(ref->inode);
#endif /* MY_ABC_HERE */
	}
	spin_lock(&transaction->qgroup_ref_lock);
	list_add_tail(&oper->list, &transaction->qgroup_ref_list);
	spin_unlock(&transaction->qgroup_ref_lock);
#else
...
#ifdef MY_ABC_HERE
	if (unlikely(sign < 0 && qgroup->rfer < oper->num_bytes)) {
		//underflow
		qgroup->rfer = 0;
	} else {
		qgroup->rfer += sign * oper->num_bytes;
	}
	if (unlikely(sign < 0 && qgroup->rfer_cmpr < oper->ram_bytes)) {
		//underflow
		qgroup->rfer_cmpr = 0;
	} else {
		qgroup->rfer_cmpr += sign * oper->ram_bytes;
	}
#else
	qgroup->rfer += sign * oper->num_bytes;
				    ...
				    struct btrfs_fs_info *fs_info)
{
	...
	while (1) {
		spin_lock(&transaction->qgroup_ref_lock);
		if (list_empty(&transaction->qgroup_ref_list)) {
			spin_unlock(&transaction->qgroup_ref_lock);
			break;
		}
		oper = list_first_entry(&transaction->qgroup_ref_list,
					struct btrfs_qgroup_operation, list);
		list_del_init(&oper->list);
		spin_unlock(&transaction->qgroup_ref_lock);
		if (!ret || !trans->aborted)
			ret = btrfs_qgroup_account(trans, fs_info, oper);
		if (!ret1 || !trans->aborted)
			ret1 = btrfs_usrquota_account_ref(
			           trans, fs_info, oper);
		btrfs_put_tree_mod_seq(fs_info, &oper->elem);
		syno_usrquota_inode_put(oper->inode);
		kfree(oper);
	}
...
#ifdef MY_ABC_HERE
		if (qg->not_accounting && 1 == add_delay) { /* move reserved to delayed_free */
			qg->delayed_free += num_bytes;
			qg->reserved -= num_bytes;
		} else if (2 == add_delay) { /* delayed ref dropped, release delayed_free */
			qg->delayed_free -= num_bytes;
		} else
			qg->reserved -= num_bytes;
#else
...
#ifdef MY_ABC_HERE
/*
 * 0: normal free, skip delayed_free operation.
 * 1: move reserved to delayed_free to prevent over run.
 * 2: decrease delayed_free, drop delayed_ref operation needed.
 */
void btrfs_qgroup_free(struct btrfs_root *root, u64 num_bytes)
{
	__btrfs_qgroup_free(root, num_bytes, 0);
}
void btrfs_qgroup_free_add_delay(struct btrfs_root *root, u64 num_bytes)
{
	__btrfs_qgroup_free(root, num_bytes, 1);
}
void btrfs_qgroup_release_delayed_free(struct btrfs_root *root, u64 num_bytes)
{
	__btrfs_qgroup_free(root, num_bytes, 2);
}
#endif
...
void assert_qgroups_uptodate(struct btrfs_trans_handle *trans)
{
	...
	spin_lock(&transaction->qgroup_ref_lock);
	if (list_empty(&transaction->qgroup_ref_list) && !trans->delayed_ref_elem.seq) {
		spin_unlock(&transaction->qgroup_ref_lock);
		return;
	}
	btrfs_err(trans->root->fs_info,
		...
		"seq is %#x.%x",
		trans, list_empty(&transaction->qgroup_ref_list) ? "" : " not",
...
#ifdef MY_ABC_HERE
	/*
	 * The rescan mechanism is broken right now, just skip it.
	 * The related code is removed to make it look clear.
	 */
