...
static int __btrfs_add_ordered_extent(struct inode *inode, u64 file_offset,
				      ...
				      int type, int dio, int compress_type, int write_sync)
#else
static int __btrfs_add_ordered_extent(struct inode *inode, u64 file_offset,
				      u64 start, u64 len, u64 disk_len,
...
#endif /* MY_DEF_HERE */
{
	...
	if (type == BTRFS_ORDERED_NOCOW || type == BTRFS_ORDERED_PREALLOC) {
		...
		ret = btrfs_qgroup_free_data(inode, file_offset, len);
		if (ret < 0)
			return ret;
		ret = 0;
	} else {
...
#ifdef MY_DEF_HERE
	if (write_sync)
		set_bit(BTRFS_ORDERED_SYNC, &entry->flags);
#endif /* MY_DEF_HERE */
			     ...
			     u64 start, u64 len, u64 disk_len, int type, int write_sync)
{
	return __btrfs_add_ordered_extent(inode, file_offset, start, len,
					  disk_len, type, 0,
					  BTRFS_COMPRESS_NONE, write_sync);
}
#else
int btrfs_add_ordered_extent(struct inode *inode, u64 file_offset,
