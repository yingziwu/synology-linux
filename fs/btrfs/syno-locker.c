...
{
	if (btrfs_ino(&binode->vfs_inode) != BTRFS_FIRST_FREE_OBJECTID)
		return false;
...
void btrfs_syno_locker_update_work_fn(struct work_struct *work)
{
	...
	if ((fs_info->sb->s_flags & MS_RDONLY) || !btrfs_fs_compat_ro(fs_info, LOCKER)) {
...
{
	return btrfs_ino(d_inode(dentry)) == BTRFS_FIRST_FREE_OBJECTID;
}
...
static int btrfs_syno_locker_disk_root_update_trans(struct btrfs_root *root)
{
	...
	return btrfs_commit_transaction(trans, feat_root);

abort:
	btrfs_abort_transaction(trans, feat_root, ret);
	btrfs_end_transaction(trans, feat_root);
	return ret;
}
				 ...
				 bool reset)
{
	...
	u64 new_ino = new_inode ? btrfs_ino(new_inode) : 0;
...
static inline int syno_locker_xattr_set(struct btrfs_trans_handle *trans, struct inode *inode,
				       const void *buffer, size_t size, int flags)
{
	return __btrfs_setxattr(trans, inode, XATTR_SYNO_LOCKER, buffer, size, flags);
}
...
static int syno_locker_disk_inode_update(struct btrfs_trans_handle *trans, struct inode *inode)
{
	...
	WARN_ON(btrfs_ino(inode) == BTRFS_FIRST_FREE_OBJECTID);
	...
	if (ret) {
		...
		btrfs_abort_transaction(trans, binode->root, ret);
	}
...
int btrfs_syno_locker_disk_inode_read(struct inode *inode)
{
	...
	ret = __btrfs_getxattr(inode, XATTR_SYNO_LOCKER, &xattr, sizeof(xattr));
...
inline int btrfs_syno_locker_fillattr(struct inode *inode, struct kstat *stat)
{
	...
	if (binode->locker_period_end != LOCKER_DEFAULT_PERIOD_END &&
		...
		atime = binode->locker_period_end + delta.tv_sec;
		__truncate_time(&atime);
		stat->atime.tv_sec = atime;
					   ...
					   struct btrfs_ioctl_syno_locker_args *args)
{
	...
	if (args->flags & BTRFS_LOCKER_STATE) {
		...
		if (args->state != LS_OPEN && S_ISDIR(binode->vfs_inode.i_mode) &&
		    btrfs_ino(&binode->vfs_inode) != BTRFS_FIRST_FREE_OBJECTID) {
...
out:
	if (ret)
		btrfs_err(binode->root->fs_info, "failed to set xattr (%u, %lld, %lld, %lld). ret=%d",
...
out:
	...
	if (ret) {
		...
		if (!pathname)
			return ret;
		path = file_path(file, pathname, PATH_MAX + 11);
		if (IS_ERR(path))
			path = "<unknown>";
		btrfs_err(fs_info, "failed to set locker properties of '%s'. err=%d.", path, ret);
		kfree(pathname);
	}
