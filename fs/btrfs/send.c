...
#endif /* MY_DEF_HERE */
			ret = send_set_xattr(sctx, p, SYNO_ACL_XATTR_ACCESS, strlen(SYNO_ACL_XATTR_ACCESS), data, data_len);
			if (ret < 0)
				goto out;
#ifdef MY_DEF_HERE
			}
			     ...
			     void *ctx)
{
	...
	return record_ref(ppctx->sctx->parent_root, num, dir, index, name,
	                  ppctx->sctx, ppctx->refs);
}
