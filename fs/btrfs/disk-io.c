			   ...
			   int verify)
{
	...
	if (verify) {
		if (memcmp_extent_buffer(buf, result, 0, csum_size)) {
			...
			if (buf->nr_retry && buf->can_retry) {
				if (buf->nr_retry > 1 && buf->prev_bad_csum == found)
					set_bit(EXTENT_BUFFER_RETRY_ERR, &buf->bflags);
				else
					buf->prev_bad_csum = found;
			}
#else
...
#ifdef MY_ABC_HERE
	if (-EIO == ret && root->fs_info->open && !btrfs_root_readonly(root)
			&& !(test_bit(BTRFS_FS_STATE_TRANS_ABORTED, &root->fs_info->fs_state))
			&& __ratelimit(&meta_err_rate_limit)) {
		btrfs_err(root->fs_info, "cannot fix %llu, record in meta_err", eb->start);
		SynoBtrfsMetaCorruptedReport(root->fs_info->fs_devices->fsid, eb->start);
	}
#endif /* MY_ABC_HERE */
	return ret;
}
				 ...
				 struct extent_buffer *eb)
{
	...
	while (fs_devices) {
		 ...
		 */
		if (fs_devices == root->fs_info->fs_devices &&
		    btrfs_fs_incompat(root->fs_info, METADATA_UUID))
...
static void fix_node_key(struct btrfs_root *root, struct extent_buffer *node, int slot)
{
	...
	blocksize = btrfs_level_size(root, btrfs_root_level(&root->root_item) - 1);
			       ...
			       struct extent_buffer *leaf)
{
	...
	for (slot = 0; slot < nritems - 1; slot++) {
		...
		// We don't need to check item end for slot.
#else
		/*
...
fix_next:
			...
			if (btrfs_comp_cpu_keys(&key, &next_key) >= 0) {
				if (!fix_second) {
					fix_second = 1;
					fix_node_key(root, node, slot + 1);
					goto fix_next;
				} else {
					btrfs_crit(root->fs_info, "node bad key order, "
							"block=%llu, root=%llu, slot=%d", btrfs_header_bytenr(node), btrfs_header_owner(node), slot);
					return -EIO;
				}
			}
			btrfs_warn(root->fs_info, "corrupt node fixed, bad key order, "
					"block=%llu, root=%llu, slot=%d", btrfs_header_bytenr(node), btrfs_header_owner(node), slot);
			 ...
			 u64 parent_transid)
{
	...
	read_extent_buffer_pages(&BTRFS_I(btree_inode)->io_tree,
				 buf, 0, WAIT_NONE, btree_get_extent, 0, 0, parent_transid);
#else
	read_extent_buffer_pages(&BTRFS_I(btree_inode)->io_tree,
...
#ifdef MY_ABC_HERE
static struct btrfs_root *btrfs_alloc_root(struct btrfs_fs_info *fs_info, int monitor)
#else
...
#endif /* MY_ABC_HERE */
{
	...
	if (percpu_counter_init(&root->eb_hit, 0) ||
			percpu_counter_init(&root->eb_miss, 0)) {
...
#ifdef MY_ABC_HERE
	root = btrfs_alloc_root(fs_info, 0);
#else
	root = btrfs_alloc_root(fs_info);
#endif /* MY_ABC_HERE */
	if (!root)
...
int btrfs_init_fs_root(struct btrfs_root *root)
{
	...
	ret = percpu_counter_init(delalloc_bytes, 0);
	if (ret < 0)
		goto fail;
			    ...
			    struct btrfs_super_block *sb, int mirror_num)
{
	...
	if (nodesize != btrfs_super_leafsize(sb)) {
	       ...
	       char *options)
{
...
#ifdef MY_ABC_HERE
	BUG_ON(sizeof(struct btrfs_super_block) != 4096);
#endif /* MY_ABC_HERE */
#ifdef MY_ABC_HERE
...
#ifdef MY_ABC_HERE
	ret = percpu_counter_init(&fs_info->eb_hit, 0);
	if (ret) {
		err = ret;
		goto fail;
	}

	ret = percpu_counter_init(&fs_info->eb_miss, 0);
	if (ret) {
		err = ret;
		goto fail;
	}

	ret = percpu_counter_init(&fs_info->meta_write_pages, 0);
	if (ret) {
		err = ret;
		goto fail;
	}

	ret = percpu_counter_init(&fs_info->data_write_pages, 0);
	if (ret) {
		err = ret;
		goto fail;
	}

	ret = percpu_counter_init(&fs_info->delayed_meta_ref, 0);
	if (ret) {
		err = ret;
		goto fail;
	}

	ret = percpu_counter_init(&fs_info->delayed_data_ref, 0);
	if (ret) {
		err = ret;
		goto fail;
	}

	ret = percpu_counter_init(&fs_info->write_flush, 0);
	if (ret) {
		err = ret;
		goto fail;
	}

	ret = percpu_counter_init(&fs_info->write_fua, 0);
	if (ret) {
		err = ret;
		goto fail;
	}
...
#ifdef MY_ABC_HERE
	tree_root = fs_info->tree_root = btrfs_alloc_root(fs_info, 0);
	chunk_root = fs_info->chunk_root = btrfs_alloc_root(fs_info, 0);
#else
	...
	if (features & BTRFS_FEATURE_COMPAT_SYNO_CASELESS) {
		if (syno_generation != generation) {
			btrfs_warn(fs_info, "Clear syno caseless feature, gen(%llu) != syno_gen(%llu), label:(%s)\n", generation, syno_generation, disk_super->label);
	...
	fs_info->delalloc_workers =
		btrfs_alloc_workqueue(fs_info, "delalloc", flags, max_active, 2);

	fs_info->flush_workers =
		btrfs_alloc_workqueue(fs_info, "flush_delalloc", flags, max_active, 0);
	...
	fs_info->endio_meta_write_workers =
		btrfs_alloc_workqueue(fs_info, "endio-meta-write", flags, max_active, 2);
	fs_info->endio_raid56_workers =
		btrfs_alloc_workqueue(fs_info, "endio-raid56", flags, max_active, 4);
	fs_info->rmw_workers =
	...
	fs_info->endio_freespace_worker =
		btrfs_alloc_workqueue(fs_info, "freespace-write", flags, max_active, 0);
	fs_info->delayed_workers =
		btrfs_alloc_workqueue(fs_info, "delayed-meta", flags, max_active, 0);
	...
	fs_info->syno_multiple_writeback_workers =
		btrfs_alloc_workqueue(fs_info, "syno_multiple_writeback", flags, max_active, 2);
#endif /* MY_ABC_HERE */
#ifdef MY_ABC_HERE
...
#ifdef MY_ABC_HERE
	if (!fs_info->usrquota_rescan_workers) {
		err = -ENOMEM;
		goto fail_sb_buffer;
	}
#endif
...
retry_root_backup:
...
#ifdef MY_ABC_HERE
	if (btrfs_test_opt(tree_root, NO_QUOTA_TREE))
		usrquota_root = ERR_PTR(-ENOENT);
	else
#endif /* MY_ABC_HERE */
	usrquota_root = btrfs_read_tree_root(tree_root, &location);
	if (!IS_ERR(usrquota_root)) {
		set_bit(BTRFS_ROOT_TRACK_DIRTY, &usrquota_root->state);
		fs_info->usrquota_enabled = 1;
	...
	if (btrfs_fs_compat(fs_info, BLOCK_GROUP_CACHE_TREE)) {
		...
		block_group_cache_root = btrfs_read_tree_root(tree_root, &location);
		if (IS_ERR(block_group_cache_root)) {
			ret = PTR_ERR(block_group_cache_root);
			goto recovery_tree_root;
		}
		set_bit(BTRFS_ROOT_TRACK_DIRTY, &block_group_cache_root->state);
	...
	syno_usage_root = btrfs_read_tree_root(tree_root, &location);
	if (!IS_ERR(syno_usage_root)) {
		set_bit(BTRFS_ROOT_TRACK_DIRTY, &syno_usage_root->state);
		fs_info->syno_usage_root = syno_usage_root;
	}

	location.objectid = BTRFS_SYNO_EXTENT_USAGE_TREE_OBJECTID;
	syno_extent_usage_root = btrfs_read_tree_root(tree_root, &location);
	if (!IS_ERR(syno_extent_usage_root)) {
		set_bit(BTRFS_ROOT_TRACK_DIRTY, &syno_extent_usage_root->state);
...
#ifdef MY_ABC_HERE
	ret = btrfs_debugfs_add_one(fs_info);
	if (ret) {
...
#ifdef MY_ABC_HERE
		log_tree_root = btrfs_alloc_root(fs_info, 0);
#else
...
#ifdef MY_ABC_HERE
	btrfs_usrquota_rescan_resume(fs_info);
#endif /* MY_ABC_HERE */
#ifdef MY_ABC_HERE
...
static int write_all_supers(struct btrfs_root *root, int max_mirrors)
{
	...
	list_for_each_entry_rcu(dev, head, dev_list) {
		...
		if (ret < 0) {
			...
			btrfs_error(root->fs_info, -EUCLEAN,
