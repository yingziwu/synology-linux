...
static int __btrfs_create_syno_feat_tree(struct btrfs_trans_handle *trans, struct btrfs_fs_info *fs_info)
{
	...
	syno_feat_root = btrfs_create_tree(trans, fs_info,
					    BTRFS_SYNO_FEATURE_TREE_OBJECTID);
	if (IS_ERR(syno_feat_root)) {
		ret = PTR_ERR(syno_feat_root);
		goto out;
	}
	fs_info->syno_feat_root = syno_feat_root;
	fs_info->syno_feat_root->block_rsv = &fs_info->global_block_rsv;
...
static int btrfs_create_syno_feat_tree(struct btrfs_fs_info *fs_info)
{
	...
	if (ret) {
		btrfs_err(fs_info, "BTRFS: failed to create syno feature tree, ret=[%d]", ret);
		goto out;
	}
	...
	if (ret) {
		btrfs_err(fs_info, "BTRFS: failed to update status of syno feature tree at the first time, ret=[%d]", ret);
...
int btrfs_syno_feat_tree_enable(struct btrfs_fs_info *fs_info)
{
	...
	if (fs_info->syno_feat_root) {
		btrfs_err(fs_info, "BTRFS: we are going to clean up syno feature tree because the status is not enabled");
		ret = btrfs_syno_feat_tree_disable(fs_info);
		if (ret) {
			btrfs_err(fs_info, "BTRFS: failed to disable and clean up syno feature tree, ret: [%d].", ret);
			goto out;
		}
	}
#endif /* MY_ABC_HERE */
...
static int btrfs_clear_syno_feat_tree(struct btrfs_fs_info *fs_info)
{
	...
	while (1) {
		...
		if (ret) {
			btrfs_err(root->fs_info, "BTRFS: failed to delete item of syno feature tree, ret: [%d].", ret);
			goto end_trans;
		}
		...
		if (syno_feat_tree_need_stop(root->fs_info)) {
			ret = -EAGAIN;
			btrfs_debug(root->fs_info, "BTRFS: drop tree early exit, ret: [%d].", ret);
			goto out;
		}
		if (fatal_signal_pending(current)) {
			btrfs_debug(root->fs_info, "BTRFS: catch interrupt singal as cleaning up syno feature tree.");
			ret = -EINTR;
			goto out;
		}
	}
...
static int btrfs_delete_syno_feat_tree_root(struct btrfs_fs_info *fs_info)
{
	...
	ret = btrfs_del_root(trans, tree_root, &syno_feat_root->root_key);
	if (ret) {
		btrfs_err(fs_info, "BTRFS: failed to delete the root item about syno feature tree, ret: [%d].", ret);
		goto abort;
	}
	...
	clean_tree_block(trans, tree_root->fs_info, syno_feat_root->node);
	btrfs_tree_unlock(syno_feat_root->node);
	btrfs_free_tree_block(trans, syno_feat_root, syno_feat_root->node, 0, 1);

	free_extent_buffer(syno_feat_root->node);
	free_extent_buffer(syno_feat_root->commit_root);
	kfree(syno_feat_root);
...
int btrfs_syno_feat_tree_disable(struct btrfs_fs_info *fs_info)
{
	...
	if (ret) {
		btrfs_err(fs_info, "BTRFS: failed to clean up syno feature tree, ret: [%d].", ret);
		goto out;
	}

	ret = btrfs_delete_syno_feat_tree_root(fs_info);
	if (ret) {
		btrfs_err(fs_info, "BTRFS: failed to delete syno feature tree root, ret: [%d].", ret);
		goto out;
	}
	btrfs_info(fs_info, "BTRFS: have finished to clean up syno feature tree, ret: [%d].", ret);
...
int btrfs_syno_feat_tree_status_update(struct btrfs_trans_handle *trans, struct btrfs_fs_info *fs_info)
{
	...
	extra_size = sizeof(struct btrfs_syno_feat_tree_status_item);

	ret = syno_feat_tree_update_helper(trans, root, path, &key, extra_size);
	if (ret < 0)
		goto out;
...
int btrfs_syno_feat_tree_load_status_from_disk(struct btrfs_fs_info *fs_info)
{
	...
	if (!fs_info->syno_feat_root) {
		btrfs_info(fs_info, "BTRFS: root of syno feature tree is null");
		return 0;
	}

	path = btrfs_alloc_path();
	if (!path) {
		btrfs_err(fs_info, "BTRFS: failed to allocate path, err: [%ld]", PTR_ERR(path));
		ret = -ENOMEM;
		goto out;
	}
	...
	if (ret < 0) {
		btrfs_err(fs_info, "BTRFS: failed to search slot on syno feature tree, ret: [%d].", ret);
		goto out;
	} else if (ret > 0) { /* tree empty */
		btrfs_warn(fs_info, "BTRFS: feature tree is empty.");
		ret = 0;
		goto out;
	}
	...
	case SYNO_FEAT_TREE_ST_DISABLE:
		btrfs_warn(fs_info, "BTRFS: disable status [%llx] has detected. We are going to clean up feautre tree.", fs_info->syno_feat_tree_status.status);
		break;
	default:
		btrfs_err(fs_info, "BTRFS: invalid status [%llx] of syno feature tree has detected.", fs_info->syno_feat_tree_status.status);
		break;
	}
