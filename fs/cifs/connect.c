...
static int cifs_logon_vet_description(const char *desc)
{
	char *p;
...
struct key_type cifs_key_type_logon = {
	.name			= "logon",
	...
	.vet_description	= cifs_logon_vet_description,
};
...
cifs_set_cifscreds(struct smb_vol *vol, struct cifs_ses *ses)
{
	...
	key = request_key(&cifs_key_type_logon, desc, "");
	if (IS_ERR(key)) {
		if (!ses->domainName) {
