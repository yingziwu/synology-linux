...
static inline int synoacl_op_arbit_chg_ok(struct dentry * dentry, unsigned int cmd, int tag, int mask)
{
	struct inode *inode = dentry->d_inode;

	if (inode->i_op->syno_arbit_chg_ok) {
		return inode->i_op->syno_arbit_chg_ok(dentry, cmd, tag, mask);
	}

	return synoacl_mod_archive_change_ok(dentry, cmd, tag, mask);
}
