...
static int ext4_feature_set_ok(struct super_block *sb, int readonly)
{
	...
	if (EXT4_HAS_RO_COMPAT_FEATURE(sb, ~EXT4_FEATURE_RO_COMPAT_SUPP)) {
		...
		if (EXT4_FEATURE_RO_COMPAT_METADATA_CSUM ==
			...
			&& is_syno_ext(sb)) {
			/* only metadata_csum volume created by DSM can be ignored */
...
no_journal:
	 ...
	 * we will drop i_projid and i_syno_archive_bit to avoid inode
	 * expand when upgrading.
	 */
...
void ext4_fill_mount_path(struct super_block *sb, char *szPath)
{
	...
	if (sbi->s_mount_path)
		kfree(sbi->s_mount_path);
	sbi->s_mount_path = kstrdup(szPath, GFP_KERNEL);
}
