...
#ifdef SYNO_RESIZE_INODE_SIZE_EXTEND
	blkShift = gdblock - 1;							// The original is "blkShift = (gdblock-1)/EXT4_ADDR_PER_BLOCK(sb);"
	blkShift = blkShift/EXT4_ADDR_PER_BLOCK(sb);	// but it will make arm/ppc compiler error.
	data = EXT4_I(inode)->i_data + EXT4_IND_BLOCK;
	if (le32_to_cpu(*data)) {
		blkShift = blkShift % le32_to_cpu(*data);
	} else {
		blkShift = 0;
	}
#endif
...
#ifdef SYNO_RESIZE_INODE_SIZE_EXTEND
	dind = sb_bread(sb, le32_to_cpu(*data)+blkShift);
#else
			      ...
			      ext4_group_t group)
{
	...
	blkShift = blk - 1;								// The original is "blkShift = (blk-1)/EXT4_ADDR_PER_BLOCK(sb);"
	blkShift = blkShift/EXT4_ADDR_PER_BLOCK(sb);	// but it will make arm/ppc compiler error.
	data = EXT4_I(inode)->i_data + EXT4_IND_BLOCK;
	if (le32_to_cpu(*data)) {
		blkShift = blkShift % le32_to_cpu(*data);
	...
	for (res = 0; res < reserved_gdb; res++, blk++) {
		...
		if (++data < end)
			continue;
		/* data is not in the block, try next block. */
		brelse(dind);
		blkShift++;
		data = EXT4_I(inode)->i_data + EXT4_DIND_BLOCK;
		dind = sb_bread(sb, le32_to_cpu(*data)+blkShift);
		...
		/* data is not in next block, try first block. */
		blkShift = 0;
		brelse(dind);
		data = EXT4_I(inode)->i_data + EXT4_DIND_BLOCK;
		dind = sb_bread(sb, le32_to_cpu(*data));
...
#ifdef SYNO_META_BG_PATCH
static void update_backups_meta(struct super_block *sb, int blk_off, char *data,
			   int size, int gdb_num)
{
	struct ext4_sb_info *sbi = EXT4_SB(sb);
	ext4_group_t last;
	ext4_group_t group = gdb_num;
	int rest = sb->s_blocksize - size;
	handle_t *handle;
	int err = 0, err2;

#ifdef SYNO_META_BG_BACKUP_DESC_FIX
	if (!EXT4_HAS_INCOMPAT_FEATURE(sb, EXT4_FEATURE_INCOMPAT_FLEX_BG)) {
		// FIXME: the backup desc of earlier fs without FLEX_BG is not suit for meta_bg. We skip it now. It will be fixed wh
		return;
	}
#endif
	handle = ext4_journal_start_sb(sb, EXT4_MAX_TRANS_DATA);
	...
	while (group < sbi->s_groups_count) {
		...
		backup_block = (u64)((u64)ext4_group_first_block_no(sb, group) +
				ext4_bg_has_super(sb, group));
			     ...
			     struct ext4_new_flex_group_data *flex_gd)
{
	...
	if ((u64)((u64)le32_to_cpu(es->s_free_inodes_count) + ((u64)EXT4_INODES_PER_GROUP(sb) * flex_gd->count)) < (u32)(~0U)) {
		le32_add_cpu(&es->s_free_inodes_count, EXT4_INODES_PER_GROUP(sb) *
				 flex_gd->count);
	} else {
		es->s_free_inodes_count = cpu_to_le32(~0U);
	}
#else
	le32_add_cpu(&es->s_inodes_count, EXT4_INODES_PER_GROUP(sb) *
		     flex_gd->count);
...
exit_journal:
	...
	if (!err) {
		...
		for (; gdb_num <= gdb_num_end; gdb_num++) {
			...
			gdb_bh = sbi->s_group_desc[gdb_num];
			if (meta_bg) {
				update_backups_meta(sb, gdb_bh->b_blocknr, gdb_bh->b_data,
						   gdb_bh->b_size, group);
			} else {
...
int ext4_group_add_no_flex(struct super_block *sb, struct ext4_new_group_data *input)
{
	...
	ext4_blocks_count_set(es, ext4_blocks_count(es) +
		input->blocks_count);
#ifdef SYNO_RESIZE_INODE_SIZE_EXTEND
	if ((u64)((u64)le32_to_cpu(es->s_inodes_count) + (u64)EXT4_INODES_PER_GROUP(sb)) < (u32)(~0U)) {
		le32_add_cpu(&es->s_inodes_count, EXT4_INODES_PER_GROUP(sb));
	} else {
		es->s_inodes_count = cpu_to_le32(~0U);
	}
...
#ifndef SYNO_RESIZE_INODE_SIZE_EXTEND
