			   ...
			   int size, int meta_bg)
{
	...
	while (group < sbi->s_groups_count) {
		...
		if (meta_bg == 0)
#ifdef MY_ABC_HERE
			backup_block = (sector_t)((u64)((u64)group * (u64)bpg) + blk_off);
#else
			       ...
			       struct ext4_new_flex_group_data *flex_gd)
{
	...
	if (!EXT4_HAS_INCOMPAT_FEATURE(sb, EXT4_FEATURE_INCOMPAT_FLEX_BG)) {
		credit = ext4_bg_has_super(sb, flex_gd->groups[0].group) ? 3 + reserved_gdb : 4;
	} else
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
	/*
	 * Don't check here because we will handle this later in updaing superblock
	 */
#else
...
#endif /* CONFIG_SYNO_EXT4_RESZIE_INODE_SIZE_EXTEND */
