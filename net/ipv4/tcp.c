...
#else /* MY_DEF_HERE && CONFIG_SPLICE_NET_DMA_SUPPORT */
		if ((available < target) &&
		    (len > sysctl_tcp_dma_copybreak) && !(flags & MSG_PEEK) &&
		    !sysctl_tcp_low_latency &&
		    net_dma_find_channel()) {
...
do_prequeue:
	...
	(defined(MY_DEF_HERE) && defined(CONFIG_SPLICE_NET_DMA_SUPPORT))
				if(msg->msg_flags & MSG_KERNSPACE)
					err = skb_copy_datagram_iovec1(skb, offset, msg->msg_iov, used);
				else
#endif /* MY_ABC_HERE || (MY_DEF_HERE && CONFIG_SPLICE_NET_DMA_SUPPORT)*/
