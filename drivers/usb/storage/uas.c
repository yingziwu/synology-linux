...
#define UAS_NO_ATA_PASS_THRU	(1 << 2)
#define UAS_NO_TEST_UNIT_READY	(1 << 3)
...
#endif
{
	...
	if (unlikely((cmnd->cmnd[0] == ATA_12 || cmnd->cmnd[0] == ATA_16) &&
		(devinfo->quirks & UAS_NO_ATA_PASS_THRU))) {
		...
		goto err_done;
	} else if ((cmnd->cmnd[0] == TEST_UNIT_READY) &&
		(devinfo->quirks & UAS_NO_TEST_UNIT_READY)) {
...
static struct usb_device_id uas_usb_ids[] = {
	{ USB_DEVICE_VER(0x1759, 0x5002, 0x2270, 0x2270), .driver_info = UAS_SENSE_IU_R02 | UAS_NO_TEST_UNIT_READY },
	{ USB_DEVICE_VER(0x174c, 0x55aa, 0x0100, 0x0100), .driver_info = UAS_SENSE_IU_R02 },
	{ USB_DEVICE_VER(0x05e3, 0x0733, 0x5405, 0x5405), .driver_info = UAS_NO_ATA_PASS_THRU },
...
static void uas_configure_endpoints(struct uas_dev_info *devinfo)
{
	...
	devinfo->num_streams = usb_alloc_streams(devinfo->intf, eps + 1, 3, 8,
								GFP_KERNEL);
...
static int uas_is_etron_hcd(struct usb_device *udev)
{
	struct usb_hcd *hcd = bus_to_hcd(udev->bus);
