...
static int rtk_usb_manager_probe(struct platform_device *pdev) {
	...
	if (node && of_device_is_available(node)) {
		...
		if (gpio_is_valid(gpio)) {
			...
			dev_dbg(dev, "port0-power-gpio no found");
		}
		...
		if (gpio_is_valid(gpio)) {
			...
			dev_dbg(dev, "port1-power-gpio no found");
		}
		...
		if (gpio_is_valid(gpio)) {
			...
			dev_dbg(dev, "port2-power-gpio no found");
		}
	...
	if (node && of_device_is_available(node)) {
		...
		if (gpio_is_valid(gpio)) {
			...
			dev_dbg(dev, "Error switch_ctrl-gpio no found");
