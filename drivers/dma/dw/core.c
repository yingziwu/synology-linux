...
void print_lli(struct dw_lli *lli)
{
        printk ("%s: \
                \n\tlli->sar=0x%x \
                \n\tlli->dar=0x%x \
                ...
                \n\tlli->sstat=0x%x \
                \n\tlli->dstat=0x%x \n", __func__, \
                lli->sar, lli->dar, lli->llp, lli->ctllo, lli->ctlhi, \
                lli->sstat, lli->dstat);
}

void dwc_special_regs_dump(struct dw_dma *dw)
{
	printk ("%s:%d: \
		\ndma_readl(dw, RAW.XFER)=0x%x \
		\ndma_readl(dw, RAW.BLOCK)=0x%x \
		\ndma_readl(dw, RAW.ERROR)=0x%x \
		\ndma_readl(dw, RAW.SRC_TRAN)=0x%x \
		\ndma_readl(dw, RAW.DST_TRAN)=0x%x \
		\ndma_readl(dw, MASK.XFER)=0x%x \
		\ndma_readl(dw, MASK.BLOCK)=0x%x \
		\ndma_readl(dw, MASK.ERROR)=0x%x \
		\ndma_readl(dw, MASK.SRC_TRAN)=0x%x \
		\ndma_readl(dw, MASK.DST_TRAN)=0x%x \
		\ndma_readl(dw, STATUS.XFER)=0x%x \
		\ndma_readl(dw, STATUS.BLOCK)=0x%x \
		\ndma_readl(dw, STATUS.ERROR)=0x%x \
		\ndma_readl(dw, STATUS.SRC_TRAN)=0x%x \
		\ndma_readl(dw, STATUS.DST_TRAN)=0x%x \n", __func__, __LINE__, \
		dma_readl(dw, RAW.XFER), dma_readl(dw, RAW.BLOCK), \
		dma_readl(dw, RAW.ERROR), dma_readl(dw, RAW.SRC_TRAN),\
		dma_readl(dw, RAW.DST_TRAN), dma_readl(dw, MASK.XFER),\
		dma_readl(dw, MASK.BLOCK), dma_readl(dw, MASK.ERROR), \
		dma_readl(dw, MASK.SRC_TRAN), dma_readl(dw, MASK.DST_TRAN), \
		dma_readl(dw, STATUS.XFER), dma_readl(dw, STATUS.BLOCK), \
		dma_readl(dw, STATUS.ERROR), dma_readl(dw, STATUS.SRC_TRAN), \
		dma_readl(dw, STATUS.DST_TRAN));
}

void dwc_ch_conf_dump(struct dw_dma_chan *dwc)
{
...
int dw_dma_probe(struct dw_dma_chip *chip, struct dw_dma_platform_data *pdata)
{
	...
	dw->clk = clk_get(chip->dev, "DUS");
	if (IS_ERR(dw->clk)){
		return PTR_ERR(dw->clk);
	}else{
		printk ("%s:%d: dw_dmac clk rate: %lu \n", __func__, __LINE__, \
				clk_get_rate(dw->clk));
	}
