...
static void rtk_sdmmc_init_card(struct mmc_host *host, struct mmc_card *card)
{
        host->card = card;
}
...
static int rtk_sdmmc_probe(struct platform_device *pdev)
{
	...
	if (prop) {
		...
		printk(KERN_ERR "[%s] get sdmmc-bounce value : %d \n",__func__, rtk_host->sdmmc_bounce_buf_val);
	} else {
		rtk_host->sdmmc_bounce_buf_val = 0;
		printk(KERN_ERR "[%s] assign sdmmc-bounce default value %d !!\n",__func__, rtk_host->sdmmc_bounce_buf_val);
	}

	rtk_host->sdmmc_gpio = devm_gpiod_get(&pdev->dev, "sd-power", GPIOD_OUT_LOW);
	if (IS_ERR(rtk_host->sdmmc_gpio)) {
		printk(KERN_ERR "%s: can't request gpio %d\n", __func__, desc_to_gpio(rtk_host->sdmmc_gpio));
		return -EINVAL;
	}

	rtk_host->sdmmc_wp_gpio = devm_gpiod_get(&pdev->dev, "sd-wp", GPIOD_IN);
	if (IS_ERR(rtk_host->sdmmc_wp_gpio)) {
		printk(KERN_ERR "%s: can't request gpio %d\n", __func__, desc_to_gpio(rtk_host->sdmmc_wp_gpio));
		return -EINVAL;
	}

	rtk_host->sdmmc_cd_gpio = devm_gpiod_get(&pdev->dev, "sd-cd", GPIOD_IN);
	if (IS_ERR(rtk_host->sdmmc_cd_gpio)) {
		printk(KERN_ERR "%s: can't request gpio %d\n", __func__, desc_to_gpio(rtk_host->sdmmc_cd_gpio));
		return -EINVAL;
	}
...
static int rtk_sdmmc_remove(struct platform_device *pdev)
{
	...
	if (mmc) {
		...
		if(rtk_host->sdmmc_bounce_buf_val==1) {
			printk(KERN_ERR "release SDMMC bounce buffer !!!\n");
