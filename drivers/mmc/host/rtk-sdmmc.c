...
static void rtk_sdmmc_pm_complete(struct device *dev)
{
	unsigned long flags2;
		...
		rtk_lockapi_lock(flags2, (char *)__FUNCTION__);    //lock 0x98012xxx for fear that emmc access followed by 98xx_x9xx or 98xxx8xx
...
static int rtk_sdmmc_pm_suspend(struct device *dev)
{
	...
	if(reginfo & SD_EXISTENCE) {
		...
		while(time_before(jiffies, timeout)){
			rtk_lockapi_lock(flags2, (char *)__FUNCTION__);
...
static void rtk_sdmmc_hw_initial(struct rtk_sdmmc_host *rtk_host)
{
    ...
    int bErrorRetry_1 = 0;
    unsigned long flags2;
    struct clk * clk_cr = NULL;
    struct clk * clk_sd_ip = NULL;
	...
	clk_sd_ip = clk_get(NULL, "clk_en_sd_ip");
...
static void rtk_sdmmc_timeout(unsigned long data)
{
    ...
    rtk_host = (struct rtk_sdmmc_host *)data;
    ...
    int_status = readl(rtk_host->sdmmc+CR_SD_ISR);
    int_status_en = readl(rtk_host->sdmmc+CR_SD_ISREN);
