...
static const struct intel_community cnlh_communities[] = {
	CNL_COMMUNITY(0, 0, 50),
	CNL_COMMUNITY(1, 51, 154),
	/*
	 * ACPI MMIO resources are returned in reverse order for
	 * communities 3 and 4.
	 */
	CNL_COMMUNITY(2, 155, 248),
	CNL_COMMUNITY(3, 249, 298),
};
...
static int __init cnl_pinctrl_init(void)
{
	return platform_driver_register(&cnl_pinctrl_driver);
}
subsys_initcall(cnl_pinctrl_init);

static void __exit cnl_pinctrl_exit(void)
{
	platform_driver_unregister(&cnl_pinctrl_driver);
}
module_exit(cnl_pinctrl_exit);
