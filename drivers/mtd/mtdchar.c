 ...
 * The updater should call syno_mtd_alloc() before doing system upgrade,
 ...
 *                 </ul>
 * 
 * @return Upon successful malloc, syno_mtd_alloc() return 0.  Otherwise
 *         -ENOMEM is returned.
 * @example <PRE>
 * if (syno_mtd_alloc(TRUE) == 0)
 * {
 *      system("dd if=zImage of=/dev/mtd1 bs=128k");
 *      syno_mtd_alloc(FALSE);
 * }
 * </PRE>
 * 
 * @see init_mtdchar
 ...
 * @see write_kbuf_sem
 */
int sys_syno_mtd_alloc(BOOL blMalloc)
{
    int retval = 0;
