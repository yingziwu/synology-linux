...
/*
 * efi-pstore will append part and count, rtk use idx.
 * libhwcontrol retrieve timestamp from id / 100000
 */
#define MAX_IDX 100000
#endif /* MY_DEF_HERE */

#if defined(MY_DEF_HERE)
static char mtddev[80] = "pstore";
#else /* MY_DEF_HERE */
...
#if defined(MY_DEF_HERE)
static unsigned long record_size = 12288;
#else /* MY_DEF_HERE */
static unsigned long record_size = 32768;
#endif /* MY_DEF_HERE */
module_param(record_size, ulong, 0400);
MODULE_PARM_DESC(record_size, "size of each dump record");
			    ...
			    bool * compressed, size_t * log_size)
{
	...
	*type = oops_hdr.type;
#if defined(MY_DEF_HERE)
	*id = ((u64) oops_hdr.timestamp * MAX_IDX) + i;
#else /* MY_DEF_HERE */
	*id = i;
#endif /* MY_DEF_HERE */
				   ...
				   struct pstore_info *psi)
{
	...
	*buf = kmalloc(log_size, GFP_KERNEL);
	...
	pr_info
	    ("rtksfc_pstore: read log from record %d in MTD partition \"%s\"\n",
	     rcd, mtddev);
				...
				size_t size, struct pstore_info *psi)
{
	...
	if (cxt->nextrcd >= cxt->oops_records) {
		pr_err("rtksfc_pstore: no storage space left\n");
		return -ENOSPC;
	}

	pr_info
	    ("rtksfc_pstore: write log to record %d in MTD partition \"%s\"\n",
	     cxt->nextrcd, mtddev);
				...
				int size)
{
	...
	if (ret) {
		...
		pr_warning
		    ("rtksfc_pstore: erase of region [0x%llx, 0x%llx] on \"%s\" failed\n",
				...
				struct timespec time, struct pstore_info *psi)
{
	...
	pr_info
	    ("rtksfc_pstore: remove log from record %llu in MTD partition \"%s\"\n",
	     id, mtddev);
#if defined(MY_DEF_HERE)
	rtk_sfc_erase_record(cxt, (id % MAX_IDX) * record_size, record_size);
#else /* MY_DEF_HERE */
...
static int __init rtk_sfc_pstore_init(void)
{
	...
	if (IS_ERR(mtd)) {
		pr_err("rtksfc_pstore: no MTD partition \"%s\" found\n",
		       mtddev);
		return -ENODEV;
	}

	pr_info
	    ("rtksfc_pstore: attach to MTD partition \"%s\" with size %llu and erasesize %u\n",
	...
	if ((record_size & (erasesize - 1)) != 0) {
		pr_err
		    ("rtksfc_pstore: record_size must be a multiple of erasesize %u\n",
		     mtd->erasesize);
		rc = -EINVAL;
		goto fail_device;
	}
	...
	pr_info("rtksfc_pstore: %d records of record_size %lu\n",
	...
	if (rc) {
		if (rc != -EPERM)
			pr_err("rtksfc_pstore: cannot register with pstore\n");
		goto fail_buf;
	}
