...
#ifdef MY_ABC_HERE
extern char* syno_eunit_ledtrig_name_get(int iEunitNum, int iLedNum);
...
static struct i2c_client *gpClient[SYNO_ATMEGA_NUM_MAX] = {NULL, NULL};
#endif
				...
				const struct i2c_device_id *id)
{
...
#ifdef MY_ABC_HERE
		eunit_index = syno_i2c_bus_to_eunit_index(pI2CNode);
#endif /* MY_ABC_HERE */
	} else {
	...
	for (i = 0; i < giSynoAtmegaNum; ++i) {
		if(cl->addr == gpClient[i]->addr) {
			device_index = i;
			break;
		}
	}
	if (device_index < 0) {
		dev_err(at->dev, "fail to get device index");
		goto END;
	}
#endif
	...
	for (i = 0 ; i < pdata->num_nodes ; i++) {
		...
		snprintf( pdata->node[i].name, 64, "syno_eunit%d_led%d", eunit_index, ((24 * device_index) + i));
		}
...
#ifdef MY_ABC_HERE
		pdata->node[i].default_trigger = syno_eunit_ledtrig_name_get(eunit_index, 24 * device_index + i);
#endif /* MY_ABC_HERE */
		}
        }
...
#ifdef MY_DEF_HERE
struct i2c_board_info __initdata LedI2CBoardInfo[SYNO_ATMEGA_NUM_MAX] = {
        {
                I2C_BOARD_INFO("atmega1608", 0x00),
                .platform_data = &syno_atmega1608_pdata,
        },
        {
                I2C_BOARD_INFO("atmega1608", 0x00),
                .platform_data = &syno_atmega1608_pdata,
...
static int __init atmega1608_init(void)
{
	int i = 0, iErr = -1;
	struct i2c_adapter *pAdapter = NULL;

	if (giSynoAtmegaNum > SYNO_ATMEGA_NUM_MAX) {
		printk(KERN_ERR "%s: invalid parameter giSynoAtmegaNum(%d) > SYNO_ATMEGA_NUM_MAX(%d)\n",
				__func__, giSynoAtmegaNum, SYNO_ATMEGA_NUM_MAX);
		goto END;
	}
	...
	for (i = 0; i < giSynoAtmegaNum; ++i) {
		/*regist board info*/
		LedI2CBoardInfo[i].addr = gSynoAtmegaAddr[i];
		gpClient[i] = i2c_new_device(pAdapter, &LedI2CBoardInfo[i]);
		if (gpClient[i] == NULL) {
			printk(KERN_ERR "%s: failed to initial device %d\n", __func__, i);
			goto END;
		}
	}
	iErr = i2c_add_driver(&atmega1608_driver);
...
module_init(atmega1608_init);

static void __exit atmega1608_exit(void)
{
	int i = 0;

	for (i = 0; i < giSynoAtmegaNum; ++i) {
		if (gpClient[i]) {
			i2c_unregister_device(gpClient[i]);
			gpClient[i] = NULL;
		}
	}
	i2c_del_driver(&atmega1608_driver);
}
module_exit(atmega1608_exit);
#else /* MY_DEF_HERE */
