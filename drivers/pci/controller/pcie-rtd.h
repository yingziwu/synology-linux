...
struct rtd_pcie_port {
	...
	raw_spinlock_t lock;
	DECLARE_BITMAP(irq_bitmap, MAX_RTK_MSI_IRQS);
	struct msi_controller msi_chip;
};

struct rtd_pcie_ops {
	char name[10];
	int (*get_resource)(struct rtd_pcie_port *pcie);
	int (*init)(struct rtd_pcie_port *pcie);
	int (*deinit)(struct rtd_pcie_port *pcie);
	int (*hwinit)(struct rtd_pcie_port *pcie);
	int (*msi_init)(struct rtd_pcie_port *pcie);
};
