...
int ve_pd_power_on(int id)
{
	...
	trace_rtk_pm_event(dev_name(data->pd_dev), "power_on");
...
int ve_pd_power_off(int id)
{
	...
	trace_rtk_pm_event(dev_name(data->pd_dev), "power_off_async");
...
static void ve_pd_init_pd_dev(struct device *dev, int index)
{
	...
	/* power off */
	trace_rtk_pm_event(dev_name(data->pd_dev), "request_power_off");
	pm_runtime_get_sync(data->pd_dev);
	pm_runtime_mark_last_busy(data->pd_dev);
	pm_runtime_put_autosuspend(data->pd_dev);
	trace_rtk_pm_event(dev_name(data->pd_dev), "request_power_off_completed");
}
