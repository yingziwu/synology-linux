...
int rtk_usb_init_port_power_on_off(struct device *usb_dev, bool on)
{
	...
	else if (!__usb_port_power_on_off(data->dev, &data->port1, on, usb_node))
		__usb_port_enable(data, &data->port1, usb_node, true);
	else if (!__usb_port_power_on_off(data->dev, &data->port2, on, usb_node))
		__usb_port_enable(data, &data->port2, usb_node, true);
	else if (!__usb_port_power_on_off(data->dev, &data->port3, on, usb_node))
