...
static int hse_hw_init(struct hse_device *hdev)
{
	...
	val = hse_should_disable_bypass_en(hdev) ? 0 : 1;
	hse_write(hdev, HSE_REG_BYPASS, val);
	return 0;
}
...
static int hse_runtime_suspend(struct device *dev)
{
	...
	reset_control_assert(hdev->rstc_bist);
...
static int hse_runtime_resume(struct device *dev)
{
	...
	reset_control_deassert(hdev->rstc_bist);
...
int hse_hw_reset(struct hse_device *hdev)
{
	if (!hdev->rstc) {
		dev_err(hdev->dev, "no reset control\n");
		return -EINVAL;
	}
	reset_control_reset(hdev->rstc);
	hse_hw_init(hdev);
	return 0;
}
...
static int hse_probe(struct platform_device *pdev)
{
	...
	hdev->dev = dev;
	hdev->quirks = of_device_get_match_data(dev);
	...
	hdev->rstc = devm_reset_control_get_optional(dev, "core");
	if (IS_ERR(hdev->rstc)) {
	...
	hdev->rstc_bist = devm_reset_control_get_optional(dev, "bist");
	if (IS_ERR(hdev->rstc_bist)) {
		ret = PTR_ERR(hdev->rstc_bist);
		dev_dbg(dev, "failed to get reset control: %d\n", ret);
		hdev->rstc_bist = NULL;
	}
...
static const struct hse_quirks rtd1619b_quirks = { 1 };

static const struct of_device_id hse_ids[] = {
	{ .compatible = "realtek,highspeed-streaming-engine" },
	{ .compatible = "realtek,rtd1619b-highspeed-streaming-engine", .data =  &rtd1619b_quirks, },
	{}
};
