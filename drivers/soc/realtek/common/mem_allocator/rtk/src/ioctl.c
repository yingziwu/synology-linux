					 ...
					 *range_data)
{
	...
	if (!ion_flag_is_protected(buffer->flags) &&
		...
		if (ion_rtk_get_phys(handle, &addr, &len) == 0) {
			...
			dma_addr_t paddr = dma_map_page(buffer->dev->dev.this_device,
							sg_page(buffer->sg_table->sgl),
							offset,
							range_data->len, dir);
			dma_sync_single_for_device(buffer->dev->dev.this_device, range_data->phyAddr,
						   range_data->len, dir);
			dma_unmap_page(buffer->dev->dev.this_device, paddr, range_data->len, dir);
		}
	}
				   ...
				   struct ion_rtk_carveout_meminfo *info)
{
	...
	for (i = 0; i < 32; i++) {
		...
		heap =
		    ion_get_client_heap_by_mask(buffer->dev,
						target_heap_id_mask);
		if (heap == NULL)
			continue;
