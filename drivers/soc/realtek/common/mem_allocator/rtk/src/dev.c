...
static unsigned int rtk_ion_cma_extFlags(struct device_node *node,
					 struct cma *cma)
{
	unsigned long pool_base = cma_get_base(cma);
	size_t pool_size = cma_get_size(cma);
	unsigned int ret = 0;
	const u32 *prop;
	int size = 0;
	int i;
						  ...
						  *dt_node)
{
	...
	pdata->heaps =
	    kcalloc(num_heaps, sizeof(struct ion_platform_heap), GFP_KERNEL);
	if (!pdata->heaps) {
		ret = -ENOMEM;
		goto free_pdata;
	}
	...
	for_each_child_of_node(dt_node, node) {
		...
		if (use_cma_pools) {
			for (i = 0; i < cma_area_count; i += 1) {
				...
				if (skip_dma_default_array
				    && cma == dma_contiguous_default_area) {
					dev_err(idev,
						"do not add dma_contiguous_default_area to the pool list. (cma_area_count=%d)\n",
						cma_area_count);
					continue;
				}

				if (cma_get_size(cma) == 0)
					continue;
...
static struct ion_heap_data *rtk_ion_get_heaps_buffer(size_t * cnt)
{
	static size_t ion_heap_cnt = 0;
	static struct ion_heap_data *buffer = NULL;
	do {
		static struct ion_heap_query query;
		mm_segment_t oldfs;

		memset(&query, 0, sizeof(struct ion_heap_query));

		if (ion_query_heaps(&query) != 0 || query.cnt == 0) {
			pr_err("%s ion_query_heaps error!\n", __func__);
			break;
		}

		if (buffer && query.cnt == ion_heap_cnt)
			break;
		...
		buffer =
		    kzalloc(sizeof(struct ion_heap_data) * query.cnt,
			    GFP_KERNEL);
		...
		query.heaps = (uintptr_t) buffer;

		if (ion_query_heaps(&query) != 0) {
			kfree(buffer);
			set_fs(oldfs);
			buffer = NULL;
			break;
...
unsigned int rtk_ion_heap_id_mask_tr(unsigned int heap_type_mask)
{
	...
	do {
		size_t i;
		size_t heap_cnt = 0;
		struct ion_heap_data *buffer =
		    rtk_ion_get_heaps_buffer(&heap_cnt);
