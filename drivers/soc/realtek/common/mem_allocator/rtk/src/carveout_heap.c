...
unsigned int retry_count_value = 50;
unsigned int retry_delay_value = 300;
...
static int ion_rtk_heap_map_user(struct ion_heap *heap,
				 struct ion_buffer *buffer,
				 struct vm_area_struct *vma);
static void *ion_rtk_heap_map_kernel(struct ion_heap *heap,
				     struct ion_buffer *buffer);
static struct ion_heap_ops rtk_carveout_heap_ops = {
	.allocate = ion_rtk_carveout_heap_allocate,
	.free = ion_rtk_carveout_heap_free,
	.map_user = ion_rtk_heap_map_user,
	.map_kernel = ion_rtk_heap_map_kernel,
				       ...
				       struct seq_file *s)
{
	...
	struct ion_rtk_carveout_heap *rtk_carveout_heap =
	    container_of(heap, struct ion_rtk_carveout_heap, heap);
	mutex_lock(&rtk_carveout_heap->lock);
	ret = seq_buf_print_seq(s, &rtk_carveout_heap->trace_seq_buf);
	mutex_unlock(&rtk_carveout_heap->lock);
	return ret;
...
retry:
			...
			if (page_info == NULL && retry_count < retry_count_value && rtk_flag_is_protected_dynamic(flags)) {
				msleep(retry_delay_value-100*(retry_count%3));
				retry_count++;
				goto retry;
			}
			if (retry_count > 0)
				pr_err("%s: ion cma_alloc page_info = 0x%p, retry_delay = %d, count = %d, MAX_COUNT = %d\n",
					__func__, page_info, retry_delay_value, retry_count, retry_count_value);
		} while (0);
	...
	if (page_info) {
		if (ion_flag_canAccess(page_info->flags)) {
			...
			if(!!(buffer->flags & ION_FLAG_CACHED))
				page_info->sync(buffer->dev->dev.this_device, page_info, DMA_TO_DEVICE);
		}
					...
					struct seq_file *s, void *prefix)
{
	...
	do {
		...
		ret =
		    ion_carveout_heap_dump(rtk_carveout_heap, &seq_buf, prefix);

		seq_buf_print_seq(s, &seq_buf);
...
int ion_rtk_heap_map_user(struct ion_heap *heap, struct ion_buffer *buffer,
			  struct vm_area_struct *vma)
{
...
void *ion_rtk_heap_map_kernel(struct ion_heap *heap, struct ion_buffer *buffer)
{
	struct scatterlist *sg;
	int i, j;
	void *vaddr;
