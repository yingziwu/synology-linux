...
static int __init buflock_module_init (void)
{
    ...
    do {
        ...
        do {
            ...
            module->ion_handle = ion_alloc(sizeof(unsigned int) * module->max_handles,
            ...
            module->dmabuf = dma_buf_get(module->ion_handle);
            if (IS_ERR(module->dmabuf)) {
                ret = PTR_ERR(module->dmabuf);
                break;
            }

            __close_fd(current->files, module->ion_handle);
