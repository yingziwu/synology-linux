...
#else
#define RPC_INTR_VE3_DEV_TOTAL 0
#endif
...
static inline void peek_rpc_struct(const char *func, RPC_DEV *dev, loff_t num)
{
	RPC_STRUCT rpc;
	uint32_t pid;
	uint32_t arg;
	...
	if (rpc.programID == REPLYID && rpc.versionID == REPLYID)
		trace_rtk_rpc_peek_rpc_reply((struct rpc_struct_tp *)&rpc,
				(u32)refclk_get_counter(refclk), num, pid);
	else
		trace_rtk_rpc_peek_rpc_request((struct rpc_struct_tp *)&rpc,
				(u32)refclk_get_counter(refclk), num, rpc.taskID);
}
