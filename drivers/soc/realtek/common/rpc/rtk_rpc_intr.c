...
phys_addr_t rtk_rpc_ion_pa(struct ion_buffer *buffer)
{
	...
	do {
		...
		if (handle < 0) {
			pr_err("[%s] ERROR: ion_alloc fail %d(heap:0x%x flags:0x%x)\n", __func__,
					handle, ion_alloc_heap_mask, ion_alloc_flags);
			reply_value = -1U;
			break;
		}
...
unsigned int rpc_ion_alloc_handler(RPC_DEV_EXTRA *extra, const struct fw_alloc_parameter * param)
{
	...
	do {
		...
		handle = ion_alloc(alloc_size, ion_alloc_heap_mask, ion_alloc_flags);

		if ((handle < 0 ) && (ion_alloc_flags & ION_FLAG_CMA)) {
...
void rpc_ion_handler(RPC_DEV_EXTRA *extra)
{
	do {
		...
		peek_rpc_struct(__func__, extra->dev, 0);

		if (r_program_read(extra, extra->dev, (char *) rpc, sizeof(RPC_STRUCT)) != sizeof(RPC_STRUCT)) {
			pr_err("%s:%d\n", __func__, __LINE__);
			break;
		}
		...
		switch (remote_cmd) {
			...
			case RPC_REMOTE_CMD_ALLOC:
				if (rpc->parameterSize == sizeof(struct fw_alloc_parameter_legacy)) {
					...
					if (r_program_read(extra, extra->dev, (char *) &param, sizeof(param)) != sizeof(param)) {
...

		{
			...
			trace_rtk_rpc_peek_rpc_reply((struct rpc_struct_tp *)&reply_rpc.header,
					(u32)refclk_get_counter(refclk), 0, rpc->taskID);
...
void ignore_rpc(RPC_DEV_EXTRA *extra, uint32_t data_size)
{
	...
	dev->ringOut = dev->ringOut + sizeof(RPC_STRUCT) + data_size;
	if (dev->ringOut >= dev->ringEnd)
		dev->ringOut = dev->ringStart + (dev->ringOut - dev->ringEnd);
}
...
int update_rpc_release_lists(int type)
{
	...
	handle = ion_alloc(4096, RTK_ION_HEAP_AUDIO_MASK, AUDIO_ION_FLAG);
	if (handle < 0) {
	...
	release_entry->rpc_dmabuf = dma_buf_get(handle);

	__close_fd(current->files, handle);

	release_entry->paddr = rtk_rpc_ion_pa(release_entry->rpc_dmabuf->priv);
	if (release_entry->paddr == -1) {
		pr_err("[%s] get paddr failed\n", __func__);
		ret = -EINVAL;
		goto put_dma_buf;
	}

	release_entry->vaddr = rtk_rpc_ion_va(release_entry->rpc_dmabuf);
	if (!release_entry->vaddr) {
...
int RPC_DESTROY_VIDEO_FLOW(int pid, phys_addr_t paddr, void *vaddr)
{
	...
	memset(pid_info, 0, sizeof(uint32_t));
	*pid_info =  htonl(pid);
...
static int rpc_intr_flush(struct file *filp, fl_owner_t id)
{
	...
	if (rpc_intr_is_paused) {
		...
		task = find_task_by_pid_ns(proc->pid, &init_pid_ns);
	...
	if (rpc_release_entry!=NULL) {
		dma_buf_end_cpu_access(rpc_release_entry->rpc_dmabuf, DMA_BIDIRECTIONAL);
