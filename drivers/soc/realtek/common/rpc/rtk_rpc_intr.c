...
void rpc_ion_handler(RPC_DEV_EXTRA *extra)
{
	...
	if (ntohl(rpc->procedureID) == 1) { /*allocate*/
		...
		if (IS_ERR(handle)) {
			pr_err("[%s] ERROR: ion_alloc fail %p\n", __func__, (int *) handle);
		}
		...
		if (IS_ERR(handle)) {
			pr_err("[%s] ERROR: secure ion_alloc fail %p\n", __func__, (int *) handle);
		}
		if (ion_phys(fw_rpc_ion_client, handle, &phys_addr, &alloc_val) != 0) {
...
int rpc_destroy_flow(int pid)
{
	...
	handle = ion_alloc(fw_rpc_ion_client, 4096, 1024, RTK_PHOENIX_ION_HEAP_AUDIO_MASK, ION_FLAG_NONCACHED |ION_FLAG_SCPUACC | ION_FLAG_ACPUACC);
	...
	if (ion_phys(fw_rpc_ion_client, handle, &dat, &len) != 0) {
		pr_err("[malloc fail %s]\n", __FUNCTION__);
		goto exit;
	}

	cmd = ion_map_kernel(fw_rpc_ion_client, handle);
...
exit:
	if(handle != NULL) {
		ion_unmap_kernel(fw_rpc_ion_client, handle);
		ion_free(fw_rpc_ion_client, handle);
	}
	return RPC_ret;
}
