...
(struct r5conf *conf, struct bio *bio, struct syno_self_heal_stripe_head *sh, int disk_idx, sector_t sh_sector)
{
	...
	if (NULL == r5_bio) {
		pr_err("%s: [Self Heal] Failed to allocate memory for retry read at bio sector [%llu], sh-sector [%llu]\n",
				mdname(conf->mddev), (u64)bio->bi_sector, (u64)sh_sector);
		return NULL;
	}
...
static  void SYNORaid5CheckPage(struct stripe_head *sh)
{
	...
	for (i = disks;i--;) {
		// If data uses fs page, put them and point page back to rdev's own.
...
static void syno_record_syno_stat_to_conf(struct r5conf *conf, struct stripe_head *sh)
{
	...
	if (unlikely(test_bit(STRIPE_RECORDED, &sh->state))) {
		...
		if (sh->syno_stat_handle_stripe_overhead > conf->syno_stat_handle_stripe_max_overhead)
			conf->syno_stat_handle_stripe_max_overhead = sh->syno_stat_handle_stripe_overhead;
		if (sh->syno_stat_raid_run_ops_overhead > conf->syno_stat_raid_run_ops_max_overhead)
			conf->syno_stat_raid_run_ops_max_overhead = sh->syno_stat_raid_run_ops_overhead;
		if (sh->syno_stat_bio_fill_drain_overhead > conf->syno_stat_bio_fill_drain_max_overhead)
			conf->syno_stat_bio_fill_drain_max_overhead = sh->syno_stat_bio_fill_drain_overhead;
		if (sh->syno_stat_raid_run_ops_overhead - sh->syno_stat_bio_fill_drain_overhead > conf->syno_stat_other_raid_ops_max_overhead)
			conf->syno_stat_other_raid_ops_max_overhead = sh->syno_stat_raid_run_ops_overhead - sh->syno_stat_bio_fill_drain_overhead;
	}

	conf->syno_stat_sh_overhead += syno_stat_sh_overhead;
	conf->syno_stat_delay_overhead += sh->syno_stat_delay_overhead;
	conf->syno_stat_io_overhead += sh->syno_stat_io_overhead;
...
static void mark_target_uptodate(struct stripe_head *sh, int target)
{
	...
	tgt = &sh->dev[target];
#if defined(MY_DEF_HERE)
	/* when degraded and being forced RCW, there is chance to race setting
	 * up R5_UPTODATE flag between computing xor and draining bio. We keep
	 * it exclusive with R5_SkipCopy
	 */
			...
			while (wbi && wbi->bi_sector <
				...
				if (wbi->bi_rw & REQ_DISCARD)
					...
					if (!test_bit(STRIPE_SYNCING, &sh->state) &&
						...
						writebio)
					{
						cur_bi_sector = wbi->bi_sector;
						for (bi_vec_idx = 0; bi_vec_idx < wbi->bi_vcnt; bi_vec_idx++) {
					...
					if (test_and_clear_bit(R5_UseFSPage, &dev->flags)) {
						restore_r5page(dev->r5page, dev->page);
						put_page(dev->page);
						dev->vec.bv_page = dev->page = dev->r5page;
						dev->rvec.bv_page = dev->vec.bv_page;
					}
	...
	case check_state_check_result:
		...
		if ((sh->ops.zero_sum_result & SUM_CHECK_P_RESULT) == 0)
			...
			if (MD_SYNC_DEBUG_ON == conf->mddev->sync_debug) {
				printk(KERN_ERR "md/raid5:%s: raid5 not sync in sector: %llu, size: %u\n", mdname(conf->mddev), (u64) sh->sector, STRIPE_SECTORS);
			}
#endif /* MY_ABC_HERE */
...
#ifdef MY_ABC_HERE
			if (MD_SYNC_DEBUG_ON == conf->mddev->sync_debug) {
				printk(KERN_ERR "md/raid6:%s: raid6 not sync in sector: %llu, size: %u\n", mdname(conf->mddev), (u64) sh->sector, STRIPE_SECTORS);
			}
#endif /* MY_ABC_HERE */
			atomic64_add(STRIPE_SECTORS, &conf->mddev->resync_mismatches);
			if (test_bit(MD_RECOVERY_CHECK, &conf->mddev->recovery))
	...
	if (!uptodate) {
		if (IsDeviceDisappear(rdev->bdev)) {
			...
			sector_t report_sector = raid5_compute_sector(conf,
				raid_bi->bi_sector, 0, &dd_idx, NULL) + rdev->data_offset;

			SynoReportBadSector(report_sector, READ,
...
static void syno_do_full_stripe_merge(struct mddev *mddev, struct stripe_head *sh)
{
	...
	for (i = 0; i < chunk_sectors; i += STRIPE_SECTORS) {
		sh1 = get_active_stripe(conf, stripe_addr+i, 0, 1, 0);
		if (!sh1) {
			break;
		}
...
static int syno_raid6_self_heal_set_src_pages(struct r5conf *conf, struct syno_self_heal_stripe_head *sh, struct raid5_percpu *percpu, int disk_idx, struct page *dst_page)
{
	...
	struct page **src_page = percpu->scribble;
...
static void syno_raid5_self_heal_compute_data_complete(void *bio_clone_ref)
{
	...
	if (bio_clone->bi_sector < master_bio->bi_sector) {
		src_page_offset = (master_bio->bi_sector - bio_clone->bi_sector) * 512;
		dst_page_offset = 0;
	} else if (bio_clone->bi_sector > master_bio->bi_sector) {
		src_page_offset = 0;
		dst_page_offset = (bio_clone->bi_sector - master_bio->bi_sector) * 512;
	}
		...
		if (0 != syno_self_heal_record_hash_value(heal_record, master_bio)) {
			if (heal_record->retry_cnt <= max_retry_cnt) {
				pr_err("%s: [Self Heal] Retry sector [%llu] round [%d/%d] finished: get same result, retry next round\n",
						mdname(mddev), (u64)master_bio->bi_sector, heal_record->retry_cnt - 1, max_retry_cnt);
			...
			pr_err("%s: [Self Heal] Retry sector [%llu] round [%d/%d] finished: return result to upper layer\n",
					mdname(mddev), (u64)master_bio->bi_sector, heal_record->retry_cnt, max_retry_cnt);
...
static void syno_raid5_self_heal_compute_data(struct r5conf *conf, struct syno_self_heal_stripe_head *sh, struct md_self_heal_record *heal_record, int disk_idx, struct bio *bio_clone)
{
	...
	if (2 == retry_cnt) {
		pr_err("%s: [Self Heal] Retry sector [%llu] round [%d/%d] choose p-disk\n",
				mdname(mddev), (u64)master_bio->bi_sector, retry_cnt, max_retry_cnt);
		init_async_submit(&submit, ASYNC_TX_FENCE|ASYNC_TX_XOR_ZERO_DST, NULL, syno_raid5_self_heal_compute_data_complete,
				bio_clone, percpu->scribble + sizeof(struct page *) * (disks + 2));
...
static void syno_raid5_self_heal_bio_submit_end_request(struct bio *bio_submit, int error)
{
	int uptodate = test_bit(BIO_UPTODATE, &bio_submit->bi_flags);
...
static void syno_raid5_self_heal_retry_read(struct r5conf *conf, struct bio *master_bio, int bl_should_init)
{
	...
	logical_sector = master_bio->bi_sector & ~((sector_t)STRIPE_SECTORS - 1);
	...
	for (; logical_sector < last_sector; logical_sector += STRIPE_SECTORS) {
		...
		bio_clone->bi_sector = logical_sector;
...
used_data_correction_resource_show(struct mddev *mddev, char *page)
{
	...
	if (conf) {
		...
		if (!spin_trylock(&conf->syno_self_heal_master_bio_list_lock)) {
			pr_err("%s: [Self Heal] Failed to get retry master bio lock\n", mdname(mddev));
		} else {
			...
			while (remain_bio) {
				pr_err("%s: [Self Heal] retry_bio: bio(%d:%p) at sector [%llu] length [%llu]\n",
						mdname(mddev), cnt_retry_bio++, remain_bio, (u64)remain_bio->bi_sector, (u64)bio_sectors(remain_bio));
...
raid5_show_syno_stat(struct mddev  *mddev, char *page)
{
	...
	if (conf) {
		return sprintf(page, "%llu %llu %llu %llu %llu\n%llu %llu %llu %llu\n%llu %llu %llu\n%llu %llu\n%llu %llu %llu\n%llu %llu %llu\n%llu %llu %llu\n",
				...
				conf->syno_stat_total_stripe_cnt,
				conf->syno_stat_total_stripe_cnt,	//Align with 4.4.x, the handle stripe cnt is same as total stripe cnt in 3.10 kernel
				conf->syno_stat_full_write_stripe_cnt,
				conf->syno_stat_rmw_cnt,
				conf->syno_stat_rcw_cnt,
