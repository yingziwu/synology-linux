...
#define AL_ETH_KR_LT_FAILURES_TO_RESET	(10)
#endif
...
static struct {
	...
	{ "AnnapurnaLabs unified 1Gbe/10Gbe pcie NIC" },
	};
...
al_eth_function_reset(struct al_eth_adapter *adapter)
{
	...
	if (adapter->board_type == ALPINE_NIC) {
		adapter_base = adapter->udma_base - 128 * 0x400;

		rc = al_eth_flr_rmn(&al_eth_fpga_read_pci_config,
				&al_eth_fpga_write_pci_config,
				adapter_base, adapter->mac_base);
//		writel(0x80, adapter_base + 0x93); /* enable MSI-X in the adapter */
	} else {
...
al_eth_board_params_init(struct al_eth_adapter *adapter)
{
	...
	if (rc) { /* if params are not initialized, use the following default */
		adapter->ref_clk_freq = AL_ETH_REF_FREQ_375_MHZ;
		adapter->mdio_freq = AL_ETH_DEFAULT_MDIO_FREQ_KHZ;

		switch (PCI_SLOT(adapter->pdev->devfn)) {
		case 0:
		case 2:
			adapter->mac_mode = AL_ETH_MAC_MODE_10GbE_Serial;
			adapter->sfp_detection_needed = false;
...
al_eth_flow_ctrl_config(struct al_eth_adapter *adapter)
{
	...
	flow_ctrl_params->prio_q_map[0][0] = 0x0;
...
#ifdef CONFIG_AL_ETH_ALLOC_PAGE
		if (unlikely(al_eth_alloc_rx_page(adapter, rx_info, __GFP_COLD | GFP_ATOMIC | __GFP_COMP) < 0)) {
#elif defined(CONFIG_AL_ETH_ALLOC_FRAG)
...
//Do nothing
#else
	adapter->last_link_status.link_up = false;
	adapter->lt_completed = false;
#endif
...
#else
/*
 * Configure tx and rx serdes parameters to basic values
 *	For DA - these parameters used for fall-back in case link training
 *		 failed or not supported by the link partner
 *	For optic SFP - used as static configuration.
 */
void al_eth_serdes_da_tx_basic_config(struct al_serdes_obj	*serdes_obj,
				      enum al_serdes_group	grp,
				      enum al_serdes_lane	lane)
{
	struct al_serdes_adv_tx_params basic_tx;

	basic_tx.override		= AL_TRUE;
	basic_tx.amp			= 0x7;
	basic_tx.total_driver_units	= 0x19;
	basic_tx.c_plus_1		= 0x5;
	basic_tx.c_plus_2		= 0;
	basic_tx.c_minus_1		= 0x2;
	basic_tx.slew_rate		= 0;

	al_serdes_tx_advanced_params_set(serdes_obj, grp, lane, &basic_tx);
}

void al_eth_serdes_da_rx_basic_config(struct al_serdes_obj	*serdes_obj,
				      enum al_serdes_group	grp,
				      enum al_serdes_lane	lane)
{
	...
	basic_rx.dfe_gain		= 0x0;
	basic_rx.dfe_first_tap_ctrl	= 0x4;
	basic_rx.dfe_secound_tap_ctrl	= 0x8;
	basic_rx.dfe_third_tap_ctrl	= 0xa;
	basic_rx.dfe_fourth_tap_ctrl	= 0x0;
	basic_rx.low_freq_agc_gain	= 0x7;
	basic_rx.precal_code_sel	= 0;
	basic_rx.high_freq_agc_boost	= 0x1b;
#else
	basic_rx.dcgain			= 0x4;
	basic_rx.dfe_3db_freq		= 0x4;
	basic_rx.dfe_gain		= 0x3;
	basic_rx.dfe_first_tap_ctrl	= 0x5;
	basic_rx.dfe_secound_tap_ctrl	= 0x1;
	basic_rx.dfe_third_tap_ctrl	= 0x8;
	basic_rx.dfe_fourth_tap_ctrl	= 0x1;
	basic_rx.low_freq_agc_gain	= 0x7;
	basic_rx.precal_code_sel	= 0;
	basic_rx.high_freq_agc_boost	= 0x1d;
#endif

	al_serdes_rx_advanced_params_set(serdes_obj, grp, lane, &basic_rx);
}

void al_eth_serdes_optic_tx_basic_config(struct al_serdes_obj	*serdes_obj,
					 enum al_serdes_group	grp,
					 enum al_serdes_lane	lane)
{
	struct al_serdes_adv_tx_params basic_tx;
	...
	basic_tx.override		= AL_TRUE;
	basic_tx.amp			= 0x1;
	basic_tx.total_driver_units	= 0x13;
	basic_tx.c_plus_1		= 0x2;
	basic_tx.c_plus_2		= 0;
	basic_tx.c_minus_1		= 0;
	basic_tx.slew_rate		= 0;

	al_serdes_tx_advanced_params_set(serdes_obj, grp, lane, &basic_tx);
}

void al_eth_serdes_optic_rx_basic_config(struct al_serdes_obj	*serdes_obj,
					 enum al_serdes_group	grp,
					 enum al_serdes_lane	lane)
{
	struct al_serdes_adv_rx_params basic_rx;
	...
	basic_rx.dfe_3db_freq		= 0x7;
	basic_rx.dfe_gain		= 0x0;
	basic_rx.dfe_first_tap_ctrl	= 0x0;
	basic_rx.dfe_secound_tap_ctrl	= 0x8;
	basic_rx.dfe_third_tap_ctrl	= 0x0;
	basic_rx.dfe_fourth_tap_ctrl	= 0x8;
	basic_rx.low_freq_agc_gain	= 0x7;
	basic_rx.precal_code_sel	= 0;
	basic_rx.high_freq_agc_boost	= 0x4;
...
static void al_eth_kr_link_status(struct al_eth_adapter *adapter)
{
	int ret;
	...
	if (adapter->serdes_init == false) {
		netdev_err(adapter->netdev, "%s serdes wasn't initialized - "
					    "can't run LT\n", __func__);
		return;
	}
	...
	/* link status lost */
	if ((adapter->last_link_status.link_up == true) &&
	    (status.link_up == false)) {
		netdev_dbg(adapter->netdev, "%s link status lost\n", __func__);
		netif_carrier_off(adapter->netdev);

		/* if link status lost lt is needed again */
		adapter->lt_completed = false;
	}

	adapter->last_link_status = status;

	if (adapter->lt_completed == false) {
		...
		netdev_dbg(adapter->netdev, "%s start an and lt\n", __func__);

		an_adv.selector_field = 1;
		an_adv.capability = 0;
		an_adv.remote_fault = 0;
		an_adv.acknowledge = 0;
		an_adv.next_page = 0;
		get_random_bytes(&an_adv.transmitted_nonce, 1);
		an_adv.transmitted_nonce &= 0x1f;
		an_adv.technology = AL_ETH_AN_TECH_10GBASE_KR;
		/* TODO: Add fec support */
		an_adv.fec_capability = 0;

		ret = al_eth_an_lt_execute(&adapter->hal_adapter,
					   &adapter->serdes_obj,
					   adapter->serdes_grp,
					   adapter->serdes_lane,
					   &an_adv,
		...
		if (ret == 0) {
			netdev_dbg(adapter->netdev, "%s lt finished successfully"
				   " call cerrier on\n", __func__);

			adapter->lt_completed = true;
			adapter->last_link_status.link_up = true;
			adapter->lt_failures = 0;
			netif_carrier_on(adapter->netdev);
		} else {
			adapter->lt_failures++;
			if (adapter->lt_failures > AL_ETH_KR_LT_FAILURES_TO_RESET) {
				netdev_warn(adapter->netdev,
					    "%s: failed to establish LT %d times. reset serdes\n",
					    __func__, AL_ETH_KR_LT_FAILURES_TO_RESET);
				al_serdes_pma_hard_reset_lane(&adapter->serdes_obj,
							       adapter->serdes_grp,
							       adapter->serdes_lane,
							       AL_TRUE);
				al_serdes_pma_hard_reset_lane(&adapter->serdes_obj,
			...
			/* In case auto-negotiation or link training failed -
			 * configure basic values to be able to work with
			 * partners that doesn't implement link training */
			al_eth_serdes_da_rx_basic_config(&adapter->serdes_obj,
							 adapter->serdes_grp,
							 adapter->serdes_lane);
			al_eth_serdes_da_tx_basic_config(&adapter->serdes_obj,
							 adapter->serdes_grp,
							 adapter->serdes_lane);

			/* After setting basic values check for link. if no
			 * remote or local fault detected, indicate link up */
			al_eth_link_status_get(&adapter->hal_adapter, &status);

			if (adapter->last_link_status.link_up == true) {
				netdev_info(adapter->netdev,
					   "link training failed - use basic configuration\n");
				adapter->lt_completed = true;
				netif_carrier_on(adapter->netdev);
			}
		}
...
#define SFP_10G_DA_ACTIVE		0x4
#define SFP_10G_DA_PASSIVE		0x8

static int al_eth_i2c_byte_read(struct al_eth_adapter *adapter,
				uint8_t addr,
				uint8_t *val)
#endif
{
...
#else
	i2c_adapter = i2c_get_adapter(adapter->i2c_adapter_id);
#endif
...
#else // CONFIG_SYNO_ALPINE_A0
static int al_eth_sfp_detect(
	struct al_eth_adapter		*adapter,
	enum al_eth_sfp_detect_evt	*evt)
{
	int rc = 0;
	uint8_t sfp_10g;
	uint8_t sfp_1g;
	uint8_t sfp_cable_tech;
	struct al_eth_sfp_detect_stat *stat = &adapter->sfp_detect_status;

	do {
		rc = al_eth_i2c_byte_read(adapter,
	...
	if (rc) {
		if (stat->valid && !stat->connected) {
			netdev_dbg(adapter->netdev, "%s: Disconnected (no change)\n", __func__);
			*evt = AL_ETH_SFP_DETECT_EVT_NO_CHANGE;
		} else {
			netdev_info(adapter->netdev, "%s: Disconnected\n", __func__);
			*evt = AL_ETH_SFP_DETECT_EVT_DISCONNECTED;
		}
		stat->valid = true;
		stat->connected = false;
		return 0;
	} else if (!stat->valid || !stat->connected) {
		stat->valid = true;
		stat->connected = true;
		netdev_info(adapter->netdev, "%s: Connected\n", __func__);
		*evt = AL_ETH_SFP_DETECT_EVT_CONNECTED;
	} else if ((stat->sfp_10g == sfp_10g) &&
		(stat->sfp_1g == sfp_1g) &&
		(stat->sfp_cable_tech == sfp_cable_tech)) {
		netdev_dbg(adapter->netdev, "%s: Connected (no change)\n", __func__);
		*evt = AL_ETH_SFP_DETECT_EVT_NO_CHANGE;
		return 0;
	} else {
		netdev_info(adapter->netdev, "%s: Connected (changed)\n", __func__);
		*evt = AL_ETH_SFP_DETECT_EVT_CHANGED;
	}

	stat->sfp_10g = sfp_10g;
	stat->sfp_1g = sfp_1g;
	stat->sfp_cable_tech = sfp_cable_tech;

	/* determine device type */
	if (sfp_10g != 0) {
		if (sfp_cable_tech & (SFP_10G_DA_ACTIVE | SFP_10G_DA_PASSIVE)) {
			netdev_info(adapter->netdev,
				   "%s: DA cable detected\n", __func__);
			/* in case of DA need to perform link training */
			stat->lt_en = true;
			stat->an_en = true;
		} else {
			netdev_info(adapter->netdev,
				   "%s: 10G optic sfp detected\n", __func__);
			stat->lt_en = false;
			stat->an_en = false;
		}

		stat->mac_mode = AL_ETH_MAC_MODE_10GbE_Serial;

	} else if (sfp_1g) {
		netdev_info(adapter->netdev,
			   "%s: 1G SGMII sfp detected\n", __func__);

		stat->mac_mode = AL_ETH_MAC_MODE_SGMII;
	} else {
		netdev_err(adapter->netdev, "Unknown sfp inserted\n");
		return -EINVAL;
	}
...
static void al_eth_sfp_detection_apply(
	struct al_eth_adapter	*adapter)
{
	...
	if ((adapter->auto_speed) && ((!adapter->mac_mode_set) ||
		...
		enum alpine_serdes_eth_mode mode =
			(adapter->sfp_detect_status.mac_mode == AL_ETH_MAC_MODE_SGMII) ?
			ALPINE_SERDES_ETH_MODE_SGMII :
			ALPINE_SERDES_ETH_MODE_KR;
...
#endif
	adapter->mac_mode = adapter->sfp_detect_status.mac_mode;
	adapter->mac_mode_set = true;
}
...
static void al_eth_link_status_task(struct work_struct *work)
{
...
#else //CONFIG_SYNO_ALPINE_A0
	enum al_eth_sfp_detect_evt sfp_detect_evt;

	if (adapter->sfp_detection_needed) {
		if (al_eth_sfp_detect(adapter, &sfp_detect_evt))
			goto reschedule;

		switch (sfp_detect_evt) {
		case AL_ETH_SFP_DETECT_EVT_CHANGED:
			al_eth_down(adapter);
		case AL_ETH_SFP_DETECT_EVT_CONNECTED:
			al_eth_sfp_detection_apply(adapter);
			if (al_eth_up(adapter))
				goto reschedule;
			break;
		case AL_ETH_SFP_DETECT_EVT_DISCONNECTED:
			al_eth_down(adapter);
			goto reschedule;
		case AL_ETH_SFP_DETECT_EVT_NO_CHANGE:
			if ((!adapter->sfp_detect_status.valid) ||
				(!adapter->sfp_detect_status.connected))
				goto reschedule;
			break;
		default:
			goto reschedule;
	...
	if ((adapter->mac_mode == AL_ETH_MAC_MODE_10GbE_Serial) &&
	    (adapter->an_en == true) && (adapter->lt_en == true)) {
		al_eth_kr_link_status(adapter);
	} else {
		struct al_eth_link_status status;

		al_eth_serdes_optic_rx_basic_config(&adapter->serdes_obj,
						    adapter->serdes_grp,
						    adapter->serdes_lane);
		al_eth_serdes_optic_tx_basic_config(&adapter->serdes_obj,
						    adapter->serdes_grp,
						    adapter->serdes_lane);
		...
		if ((status.link_up == true) &&
		    (adapter->last_link_status.link_up == false)) {
			netdev_info(adapter->netdev, "%s link status up\n", __func__);

			netif_carrier_on(adapter->netdev);
		} else if ((status.link_up == false) &&
			 (adapter->last_link_status.link_up == true)) {
			netdev_info(adapter->netdev, "%s link status down\n", __func__);
...
#else
	params.eeprom_read = &al_eth_i2c_byte_read;
	params.eeprom_context = adapter;
#endif
...
#else //CONFIG_SYNO_ALPINE_A0
	if (adapter->sfp_detection_needed) {
		adapter->sfp_detect_status.valid = false;
		rc = al_eth_sfp_detect(adapter, &sfp_detect_evt);
		if (rc)
			return rc;

		if (sfp_detect_evt != AL_ETH_SFP_DETECT_EVT_DISCONNECTED)
			al_eth_sfp_detection_apply(adapter);
	}

	if ((!adapter->sfp_detection_needed) ||
		(sfp_detect_evt != AL_ETH_SFP_DETECT_EVT_DISCONNECTED)) {
		rc = al_eth_up(adapter);
		if (rc)
			return rc;
	}
...
//Do nothing
#else
	al_eth_sysfs_terminate(&adapter->pdev->dev);
#endif
...
static void al_eth_get_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
{
	struct al_eth_adapter *adapter = netdev_priv(dev);

	wol->supported = WAKE_MAGIC;
	wol->wolopts = adapter->wol_opts;
}

static int al_eth_set_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
{
	struct al_eth_adapter *adapter = netdev_priv(dev);

	if (wol->wolopts & (~WAKE_MAGIC)) {
		printk("WOL cat not be set\n");
		return -EOPNOTSUPP;
	}

	adapter->wol_opts = wol->wolopts;
	return 0;
}
#endif
	       ...
	       struct al_eth_pkt *hal_pkt, struct sk_buff *skb)
{
	...
	if ((skb->ip_summed == CHECKSUM_PARTIAL) || mss) {
		struct al_eth_meta_data *meta = &tx_info->hal_meta;
		if (mss)
...
al_eth_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (last_frag > (AL_ETH_PKT_MAX_BUFS - 2)) {
		int i;
...
static inline void al_eth_random_addr(struct net_device *dev)
{
...
al_eth_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (ent->driver_data == ALPINE_NIC)
		rc = pcim_iomap_regions(pdev, (1 << 0), DRV_MODULE_NAME);
	else
	...
	if (adapter->board_type == ALPINE_NIC) {
		...
		adapter_pci_cmd = readw(iomap[0] + PCI_COMMAND);
		adapter_pci_cmd |= PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER;
		adapter_pci_cmd &= ~PCI_COMMAND_INTX_DISABLE;

		writew(adapter_pci_cmd, iomap[0] + PCI_COMMAND);
		/* enable forwarding interrupts from eth through pci end point*/
		writel(0x1FFFF, iomap[0] + 0x201210); /* FPGA */

		pr_info("enable msix in the adapter: %x\n", readl(iomap[0] + 0x90));
		adapter->udma_base = iomap[0] + 128 * 0x400;
		adapter->ec_base = iomap[0] + 20 * 0x400;
		adapter->mac_base = iomap[0] + 36 * 0x400;
	} else {
	...
	al_eth_board_params_init(adapter);
	al_eth_function_reset(adapter);
