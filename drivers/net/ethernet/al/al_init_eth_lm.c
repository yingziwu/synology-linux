			     ...
			     enum al_eth_lm_link_mode	*new_mode)
{
...
#else
		rc = lm_context->eeprom_read(lm_context->eeprom_context,
					     SFP_I2C_HEADER_10G_IDX, &sfp_10g);
#endif
		if (rc)
			break;
	...
	if (rc) {
		...
		*new_mode = AL_ETH_LM_MODE_10G_DA;
		...
		lm_context->da_len = 0;
#else
	} else if (sfp_cable_tech & (SFP_10G_DA_ACTIVE | SFP_10G_DA_PASSIVE)) {
		lm_debug("%s: 10G DAC detected\n", __func__);
		*new_mode = AL_ETH_LM_MODE_10G_DA;
#endif
	} else if (sfp_10g != 0) {
...
#else
	lm_context->eeprom_read = params->eeprom_read;
	lm_context->eeprom_context = params->eeprom_context;
#endif
...
#else
	if (status.link_up == AL_FALSE)
		al_eth_led_config(lm_context->adapter, AL_FALSE);
#endif
	if (link_fault) {
		*link_fault = AL_FALSE;
...
#else
	al_eth_led_config(lm_context->adapter, AL_TRUE);
	lm_context->link_establish_failures = 0;
#endif
	*link_up = AL_TRUE;
	return 0;
