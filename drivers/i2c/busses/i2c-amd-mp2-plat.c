...
static int i2c_amd_dma_map(struct amd_i2c_common *i2c_common)
{
	...
	if (i2c_common->msg->flags & I2C_M_RD)
		i2c_common->dma_buf = kzalloc(i2c_common->msg->len, GFP_KERNEL);
	else
		i2c_common->dma_buf = kmemdup(i2c_common->msg->buf, i2c_common->msg->len, GFP_KERNEL);
...
static void i2c_amd_dma_unmap(struct amd_i2c_common *i2c_common)
{
	...
	if (i2c_common->dma_buf && i2c_common->dma_buf != i2c_common->msg->buf) {
		if (i2c_common->msg->flags & I2C_M_RD) {
			memcpy(i2c_common->msg->buf, i2c_common->dma_buf, i2c_common->msg->len);
		}
		kfree(i2c_common->dma_buf);
	}
}
...
static enum speed_enum i2c_amd_get_bus_speed(struct platform_device *pdev)
{
	...
	acpi_speed = I2C_MAX_STANDARD_MODE_FREQ;
