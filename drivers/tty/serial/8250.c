...
static void dwapb_serial_out(struct uart_port *p, int offset, int value)
{
       ...
       offset = map_8250_out_reg(p, offset) << p->regshift;
#if defined(CONFIG_PLAT_ARMADA)  || defined(MY_ABC_HERE) || defined(MY_DEF_HERE)
...
#ifdef MY_ABC_HERE
		if (NULL != syno_test_list && syno_test_list(ch, tty)) {
			goto ignore_char;
		} else if (NULL != syno_get_current && syno_get_current(ch, tty)) {
			goto ignore_char;
		} else if (NULL != funcSYNOMicropGetEvent) {
			funcSYNOMicropGetEvent(tty);
		}
#endif /* MY_ABC_HERE */
