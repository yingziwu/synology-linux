...
static int phy_rtk_sata_set_mode(struct phy *phy, enum phy_mode mode)
{
	...
	if (priv->chip_id == CHIP_ID_RTD1619) {
		writel(desc->index, base + 4);
		for (i=0; i<3; i++) {
			value = read_mdio_reg(0x20+0x40*i, base);
			if (mode)
				value = value | (0x3 << 28);
			else
				value = value & ~(0x3 << 28);
			value = value | 0x11;
			write_mdio_reg(value, base);

			value = read_mdio_reg(0x2a+0x40*i, base);
			if (mode)
				value = value & ~(0x1 << 23);
			else
				value = value | (0x1 << 23);
