...
#define FREQ_MNO_MASK_SET     (0x7FFFE)
#define FREQ_MNO(_r, _m, _n, _o) \
	{ .rate = _r, .val = ((_m) << 4) | ((_n) << 12) | ((_o) << 17) | ((_m) <= 0x18 ? 0x0000c002 : 0x0001c000), }
...
static struct clk_pll pll_ve1 = {
	...
	.freq_mask_set = FREQ_MNO_MASK_SET,
	.clkr.hw.init = &(struct clk_init_data) {
		.name         = "pll_ve1",
		.ops          = &clk_pll_ops,
		.parent_names = default_parent,
...
static struct clk_gate_data cc_gates[] = {
	...
	GATE(RTD1319_CRT_CLK_EN_EMMC_IP, "emmc_ip", NULL, 0x54, 20),
