...
int SYNO_CTRL_HDD_POWERON(int index, int value)
{
	...
	if (gpio_cansleep(HDD_ENABLE_PIN(index))) {
		gpio_set_value_cansleep(HDD_ENABLE_PIN(index), value);
	} else {
...
int SYNO_CHECK_HDD_DETECT(int index)
{
	...
	if (gpio_cansleep(HDD_DETECT_PIN(index))) {
		ret = gpio_get_value_cansleep(HDD_DETECT_PIN(index));
	} else {
...
static void ata_force_link_limits(struct ata_link *link)
{
	...
	} else if (syno_is_hw_version(HW_DS1515) && (9 == link->ap->print_id || 10 == link->ap->print_id)) {
		link->hw_sata_spd_limit = 0x3;
		ata_link_notice(link, "FORCE: PHY spd limit set to 3.0 Gbps\n");
	}
#endif /* MY_ABC_HERE */
									 ...
									 unsigned long timeout)
{
...
#ifdef MY_ABC_HERE
	if(!rc && PWR_PMP_ZERO_WATT_TYPE == syno_get_deep_sleep_pwr_type(ap)) {
		spin_lock_irqsave(ap->lock, flags);
		/* if set ATA_PFLAG_SYNO_IRQ_OFF but not set ATA_PFLAG_SYNO_IRQOFF_PWROFF_DONE,
		 * means do power off now */
		if ((ap->pflags & ATA_PFLAG_SYNO_IRQ_OFF) &&
			!(ap->pflags & ATA_PFLAG_SYNO_IRQOFF_PWROFF_DONE) &&
			IS_SYNO_PMP_CMD(tf)) {
			iIsZeroWatt = 1;
		}
		spin_unlock_irqrestore(ap->lock, flags);
	...
	if (iIsZeroWatt) {
		spin_lock_irqsave(ap->lock, flags);
		/* if it's poweroff case, we must set timeout flag to prevent it
		 * retry again */
...
#ifdef MY_ABC_HERE
		|| (dev->link->ap->pflags & ATA_PFLAG_SYNO_DS_PWROFF)
#endif /* MY_ABC_HERE */
	   )) {
...
int ata_dev_configure(struct ata_device *dev)
{
	 ...
	 * The ncq compatibility of WD5000BPKX on 88SE9215 is not tested.
	 * However, it may have some problem, we disable it.
	 * */
	pdev = to_pci_dev(ap->host->dev);
	...
	/* We disable ncq cmd of Sil3132 esata ports to reduce chance of ata error */
...
#if defined(MY_ABC_HERE)
static int syno_ata_present_print_check(void)
{
	int iRet = 1;
	/* We can't get correct detect pin on RS816 and RS217 */
	if (syno_is_hw_version(HW_RS816) || syno_is_hw_version(HW_RS217)) {
		iRet = 0;
	}
	return iRet;
}
...
int syno_need_force_retry(struct ata_port *ap)
{
	...
	if ((syno_is_hw_version(HW_RS3614xs) || syno_is_hw_version(HW_RS3614rpxs)) &&
		(ap->print_id == 1 || ap->print_id == 2 || ap->print_id == 3)) {
		iRet = 1;
	} else if (syno_is_hw_version(HW_DS916p) && ap->print_id == 5) {
		iRet = 1;
	}
