...
#if defined(MY_DEF_HERE) && defined(CONFIG_COMCERTO_AHCI_PROF)
...
int syno_need_force_retry(struct ata_port *ap)
{
	...
	if (pdev && (pdev->vendor == 0x8086 && pdev->device == 0x8c02) &&
			(syno_is_hw_version(HW_RS3614xs) || syno_is_hw_version(HW_RS3614rpxs))) {
		iRet = 1;
	}
...
struct ata_port *ata_port_alloc(struct ata_host *host)
{
	...
	ap->uiSflags = 0x0;
	ap->iFakeError = 0;
	/* Sometimes Intel ICH SATA can't get the HDD while bootup,
	 * We enable iDetectStat and appling force retry
	 */
	if (syno_need_force_retry(ap)) {
		ap->iDetectStat = 1;
	} else {
		ap->iDetectStat = 0;
	}
...
#if defined(MY_ABC_HERE) && defined(MY_DEF_HERE)
	/* Avoton Model port mapping is not in sequence
	 * So we use syno_libata_index_get to get disk number
	 */
...
#if defined(MY_ABC_HERE) && defined(MY_DEF_HERE)
	if(SYNOX64IsSupportHDDPowerCtrl()) {
		/*switch all the powers on.*/
#ifdef MY_DEF_HERE
