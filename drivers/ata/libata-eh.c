...
void SendPortDisEvent(struct work_struct *work)
{
	...
	funcSYNOSendDiskPortLostEvent(slotNumber, PORT_LOST_DISABLED);
#endif /* MY_ABC_HERE */
...
void SendPortRetryFailedEvent(struct work_struct *work)
{
	...
	funcSYNOSendDiskPortLostEvent(slotNumber, PORT_LOST_RETRY_FAILED);
END:
#endif /* MY_ABC_HERE */
	return;
}
...
void ata_scsi_port_error_handler(struct Scsi_Host *host, struct ata_port *ap)
{
...
#ifdef MY_ABC_HERE
	int iDeepTries = SYNO_SATA_RETRY_TIMES;
#endif
#endif
#ifdef MY_ABC_HERE
	...
	// RS3614xs & RS3614rpxs workaround: add 1 more detect try and deep try at port3 to avoid can't attach disk
	if (syno_need_force_retry(ap) && 3 == ap->print_id) {
		iDetectTries += 1;
		iDeepTries += 1;
	}
...
deep_repeat:
	...
	if(1 == iIsIRQOff) {
		/* not pmp link, and "0 < g_internal_hd_num" we need delay 7s
		 * between every disks
		 */
		if (!ap->nr_pmp_links && 0 < g_internal_hd_num) {
		...
		if (pAp_master->nr_pmp_links) {
			SynoEunitFlagSet(pAp_master, 1, ATA_PFLAG_SYNO_DS_WAKING);
		} else {
			spin_lock_irqsave(pAp_master->lock, flags);
...
acquire_repeat:
 ...
 repeat:
	...
	if (ap->pflags & ATA_PFLAG_SYNO_DS_WAKING) {
		if (pAp_master->pflags & ATA_PFLAG_SYNO_DS_WAKING) {
			/* If master has no disks, it wouldn't clear flag itself. */
			pAp_master->pflags &= ~ATA_PFLAG_SYNO_DS_WAKING;
		}
...
syno_ata_do_remap(struct ata_queued_cmd *qc, u8 blLBA48)
{
	...
	if (!buf) {
		ata_dev_printk(qc->dev, KERN_ERR, "%s:%d Failed to allocate remap page memory, so use zero page instead.\n", __FILE__, __LINE__);
	...
	ret = ata_exec_internal(qc->dev, &tf, NULL, DMA_TO_DEVICE, p, size, 0);
...
static int ata_eh_handle_dev_fail(struct ata_device *dev, int err)
{
...
#ifdef MY_ABC_HERE
	{
		...
		if (1 == ehc->tries[dev->devno] && -EIO == err) {
			...
			if (pdev && (pdev->vendor == 0x8086 &&
						(pdev->device == 0x8c02 || pdev->device == 0x1f32 || pdev->device == 0x1f22))) {
				u32 scontrol = 0;
	...
	case -EIO:
		if (ehc->tries[dev->devno] == 1) {
			...
			// don't down link speed on this chip
			if (link && link->ap && syno_is_alpine_internal_ahci(link->ap)) {
				break;
			}
#else
#error You must provide internal or external AHCI handling logic
#endif
#endif /* MY_DEF_HERE */
...
#ifdef SYNO_SATA_PM_FIRST_PORT_DELAY
