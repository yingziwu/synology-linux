...
{
	u32 scontrol = 0x0;
	u32 edma_irr_mask_reg = 0x0;
	void __iomem *port_mmio = mv_ap_base(ap);

	DBGMESG("disk %d do mv missing workaround, re-enable interrupts, clear error\n", ap->print_id);
			...
			unsigned long deadline)
{
	...
	SYNO_HORKAGE_STAGE HorkageStage = UNKNOW_HORKAGE_STAGE;
	unsigned int uiSpdLimit = 0;
	unsigned int uiSpd = 0;
#endif /* MY_ABC_HERE */
	void __iomem *mmio = hpriv->base;
...
#if defined(MY_ABC_HERE) && defined(MY_ABC_HERE)
	uiSpdLimit = link->sata_spd_limit;
	uiSpd = host_link->sata_spd;
	HorkageStage = SynoGetHorkageStage(link);
	if (FIRST_APPLY_15G == HorkageStage) {
		DBGMESG("ata port %d FIRST_APPLY_15G, don't force 3G\n",
				ap->print_id);
	} else {
		/* set this two value will cause sata_set_spd_needed(..) function return false,
		 * so speed will not be negotiated  */
		DBGMESG("ata port %d Do Fix MV IRQ MISSING Workaround, don't negotiate speed to 1.5G\n",
				ap->print_id);
...
#if defined(MY_ABC_HERE) && defined(MY_ABC_HERE)
		if (rc) {
			if (ALREADY_APPLY_15G == HorkageStage) {
				/* "spd" set 0x2 is meaning sata GEN_II speed, detail you can see SControl Register [7:4]
				 * Setting it to 0x2 is also ref. in "ata_do_link_spd_horkage()" and
				 * "sata_set_spd_needed(..) -> sata_set_spd_needed(..)"
				 * "spd_limit" set 0x1 is ref. in "ata_do_link_spd_horkage()" and
				 * "sata_set_spd_needed(..) -> sata_set_spd_needed(..)".
				 * So we set "spd" to 0x2 and "spd_limit" to 0x1. Then sata_link_hardreset(..) will
				 * negotiate the speed to GEN_I(1.5G) and set appropriate registers.
				 *  */
				DBGMESG("ata port %d ALREADY_APPLY_15G set spd_limit 0x%x, spd 0x%x to reset to 1.5G\n",
						ap->print_id, 0x1, 0x2);
				link->sata_spd_limit = 0x1;
				host_link->sata_spd = 0x2;
				rc = sata_link_hardreset(link, timing, deadline + extra,
						&online, NULL);
				rc = online ? -EAGAIN : rc;
			}
...
#if defined(MY_ABC_HERE) && defined(MY_ABC_HERE)
END:
	link->sata_spd_limit = uiSpdLimit;
	host_link->sata_spd = uiSpd;
	return rc;
#else /* MY_ABC_HERE */
	return rc;
#endif /* MY_ABC_HERE */
