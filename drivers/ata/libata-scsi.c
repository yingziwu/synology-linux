...
#elif defined(CONFIG_ARCH_GEN3)
		if(0 ==  SYNOEvansportIsBoardNeedPowerUpHDD(ap->print_id)) {
			/* this port isn't support */
			goto END;
		}
#elif defined(MY_DEF_HERE)
...
DEVICE_ATTR(syno_power_ctrl, S_IWUGO, NULL, syno_power_ctrl_store);
...
DEVICE_ATTR(syno_deep_sleep_ctrl, S_IWUGO, NULL, syno_deep_sleep_ctrl_store);
...
#else
			SYNO_CTRL_HDD_POWERON(ap->print_id, blPowerOn);
#endif /* MY_DEF_HERE */
			SleepForLatency();
		} else {
			spin_lock_irqsave(ap->lock, flags);
...
/* check if this port still have errors or activeties
 *
 ...
 */
int
syno_libata_is_port_acting(struct Scsi_Host *host)
{
	struct ata_port *ap = NULL;
	struct ata_port *pAp_master = NULL;
	unsigned long flags = 0;
	...
	if (NULL == host) {
		DBGMESG("Null scsi host, can't check is port acting\n");
		goto END;
	}
	...
	if(0 == iIsSynoPmCtlSupport(ap)) {
		/* FIXME: some platform (ex. x64) If they check acting in __syno_host_power_ctl_work(..)
		 * sometimes will cause some unkonwn waring (ex. IRQ 19; nobody cared). This seems timing
		 * issue, and can't be probably fixed. So we add this check to skip.  This include more case:
		 * ex. iIsSynoDeepSleepSupport */
		DBGMESG("disk %d doesn't support pm control, treat it not acting\n", ap->print_id);
	} else if (1 == iIsSynoIRQOff(ap)) {
		DBGMESG("disk %d is IRQ OFF, treat it not acting\n", ap->print_id);
	} else if (!ap->nr_pmp_links) {
		/* still have activeties */
		if (ap->nr_active_links) {
			DBGMESG("disk %d have activeties, active links %d\n", ap->print_id, ap->nr_active_links);
			iRet = 1;
		}
		spin_lock_irqsave(ap->lock, flags);
		/* have errors */
		if (ap->pflags) {
			DBGMESG("disk %d have errors pflags 0x%x\n", ap->print_id, ap->pflags);
			iRet = 1;
		}
		spin_unlock_irqrestore(ap->lock, flags);
		/* if have activities or errors, goto END */
		if (1 == iRet) {
			goto END;
		}
	} else {
		...
		pAp_master = SynoEunitFindMaster(ap);
		if (NULL == pAp_master) {
			printk("Can't find syno Eunit master to do port acting check\n");
			goto END;
		}
		iRet = SynoIsEunitPortActing(pAp_master);
		goto END;
	}
...
SYNO_DEEP_SLEEP_PWR_TYPE syno_get_deep_sleep_pwr_type(struct ata_port *ap)
{
	...
	if(ap->nr_pmp_links) {
		...
		/* 12 Bay EUnit not support deep sleep in manual mode */
				...
				IS_SYNOLOGY_RX1214(ap->PMSynoUnique) || IS_SYNOLOGY_DX1215(ap->PMSynoUnique)) {
...
syno_libata_set_deep_sleep(struct Scsi_Host *host, const u8 blSet)
{
	...
	ap = ata_shost_to_port(host);
	if (NULL == look_up_scsi_dev_from_ap(ap) && !(ap->uiSflags & ATA_SYNO_FLAG_PROBE_RETRY)) {
