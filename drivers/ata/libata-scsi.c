...
look_up_scsi_dev_from_ap(struct ata_port *ap)
{
	...
	ata_for_each_link(link, ap, EDGE) {
		ata_for_each_dev(dev, link, ALL) {
			if (dev->sdev && (SDEV_RUNNING == dev->sdev->sdev_state || SDEV_QUIESCE == dev->sdev->sdev_state)) {
				sdev = dev->sdev;
				return sdev;
			}
		}
...
syno_gpio_task(struct work_struct *pWork)
{
	...
	if (pTask->blIsRead) {
		gpio_func = syno_sata_pmp_read_gpio;
	} else {
		gpio_func = syno_sata_pmp_write_gpio;
	}
				   ...
				   const char *buf, size_t len)
{
	...
	while (ad->model_num) {
		if (!strn_pattern_cmp(ad->model_num, model_num, '*')) {
			if (ad->model_rev == NULL || !strn_pattern_cmp(ad->model_rev, model_rev, '*')) {
				if (input) {
