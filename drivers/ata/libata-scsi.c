...
#endif /* MY_ABC_HERE */
#if defined(MY_ABC_HERE) || defined(MY_ABC_HERE) || defined(MY_ABC_HERE) || defined(MY_DEF_HERE)
...
static int SynoIsEunitPortActing(const struct ata_port *pAp_master)
{
	...
	for (i = 0; i < pAp_master->host->n_ports; i++) {
		...
		/* Step 3. Check nr_active_links and pflags */
...
SYNO_DEEP_SLEEP_PWR_TYPE syno_get_deep_sleep_pwr_type(struct ata_port *ap)
{
	...
	if(ap->nr_pmp_links) {
		...
		/* 12 Bay EUnit not support deep sleep in manual mode */
				...
				IS_SYNOLOGY_DX1215(ap->PMSynoUnique) || IS_SYNOLOGY_DX1215II(ap->PMSynoUnique)) {
...
syno_libata_set_deep_sleep(struct Scsi_Host *host, const u8 blSet)
{
	...
	switch (PwrType) {
		case PWR_COMMON_TYPE:
			if (syno_is_hw_version(HW_DS218j) && blPowerOn) {
				if (2 == pAp_master->print_id) {
					struct ata_port *ap1 = pAp_master->host->ports[0];
					if (ap1 && (0 < (ap1->pflags & ATA_PFLAG_SYNO_IRQ_OFF))) {
						SYNO_CTRL_HDD_POWERON(1, blPowerOn);
						mdelay(jiffies_to_msecs(WAKEINTERVAL));
					}
				}
			}
