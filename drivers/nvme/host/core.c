...
int nvme_lba_write_zero(struct nvme_ns *ns, u64 lba)
{
	...
	error = nvme_submit_sync_cmd(ns->queue, &c, buf, length);
...
#ifdef MY_DEF_HERE
static void nvme_config_discard(struct nvme_ns *ns, u8 dlfeat)
#else
...
#endif /* MY_DEF_HERE */
{
	...
	if (ctrl->quirks & NVME_QUIRK_DISCARD_ZEROES ||
	    (dlfeat & NVME_NS_DLFEAT_VAL_MASK) ==
	    	NVME_NS_DLFEAT_ZEROED)
#else
	if (ctrl->quirks & NVME_QUIRK_DISCARD_ZEROES)
#endif /* MY_DEF_HERE */
...
static int nvme_revalidate_disk(struct gendisk *disk)
{
...
#ifdef MY_DEF_HERE
	if (is_snv3400_series && 0 > nvme_get_log_page(ns->ctrl, &smart_log)) {
		dev_warn(disk_to_dev(ns->disk), "%s: Get smart log failure\n",
				__func__);
		return -ENODEV;
	}
#endif /* MY_DEF_HERE */
	...
	if (is_snv3400_series &&
	    smart_log->critical_warning & NVME_SMART_CRIT_MEDIA) {
		set_disk_ro(disk, 1);
		dev_warn(ns->ctrl->dev,
			 "%s: set disk to read-only [crit warn: 0x%02x]\n",
