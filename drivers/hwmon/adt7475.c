...
#ifdef MY_DEF_HERE
#define SYNO_ADT_RETRY_COUNT 5
u8 adt7475_read_retry(const struct i2c_client *client, u8 reg)
{
	int i;
	s32 iRet = 0;

	for (i = 0; i < SYNO_ADT_RETRY_COUNT; ++i) {
		iRet = i2c_smbus_read_byte_data(client, reg);
		if (0 <= iRet) {
			break;
		}
	}
...
#define adt7475_read(reg) adt7475_read_retry(client, reg)
#else /* MY_DEF_HERE */
